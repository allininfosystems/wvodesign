...
class Product extends \Dsync\Dsync\Model\Entity\AbstractEntity
{
    ...
    protected function processCreate()
    {
        ...
        $this->createProductData($data, $productData);
     ...
     * @param array $productData
     */
    protected function createProductData($data, &$productData)
    {
        ...
        foreach ($product->getData() as $field => $value) {
            ...
            if (is_array($value)) {
                ...
                if ($value && empty($option) && $option != '0') {
                    $error = __('Invalid option ID specified for %1 (%2).', $field, $value);
     ...
     * @param array $data
     */
    protected function updateProductData(&$data)
    {
        // add websites if they are set
        if (isset($data['websites'])) {
            if ($websites = $data['websites']) {
    ...
    protected function getCategoryIds($categories)
    {
        ...
        foreach ($categories as $categoryName) {
            ...
            if (is_numeric($categoryName)) {
                $category = $this->categoryFactory->create()->load($categoryName);
            } else {
