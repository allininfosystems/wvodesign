...
\*======================================================================*/
/**
* digipost_007 Import Post module
*
...
class digipost_007 extends digipost_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_digipost_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'threadid',			$thread_ids_array["$post_details[messageID]"]);
			$try->set_value('mandatory', 'userid',				$user_ids_array["$post_details[authorID]"]);
			$try->set_value('mandatory', 'importthreadid',		$post_details['messageID']);


			// Non Mandatory
			if($post_details['parentID'])
			{
				$try->set_value('mandatory', 'threadid',			$this->get_thread_id($Db_target, $target_database_type, $target_table_prefix, $post_details['parentID'],$post_details['forumID']));
			...
			$try->set_value('nonmandatory', 'username',			$user_name_array["$post_details[authorID]"]);
			$try->set_value('nonmandatory', 'title',			$post_details['messageSubject']);
			$try->set_value('nonmandatory', 'dateline',			($post_details['messageStamp'] / 1000));
			$try->set_value('nonmandatory', 'pagetext',			$this->html_2_bb($this->digipost_html($post_details['messageText'])));
			...
			$try->set_value('nonmandatory', 'importpostid',		$post_details['messageID']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: post -> ' . $post_details['messageSubject']);
