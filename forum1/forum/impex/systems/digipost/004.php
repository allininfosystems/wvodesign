...
\*======================================================================*/
/**
* digipost_004 Import User module
*
...
*
*/
class digipost_004 extends digipost_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '003';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_digipost_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			$try->set_value('nonmandatory', 'joindate',			($user_details['userStamp'] / 1000));
			...
			$try->add_default_value('signature', 				addslashes($this->digipost_html($this->html_2_bb($user_details['signature']))));
			$try->add_default_value('Interests', 				addslashes(trim($user_details['age']) . ' ' . trim($user_details['gender']) . '::' . trim($user_details['comments'])));
