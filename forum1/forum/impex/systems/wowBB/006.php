...
class wowBB_006 extends wowBB_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_wowBB_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'title',				$thread_details['topic_name']);
			...
			// Non Mandatory
			$try->set_value('nonmandatory', 'dateline',			strtotime($thread_details['topic_date_time']));
			$try->set_value('nonmandatory', 'postusername',		$user_name_array["$thread_details[topic_starter_id]"]);
			$try->set_value('nonmandatory', 'postuserid',		$user_ids_array["$thread_details[topic_starter_id]"]);
			...
			if ($thread_details['topic_type'] == 2 AND $thread_details['topic_status'] == 1)
			{
			...
			*/
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: thread -> ' . $thread_details['topic_name']);
