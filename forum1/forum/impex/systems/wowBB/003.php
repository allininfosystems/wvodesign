...
class wowBB_003 extends wowBB_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$usergroup_array 	= $this->get_wowBB_usergroup_details($Db_source, $source_database_type, $source_table_prefix, $usergroup_start_at, $usergroup_per_page);
		...
		foreach ($usergroup_array as $usergroup_id => $usergroup_details)
		{
			...
			// Non Mandatory
			$try->set_value('nonmandatory', 'title',				$usergroup_details['user_group_name']);
			$try->set_value('nonmandatory', 'description',			$usergroup_details['user_group_name']);
			
			$try->set_value('nonmandatory', 'usertitle',			$usergroup_details['user_group_title']);
			/*
			...
			*/
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: usergroup -> ' . $usergroup_details['user_group_name']);
