...
\*======================================================================*/
/**
* wowBB_011 Import Attachment module
*
* @package			ImpEx.wowBB
...
*
*/
class wowBB_011 extends wowBB_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '007';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$attachment_array 	= $this->get_wowBB_attachment_details($Db_source, $source_database_type, $source_table_prefix, $attachment_start_at, $attachment_per_page);
		...
		{
			$attachment_postid = $this->get_wowBB_attachment_postid($Db_source, $source_database_type, $source_table_prefix, $attachment_id);
			
			$try = (phpversion() < '5' ? $attachment_object : clone($attachment_object));
			// Mandatory
			$try->set_value('mandatory', 'filename',				$attachment_details['file_name']);
			$try->set_value('mandatory', 'filedata',				$attachment_details['file_contents']);
			$try->set_value('mandatory', 'importattachmentid',		$attachment_details['attachment_id']);

			// Non Mandatory
			$try->set_value('nonmandatory', 'userid',				$user_ids_array["$attachment_details[user_id]"]);
			$try->set_value('nonmandatory', 'dateline',				strtotime($attachment_details['upload_date']));
			$try->set_value('nonmandatory', 'visible',				'1');
			$try->set_value('nonmandatory', 'counter',				$attachment_details['downloads']);
			$try->set_value('nonmandatory', 'filesize',				strlen($attachment_details['file_contents']));
			$try->set_value('nonmandatory', 'postid',				$attachment_postid);
			$try->set_value('nonmandatory', 'filehash',				md5($attachment_details['file_contents']));
			/*
			...
			*/
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: attachment -> ' . $attachment_details['file_name']);
