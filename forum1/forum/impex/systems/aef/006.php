...
class aef_006 extends aef_000
{
	var $_dependent = '005';

	function aef_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}topics", 'tid', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'title',				html_entity_decode($data['topic']));
			$try->set_value('mandatory', 'importforumid',		$data['t_bid']);
			$try->set_value('mandatory', 'importthreadid',		$import_id);
			$try->set_value('mandatory', 'forumid',				$forum_ids_array["$data[t_bid]"]);

			// Non mandatory
			$try->set_value('nonmandatory', 'visible',			'1');
			$try->set_value('nonmandatory', 'open',				'1');
			$try->set_value('nonmandatory', 'sticky',			$data['t_sticky']);
			$try->set_value('nonmandatory', 'notes',			html_entity_decode($data['t_description']));
			$try->set_value('nonmandatory', 'views',			$data['n_views']);
			$try->set_value('nonmandatory', 'replycount',		$data['n_posts']);
