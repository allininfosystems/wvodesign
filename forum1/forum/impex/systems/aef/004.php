...
class aef_004 extends aef_000
{
	var $_dependent = '003';

	function aef_004(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			if ($data['u_member_group'] != 0)
			{
				$try->set_value('mandatory', 'usergroupid',				$user_group_ids_array["$data[u_member_group]"]);
			}
			else
			{
			...
			// Non mandatory
			$try->set_value('nonmandatory', 'pmunread',				$data['unread_pm']);
			$try->set_value('nonmandatory', 'pmtotal',				$data['pm']);
			$try->set_value('nonmandatory', 'password',				$data['password']); // un-useable salt, so use this for a random
			$try->set_value('nonmandatory', 'joindate',				$data['r_time']);
			$try->set_value('nonmandatory', 'ipaddress',			$data['r_ip']);
			...
			$year 	= substr($data['birth_date'], 0, 4);
			$month 	= substr($data['birth_date'], 5, 2);
			$day 	= substr($data['birth_date'], 8, 2);
			
			$try->set_value('nonmandatory', 'birthday',				"{$month}-{$day}-{$year}");
			$try->set_value('nonmandatory', 'birthday_search',		$data['birth_date']);
