...
*
*/
class DCFm_005 extends DCFm_000
{
	var $_dependent 	= '004';
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if ($proceed)
		{
			...
			$displayobject->update_html($displayobject->make_description('The importer will now import all the forums as forums, opposed to making the top level forums categories.'));
			$displayobject->update_html($displayobject->make_input_code("Forums to import per cycle (must be greater than 1)","forumsperpage", 1000));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Sort out everything as a forum
		$forum_array  	=  $this->get_DCFm_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
		...
		foreach ($forum_array as $forum_id => $forum)
		{
			...
			$try->set_value('nonmandatory', 'visible', 			$this->option2bin($forum['status']));
			...
			if($try->is_valid())
			{
				...
				else
				{
					$sessionobject->add_error('warning',
											 $this->_modulestring,
											 get_class($this) . "::import_category failed for " . $try->get_value('mandatory','title') . " get_DCFm_categories_details was ok.",
											 'Check database permissions and user table');
					$sessionobject->set_session_var($class_num . '_objects_failed',$sessionobject->get_session_var($class_num . '_objects_failed') + 1 );
					$displayobject->display_now("<br />Got category " . $try->get_value('mandatory','title'). " and <b>DID NOT</b> imported to the " . $target_database_type . " database");
				}
			}
			else
			{
