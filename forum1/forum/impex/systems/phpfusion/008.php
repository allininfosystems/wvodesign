...
class phpfusion_008 extends phpfusion_000
{
	...
	function phpfusion_008(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$poll_array 	= $this->get_phpfusion_poll_details($Db_source, $source_database_type, $source_table_prefix, $poll_start_at, $poll_per_page);
		...
		foreach ($poll_array as $poll_id => $poll_details)
		{
			...
			$try->set_value('nonmandatory', 'timeout',			$poll_details['poll_ended']);
			$try->set_value('nonmandatory', 'numberoptions',	$poll_details['numberoptions']);
			$try->set_value('nonmandatory', 'active',			'1');

			foreach($poll_details['voters'] AS $phpfusion_user_id => $vote)
			{
				$voter = $user_ids_array[$phpfusion_user_id];
			...
			if($try->is_valid())
			{
				...
				if($result)
				{
					// TODO: Going to be the wrong thread, though where is the pollid=threaid ?
