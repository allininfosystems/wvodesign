...
\*======================================================================*/
/**
* ipb3_010 Import Moderator module
*
* @package			ImpEx.ipb3
...
*
*/
class ipb3_010 extends ipb3_000
{
	var $_dependent 	= '007';

	function ipb3_010(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$category_ids_array	= $this->get_category_ids($Db_target, $target_database_type, $target_table_prefix);

		$forumids = $forum_ids_array + $category_ids_array;

		// Display count and pass time
		$displayobject->display_now("<h4>{$displayobject->phrases['importing']} " . $count . " {$displayobject->phrases['users']}</h4><p><b>{$displayobject->phrases['from']}</b> : " . $moderator_start_at . " ::  <b>{$displayobject->phrases['to']}</b> : " . ($moderator_start_at + $count) . "</p>");
		...
		foreach ($moderator_array as $moderator_id => $moderator_details)
		{
			...
			$moderator_details['forum_id'] = str_replace(',','',$moderator_details['forum_id']);

			$try->set_value('mandatory', 'forumid',					$forumids["$moderator_details[forum_id]"]);
		...
		// Check for page end
		if ($count == 0 OR $count < $moderator_per_page)
		{
