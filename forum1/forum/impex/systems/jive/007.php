...
class jive_007 extends jive_000
{
	var $_dependent = '003';

	function jive_007(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_jive_PM_details($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $pm_id => $pm)
		{
			...
			$userid 	= $idcache->get_id('user', $pm['recipentID']);
			$username	= $idcache->get_id('username', $pm['recipentID']);
			unset($touserarray);
			...
			$vB_pm_text->set_value('nonmandatory', 'dateline',		$pm['pMessageDate']);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm->set_value('mandatory', 'userid',    			$idcache->get_id('user', $pm['recipentID']));
					$vB_pm->set_value('mandatory', 'importpmid',  		$pm_id);

					// to is 0 from is -1
					$vB_pm->set_value('nonmandatory', 'folderid',		($pm['folderID'] == 1 ? '-1' : '0'));
					$vB_pm->set_value('nonmandatory', 'messageread',		$pm['readStatus']);
					
					if($vB_pm_text->is_valid())
					{
						if($vB_pm->import_pm($Db_target, $t_db_type, $t_tb_prefix))
						{
							if(shortoutput)
							{
