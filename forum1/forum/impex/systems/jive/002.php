...
class jive_002 extends jive_000
{
	var $_dependent = '001';

	function jive_002(&$displayobject)
	{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$try->import_usergroup($Db_target, $t_db_type, $t_tb_prefix);
			unset($try);
		}
		else
		{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_jive_usergroup_details($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
