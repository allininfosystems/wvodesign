...
\*======================================================================*/
/**
* beehive_006 Import Thread module
*
...
*
*/
class beehive_006 extends beehive_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	var $_modulestring 	= 'Import Thread';

	function beehive_006()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 		= $this->get_beehive_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			// Non Mandatory
			$try->set_value('nonmandatory', 'postusername',		$user_name_array["$thread_details[BY_UID]"]);
			$try->set_value('nonmandatory', 'postuserid',		$user_ids_array["$thread_details[BY_UID]"]);
			$try->set_value('nonmandatory', 'dateline',			strtotime($thread_details['MODIFIED']));
			
			if($thread_details['STICKY'] == 'N')
			{
			...
			$try->set_value('nonmandatory', 'replycount',		$thread_details['LENGTH']);
