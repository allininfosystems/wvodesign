...
\*======================================================================*/
/**
* beehive_004 Import User module
*
...
*
*/
class beehive_004 extends beehive_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '003';
	var $_modulestring 	= 'Import User';

	function beehive_004()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_beehive_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);

		$ugida = $this->get_imported_group_ids($Db_target, $target_database_type, $target_table_prefix);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'usergroupid',			$this->iif($ugida["$user_details[usergroupid]"], $ugida["$user_details[usergroupid]"], $ugida[69]));
			$try->set_value('mandatory', 'username',			$user_details['LOGON']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: user -> ' . $user_details['LOGON']);
