...
\*======================================================================*/
/**
* CHCForum_006 Import Thread module
*
...
*
*/
class CHCForum_006 extends CHCForum_000
{
	var $_dependent 	= '005';

	function CHCForum_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_CHCForum_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'importforumid',	$thread_details['conference_id']);
			$try->set_value('mandatory', 'title',			$thread_details['thread_subject']);
			$try->set_value('mandatory', 'importthreadid',	$thread_id);
			$try->set_value('mandatory', 'forumid',			$forum_ids_array["$thread_details[conference_id]"]);

			// Non Mandatory
			$try->set_value('nonmandatory', 'notes',		'Imported thread');
			$try->set_value('nonmandatory', 'visible',		($thread_details['hidden'] == 0 ? 1 : 0 ));
			$try->set_value('nonmandatory', 'sticky',		$thread_details['sticky']);
			$try->set_value('nonmandatory', 'views',		$thread_details['thread_views']);
			$try->set_value('nonmandatory', 'open',			($thread_details['thread_closed'] == 0 ? 1 : 0));
			$try->set_value('nonmandatory', 'replycount',	$thread_details['thread_replies']);
			$try->set_value('nonmandatory', 'postusername',	$user_name_array["$thread_details[thread_author]"]);
			$try->set_value('nonmandatory', 'postuserid',	$user_ids_array["$thread_details[thread_author]"]);
			$try->set_value('nonmandatory', 'dateline',		$thread_details['thread_datetime']);
