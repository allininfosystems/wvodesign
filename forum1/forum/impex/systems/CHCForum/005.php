...
\*======================================================================*/
/**
* CHCForum_005 Import Forum module
*
...
*
*/
class CHCForum_005 extends CHCForum_000
{
	var $_dependent 	= '004';

	function CHCForum_005(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_CHCForum_cat_details($Db_source, $source_database_type, $source_table_prefix);
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$cat['sorting']);
				...
				$try->set_value('nonmandatory', 'description',		$cat['cat_description']);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_CHCForum_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'title', 				$forum['conference_name']);
				$try->set_value('mandatory', 'displayorder',		$forum['sorting']);
				$try->set_value('mandatory', 'importforumid',		$forum['uid']);
				$try->set_value('mandatory', 'importcategoryid',	'0');
				$try->set_value('mandatory', 'options',				$this->_default_forum_permissions);
				
				$try->set_value('nonmandatory', 'description', 		$forum['conference_desc']);
