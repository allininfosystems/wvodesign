...
class CHCForum_004 extends CHCForum_000
{
	...
	function CHCForum_004(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_CHCForum_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			// Non Mandatory
			#$try->set_value('nonmandatory', 'birthday',		$user_details['date_of_birth']);
			#$try->set_value('nonmandatory', 'birthday_search',	$user_details['date_of_birth']);
			$try->set_value('nonmandatory', 'options',			$this->_default_user_permissions);
			$try->set_value('nonmandatory', 'msn',				$user_details['tx_chcforum_msn']);
			$try->set_value('nonmandatory', 'aim',				$user_details['tx_chcforum_aim']);
			$try->set_value('nonmandatory', 'yahoo',			$user_details['tx_chcforum_yahoo']);
			$try->set_value('nonmandatory', 'homepage',			$user_details['www']);
			$try->_password_md5_already = false;
			$try->set_value('nonmandatory', 'password',			trim($user_details['password'])); // It's stored in plain text ...
			...
			$try->set_value('nonmandatory', 'joindate',			$user_details['crdate']);
