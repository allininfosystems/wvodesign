...
*
*/
class ldu_005 extends ldu_000
{
	var $_dependent 	= '004';

	function ldu_005(&$displayobject)
	{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$try->set_value('mandatory', 'title', 				'Default LDU cat');
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$forum_array 	= $this->get_ldu_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
		...
		foreach ($forum_array as $forum_id => $forum_details)
		{
			...
			$try->set_value('mandatory', 'importforumid',		$forum_details['fs_id']);
			$try->set_value('mandatory', 'title',				$forum_details['fs_title']);
			$try->set_value('mandatory', 'options',				$this->_default_forum_permissions);
			$try->set_value('mandatory', 'displayorder',		$forum_details['fs_order']);
			$try->set_value('mandatory', 'parentid',			$cat_ids_array[1]);

			// Non Mandatory
			$try->set_value('nonmandatory', 'description',		$forum_details['fs_desc']);
			/*
