...
*
*/
class ldu_007 extends ldu_000
{
	var $_dependent 	= '006';

	function ldu_007(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_ldu_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'threadid',			$thread_ids_array["$post_details[fp_topicid]"]);
			$try->set_value('mandatory', 'importthreadid',		$post_details['fp_topicid']);
			if ($user_ids_array["$post_details[fp_posterid]"])
			{
				$try->set_value('mandatory', 'userid',				$user_ids_array["$post_details[fp_posterid]"]);
			}
			else
			{
			...
			$try->set_value('nonmandatory', 'ipaddress',		$post_details['fp_posterip']);
			$try->set_value('nonmandatory', 'showsignature',	'1');
			$try->set_value('nonmandatory', 'allowsmilie',		'1');
			$try->set_value('nonmandatory', 'pagetext',			$post_details['fp_text']);
			$try->set_value('nonmandatory', 'dateline',			$post_details['fp_creation']);
			$try->set_value('nonmandatory', 'username',			$post_details['fp_postername']);
