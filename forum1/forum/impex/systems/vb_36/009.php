...
*
*/
class vb_36_009 extends vb_36_000
{
	var $_dependent 	= '004';

	function vb_36_009(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($pm_array as $pm_id => $details)
		{
			...
			{
				$new_id = $idcache->get_id('user', $old_user_id);
				$touserarray[$new_id] = $username;
			}
					...
					foreach ($pms as $pm => $details)
					{
						...
						if($pm->is_valid())
						{
							...
							else
							{
								...
								$displayobject->display_now("<br />{$displayobject->phrases['failed']} :: {$displayobject->phrases['pm_not_imported']} :-> " . $try->_failedon);
			...
			{
				$displayobject->display_now("<br />{$displayobject->phrases['invalid_object']}" . $pm_text->_failedon);
				$sessionobject->add_error($Db_target, 'invalid', $class_num, $pm_id, $displayobject->phrases['invalid_object'], $try->_failedon);
				$sessionobject->set_session_var($class_num . '_objects_failed',$sessionobject->get_session_var($class_num. '_objects_failed') + 1 );
			}
			unset($pm_text);
