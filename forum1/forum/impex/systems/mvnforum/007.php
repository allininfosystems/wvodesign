...
class mvnforum_007 extends mvnforum_000
{
	var $_dependent = '006';

	function mvnforum_007($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'threadid',			$idcache->get_id('thread', $data['ThreadID']));
			$try->set_value('mandatory', 'importthreadid',		$data['ThreadID']);
			$try->set_value('mandatory', 'userid',				$idcache->get_id('user', $data['MemberID']));

			// Non mandatory
			$try->set_value('nonmandatory', 'attach',			$data['PostAttachCount']);
			$try->set_value('nonmandatory', 'visible',			'1'); //PostOption PostStatus?
			$try->set_value('nonmandatory', 'showsignature',	'1');
			$try->set_value('nonmandatory', 'ipaddress',		$data['PostCreationIP']);
			$try->set_value('nonmandatory', 'allowsmilie',		'1');
			$try->set_value('nonmandatory', 'pagetext',			$data['PostBody']);
			$try->set_value('nonmandatory', 'dateline',			strtotime($data['PostCreationDate']));
			$try->set_value('nonmandatory', 'title',			$data['PostTopic']);
			$try->set_value('nonmandatory', 'username',			$data['MemberName']);
			$try->set_value('nonmandatory', 'parentid',			$idcache->get_id('post', $data['parentid']));
			$try->set_value('nonmandatory', 'importpostid',		$data['PostID']);
			...
			if($try->is_valid())
			{
				if($try->import_post($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['post'] . ' -> ' . $data['PostTopic']);
					}
