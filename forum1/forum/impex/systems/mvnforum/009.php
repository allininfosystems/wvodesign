...
class mvnforum_009 extends mvnforum_000
{
	var $_dependent = '007';

	function mvnforum_009($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}Attachment", 'AttachID', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			if(!is_file($dir . '/' . $data['AttachID'] . '.mvn'))
			{
				$displayobject->display_now("<br /><b>{$displayobject->phrases['source_file_not']} </b> :: {$data['AttachFilename']}" . '/' . $data['AttachID'] . '.mvn');
				$sessionobject->add_error($Db_target, 'warning', $class_num, $import_id, $displayobject->phrases['attachment_not_imported'], $data['AttachID'] . ' - ' . $displayobject->phrases['attachment_not_imported_rem_1']);
				$sessionobject->set_session_var($class_num . '_objects_failed',$sessionobject->get_session_var($class_num. '_objects_failed') + 1 );
				continue;
			}

			$file = $this->vb_file_get_contents($dir . '/' . $data['AttachID'] . '.mvn');			
			
			// Mandatory
			$try->set_value('mandatory', 'importattachmentid',		$data['AttachID']);
			$try->set_value('mandatory', 'filename',				$data['AttachFilename']);
			$try->set_value('mandatory', 'filedata',				$file);

			// Non mandatory
			$try->set_value('nonmandatory', 'extension',			substr($data['AttachFilename'], strripos($data['AttachFilename'], ".")+1));
			...
			$try->set_value('nonmandatory', 'postid',				$data['PostID']);
			$try->set_value('nonmandatory', 'filesize',				$data['AttachFileSize']);
			$try->set_value('nonmandatory', 'counter',				$data['AttachDownloadCount']);
			$try->set_value('nonmandatory', 'visible',				'1'); // AttachOption ?
			$try->set_value('nonmandatory', 'dateline',				$data['AttachCreationDate']);
			$try->set_value('nonmandatory', 'userid',				$idcache->get_id('user', $data['MemberID']));
			...
			if($try->is_valid())
			{
				if($try->import_attachment($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['attachment'] . ' -> ' . $data['AttachFilename']);
					}
