...
\*======================================================================*/
/**
* mercury_007 Import Post module
*
...
*
*/
class mercury_007 extends mercury_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '006';
	var $_modulestring 	= 'Import Post';

	function mercury_007()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_mercury_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'threadid',			$thread_ids_array["$post_details[post_topic]"]);
			$try->set_value('mandatory', 'userid',				$user_ids_array["$post_details[post_author]"]);
			$try->set_value('mandatory', 'importthreadid',		$post_details['post_topic']);

			// Non Mandatory
			$try->set_value('nonmandatory', 'parentid', 		'0');
			$try->set_value('nonmandatory', 'username',			$user_name_array["$post_details[post_author]"]);
			$try->set_value('nonmandatory', 'title',			'');
			$try->set_value('nonmandatory', 'dateline',			$post_details['post_time']);
			$try->set_value('nonmandatory', 'pagetext',			$this->html_2_bb($post_details['post_text']));
			$try->set_value('nonmandatory', 'allowsmilie',		$post_details['post_emoticons']);
			$try->set_value('nonmandatory', 'showsignature',	'1');
			$try->set_value('nonmandatory', 'ipaddress',		$post_details['post_ip']);
			#$try->set_value('nonmandatory', 'iconid',			$post_details['iconid']); Would need post_icon mapping
			$try->set_value('nonmandatory', 'visible',			'1');
			$try->set_value('nonmandatory', 'attach',			$post_details['attach']);
			$try->set_value('nonmandatory', 'importpostid',		$post_details['post_id']);			
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: post from -> ' . $user_name_array["$post_details[post_author]"]);
