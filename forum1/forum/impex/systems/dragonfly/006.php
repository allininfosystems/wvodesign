...
class dragonfly_006 extends dragonfly_000
{
	var $_dependent = '005';

	function dragonfly_006($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}bbtopics", 'topic_id', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('nonmandatory', 'sticky',			'0'); // tpoic_status ?
			$try->set_value('nonmandatory', 'views',			$data['tpoic_views']);
			$try->set_value('nonmandatory', 'open',				'1'); // tpoic_status ?
			...
			$try->set_value('nonmandatory', 'dateline',			$data['topic_time']);
