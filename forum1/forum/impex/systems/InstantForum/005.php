...
*
*/
class InstantForum_005 extends InstantForum_000
{
	var $_dependent 	= '004';


	function InstantForum_005(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_InstantForum_categories_details($Db_source, $source_database_type, $source_table_prefix);
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$cat['SortOrder']);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_InstantForum_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum_details)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$forum_details['SortOrder']);
				$try->set_value('mandatory', 'parentid',			$cat_ids["$forum_details[ParentID]"]);

				// Non Mandatory
				$try->set_value('nonmandatory', 'description',		$forum_details['Description']);
				$try->set_value('nonmandatory', 'threadcount',		$forum_details['TotalTopics']);
