...
\*======================================================================*/
/**
* InstantForum_003 Import Usergroup module
*
...
*
*/
class InstantForum_003 extends InstantForum_000
{
	var $_dependent 	= '001';


	function InstantForum_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$usergroup_array 	= $this->get_InstantForum_usergroup_details($Db_source, $source_database_type, $source_table_prefix, $usergroup_start_at, $usergroup_per_page);
		...
		foreach ($usergroup_array as $usergroup_id => $usergroup_details)
		{
			...
			// Non Mandatory
			$try->set_value('nonmandatory', 'title',		$usergroup_details['PermissionName']);
			$try->set_value('nonmandatory', 'description',	$usergroup_details['PermissionName']);
			...
			if($try->is_valid())
			{
				...
				else
				{
					...
					$displayobject->display_now("<br />{$impex_phrases['failed']} :: {$displayobject->phrases['usergroup_not_imported']}");
				}
			}
			else
			{
