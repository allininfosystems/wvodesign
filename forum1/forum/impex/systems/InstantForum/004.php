...
\*======================================================================*/
/**
* InstantForum_004 Import User module
*
...
*
*/
class InstantForum_004 extends InstantForum_000
{
	var $_dependent 	= '003';


	function InstantForum_004(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_InstantForum_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			if ($user_group_ids_array["$user_details[PermissionID]"])
			{
				$try->set_value('mandatory', 'usergroupid',		$user_group_ids_array["$user_details[PermissionID]"]);
			}
			else
			{
			...
			$try->set_value('nonmandatory', 'password',			$user_details['Password']); // Just to set a random string, as its crypt can't bring them in
			$try->set_value('nonmandatory', 'yahoo',			$user_details['Yahoo']);
			$try->set_value('nonmandatory', 'posts',			$user_details['PostCount']);
			$try->set_value('nonmandatory', 'lastactivity',		strtotime($user_details['LastLoginDate']));
			$try->set_value('nonmandatory', 'lastvisit',		strtotime($user_details['LastLoginDate']));
			$try->set_value('nonmandatory', 'joindate',			strtotime($user_details['CreatedDate']));
			$try->set_value('nonmandatory', 'homepage',			$user_details['WebAddress']);

			$try->add_default_value('Location', 				$user_details['Location']);
			$try->add_default_value('Interests', 				$user_details['Interests']);
			$try->add_default_value('signature', 				$this->html_2_bb(addslashes($user_details['Signature'])));
			...
			{
				$try->set_value('nonmandatory', 'avatar', trim($user_details['AvatarURL']));
			}
