...
class vBlogetin_003 extends vBlogetin_000
{
	var $_dependent = '001';

	function vBlogetin_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_vBlogetin_users($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			*/
			...
			// Non mandatory
			$blog_user->set_value('nonmandatory', 'entries',			$data['blogdata']['entries']);
			if ($data['tandd']['title'])
			{
				$blog_user->set_value('nonmandatory', 'title',			$data['tandd']['title']);
			}
			
			if ($data['tandd']['description'])
			{
				$blog_user->set_value('nonmandatory', 'description',	$data['tandd']['description']);
			}
