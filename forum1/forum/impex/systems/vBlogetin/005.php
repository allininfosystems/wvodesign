...
class vBlogetin_005 extends vBlogetin_000
{
	var $_dependent = '004';

	function vBlogetin_005(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}blog_entry", 'entryid', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$t_and_d = $this->get_blog_description($Db_source, $s_db_type, $s_tb_prefix, $data['userid']);
			...
			$blog_text->set_value('mandatory', 'title',				$t_and_d['title']);
			$blog_text->set_value('mandatory', 'pagetext',			$data['body']);
			$blog_text->set_value('mandatory', 'dateline',			$data['dateline']);
			...
			$blog_text->set_value('nonmandatory', 'state',			($data['visible'] == 1 ? 'visible' : 'moderation')); // enum('moderation', 'visible', 'deleted')
			...
			$blog->set_value('mandatory', 'title',					$t_and_d['title']);
			$blog->set_value('mandatory', 'importblogid',			$import_id);
			$blog->set_value('nonmandatory', 'blogcategory',		$idcache->get_id('blogcategory', $data['categoryid']));
			...
			$blog->set_value('nonmandatory', 'state',				($data['draft'] ? 'draft' : ($data['visible'] == 1 ? 'visible' : 'moderation')));
