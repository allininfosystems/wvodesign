...
\*======================================================================*/
/**
* eshare_004 Import Forum module
*
...
*
*/
class eshare_004 extends eshare_000
{
	var $_dependent 	= '003';

	function eshare_004(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$forum_array 	= $this->get_eshare_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
		...
		foreach ($forum_array as $forum_id => $forum_details)
		{
			...
			$try->set_value('mandatory', 'title',					$forum_details['Data']);
			$try->set_value('mandatory', 'options',				$this->_default_forum_permissions);
			$try->set_value('mandatory', 'displayorder',			$forum_details['ObjectID']);
			$try->set_value('mandatory', 'importforumid',		 $forum_id);

			// Non Mandatory
			$try->set_value('nonmandatory', 'description',		$forum_details['Data']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span>' . $displayobject->phrases['forum'] . ' -> ' . $try->get_value('mandatory',''));
