...
*
*/
class vb2_012 extends vb2_000
{
	var $_dependent 	= '001';

	function vb2_012(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($smilie_array as $smilie_id => $smilie_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'smilietext',			$smilie_details['smilietext']);
			$try->set_value('mandatory', 'importsmilieid',		$smilie_id);

			// Non Mandatory
			$try->set_value('nonmandatory', 'title',			$smilie_details['title']);
			$try->set_value('nonmandatory', 'smiliepath',		$smilie_details['smiliepath']);
