...
class InvisionCommunityBlog_002 extends InvisionCommunityBlog_000
{
	var $_dependent 	= '001';

	function InvisionCommunityBlog_002(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		{
			// Get a list of the {$systemname} members in this current selection
			$userarray = $this->get_InvisionCommunityBlog_members_list($Db_source, $source_database_type, $source_table_prefix, $associate_start_at, $associate_per_page);
		...
		if ($do_associate == 1)
		{
			...
			foreach ($users_to_associate as $key => $value)
			{
				...
				else
				{
					...
					$sessionobject->add_error($Db_target, 'warning', $class_num, 0, $displayobject->phrases['associate_error'], $displayobject->phrases['associate_error_rem']);
