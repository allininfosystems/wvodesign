...
\*======================================================================*/
/** 
* drupal6_cms_003 Import Users module
*
...
class drupal_6_cms_003 extends drupal_6_cms_000
{
	var $_dependent 	= '001';

	function drupal_6_cms_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array  =  $this->get_drupal6_cms_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);

		$displayobject->display_now("<h4>{$displayobject->phrases['importing']} " . $user_array['count'] . " {$displayobject->phrases['users']}</h4><p><b>{$displayobject->phrases['from']}</b> : " . $user_start_at . " ::  <b>{$displayobject->phrases['to']}</b> : " . ($user_start_at + count($user_array)) . "</p>");
		...
		foreach ($user_array['data'] as $user_id => $user)
		{
			...
			$try->set_value('mandatory', 'username',		$user['name']);
			$try->set_value('mandatory', 'email',			$user['mail']);
			...
			$try->set_value('nonmandatory', 'password', 	$user['pass']);
			$try->set_value('nonmandatory', 'lastactivity',	$user['access']);
			$try->set_value('nonmandatory', 'joindate',		$user['created']);
