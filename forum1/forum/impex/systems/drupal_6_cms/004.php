...
class drupal_6_cms_004 extends drupal_6_cms_000
{
    ...
    {
        $this->_modulestring = 'Import Article'; #$displayobject->phrases['import_post'];
    }
    ...
    function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
    {
        if ($this->check_order($sessionobject,$this->_dependent))
        {
            ...
            // Ask some questions
            $displayobject->update_html($displayobject->make_input_code('Enter the number of articles per page','perpage',2000));
    ...
    function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
    {
        ...
        foreach ($article_array['data'] as $article_id => $article)
        {
            ...
            $try->set_value('mandatory', 'pagetext',        $article['body']);
            $try->set_value('mandatory', 'title',            $article['title']);
            $try->set_value('mandatory', 'postauthor',         $idcache->get_id('user', $article['uid']));
            $try->set_value('mandatory', 'previewtext',         substr($article['body'], 0, 2040));
