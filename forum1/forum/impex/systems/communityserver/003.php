...
\*======================================================================*/
/**
* communityserver_003 Import User module
*
...
*
*/
class communityserver_003 extends communityserver_000
{
	var $_dependent 	= '001';

	function communityserver_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_communityserver_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			if ($user_details['Email'] == 'noemail') { $user_details['Email'] = 'example@example.com'; }  
			...
			#$try->set_value('nonmandatory', 'password',				$user_details['Password']); // plain text
			...
			$try->set_value('nonmandatory', 'lastactivity',			strtotime($user_details['LastActivity']));
			$try->set_value('nonmandatory', 'lastvisit',			strtotime($user_details['LastActivity']));
			$try->set_value('nonmandatory', 'joindate',				strtotime($user_details['DateCreated']));
			...
			{
				$try->add_default_value('signature', 				$this->html_2_bb($user_details['Signature']));
			}			
