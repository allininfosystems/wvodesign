...
\*======================================================================*/
/**
* deluxeportal_012 Import Smilie module
*
...
*
*/
class deluxeportal_012 extends deluxeportal_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '001';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$smilie_array 	= $this->get_deluxeportal_smilie_details($Db_source, $source_database_type, $source_table_prefix, $smilie_start_at, $smilie_per_page);
		...
		foreach ($smilie_array as $smilie_id => $smilie_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'smilietext',			$smilie_details['tag']);
			$try->set_value('mandatory', 'importsmilieid',		$smilie_details['smilieid']);
			...
			$try->set_value('nonmandatory', 'imagecategoryid',	$smilie_details['imagecategoryid']);
			$try->set_value('nonmandatory', 'displayorder',		$smilie_details['displayorder']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: smilie -> ' . $smilie_details['name']);
