...
\*======================================================================*/
/**
* deluxeportal_009 Import Pmtext module
*
...
*
*/
class deluxeportal_009 extends deluxeportal_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	var $_modulestring 	= 'Private Messages';


	function deluxeportal_009()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$pm_array 	= $this->get_deluxeportal_pmtext_details($Db_source, $source_database_type, $source_table_prefix, $pmtext_start_at, $pmtext_per_page);
		...
		$displayobject->display_now('<h4>Importing ' . count($pm_array) . ' pmtexts</h4><p><b>From</b> : ' . $pmtext_start_at . ' ::  <b>To</b> : ' . ($pmtext_start_at + count($pm_array)) . '</p>');
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			$vB_pm_text->set_value('mandatory', 'message',			$this->html_2_bb($pm_text['message']));
			...
			$vB_pm_text->set_value('nonmandatory', 'dateline',		$pm['sentdate']);
			$vB_pm_text->set_value('nonmandatory', 'showsignature',	$pm['showsignature']);
			$vB_pm_text->set_value('nonmandatory', 'allowsmilie',	$pm['smilies']);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					{
						// Hmmm, what to do, might as well stick them in the inbox as read.
						$pm['isread'] = 1;
						$folder_id = 1;
					}
					...
					$vB_pm->set_value('mandatory', 'userid',			$user_ids_array["$pm[fromuserid]"]);
					$vB_pm->set_value('nonmandatory', 'folderid',		$folder_id);
					$vB_pm->set_value('nonmandatory', 'messageread',	$pm['isread']);
		...
		// Check for page end
		if (count($pm_array) == 0 OR count($pm_array) < $pmtext_per_page)
		{
