...
\*======================================================================*/
/**
* encoreII_004 Import Forum module
*
...
*
*/
class encoreII_004 extends encoreII_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '003';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_encoreII_cat_details($Db_source, $source_database_type, $source_table_prefix);
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'importcategoryid',	$cat_id+1);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_encoreII_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'parentid',			$cat_ids[intval($forum['CategoryID'])+1]);
				$try->set_value('mandatory', 'importforumid',		$forum['ID']+1);
				...
				*/
				...
				if($try->is_valid())
				{
					...
					else
					{
						$sessionobject->add_error('warning',
												 $this->_modulestring,
												 get_class($this) . "::import_category failed for " . $forum['Title'] . " get_encoreII_cat_details was ok.",
												 'Check database permissions and user table');
						$sessionobject->set_session_var($class_num . '_objects_failed',$sessionobject->get_session_var($class_num . '_objects_failed') + 1 );
						$displayobject->display_now("<br />Got category " . $forum['Title'] . " and <b>DID NOT</b> imported to the " . $target_database_type . " database");
					}
				}
				else
				{
