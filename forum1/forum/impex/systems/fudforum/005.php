...
\*======================================================================*/
/**
* fudforum_005 Import Forum module
*
...
*
*/
class fudforum_005 extends fudforum_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	var $_modulestring 	= 'Import Forum';

	function fudforum_005()
	{
		// Constructor
	}
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			// Ask some questions
			$displayobject->update_html($displayobject->make_input_code('Forums to import per cycle (must be greater than 1)','forumperpage',200));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_details($Db_source, $source_database_type, $source_table_prefix, 0, -1, 'cat', 'id');
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$cat['view_order']);
		...
		{
			// Sort out the forums
			$forum_array = $this->get_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page, 'forum', 'id');
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$forum['view_order']);

				if ($cat_ids["$forum[cat_id]"])
				{
				...
				$try->set_value('mandatory', 'replycount',			$forum['post_count']);
				$try->set_value('mandatory', 'threadcount',			$forum['thread_count']);
				...
				if($try->is_valid())
				{
					...
					else
					{
						$sessionobject->add_error('warning',
												 ...
												 'Check database permissions and user table');
						$sessionobject->set_session_var($class_num . '_objects_failed',$sessionobject->get_session_var($class_num . '_objects_failed') + 1 );
						$displayobject->display_now("<br />Got forum " . $forum['name'] . " and <b>DID NOT</b> imported to the " . $target_database_type . " database");
					}
				}
				else
				{
