...
class gossamer_threads_009 extends gossamer_threads_000
{
	var $_dependent = '007';

	function gossamer_threads_009(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$dir 				= $sessionobject->get_session_var('attachmentsfolder') . '/post';
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}PostAttachment", 'postatt_id', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$folder =  $dir . '/' . substr($data['postatt_id'], -1) . '/' . $data['postatt_id'];

			if(!is_file($folder))
			{
				$displayobject->display_now("<br /><b>{$displayobject->phrases['source_file_not']} </b> :: {$data['postatt_filename']}");
				$sessionobject->add_error($attachment_id, $displayobject->phrases['attachment_not_imported'], $data['postatt_filename'] . ' - ' . $displayobject->phrases['attachment_not_imported_rem_1']);
				$sessionobject->set_session_var($class_num . '_objects_failed',$sessionobject->get_session_var($class_num. '_objects_failed') + 1 );
				continue;
			}

			$file = $this->vb_file_get_contents($folder);

			// Mandatory
			$try->set_value('mandatory', 'importattachmentid',	$import_id);
			$try->set_value('mandatory', 'filename',			addslashes($data['postatt_filename']));
			$try->set_value('mandatory', 'filedata',			$file);

			// Non mandatory
			$try->set_value('nonmandatory', 'filehash',			md5($file));
			$try->set_value('nonmandatory', 'postid',			$data['post_id_fk']);
			$try->set_value('nonmandatory', 'filesize',			filesize($folder));
			$try->set_value('nonmandatory', 'visible',			"1");
			$try->set_value('nonmandatory', 'extension',		substr($data['postatt_filename'], strrpos($data['postatt_filename'], '.')+1));

			// Check if object is valid
			if($try->is_valid())
			{
				if($try->import_attachment($Db_target, $t_db_type, $t_tb_prefix, TRUE))
				{
					if(shortoutput)
					{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">$import_id :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['attachment'] . ' -> ' . $data['postatt_filename']);
					}
