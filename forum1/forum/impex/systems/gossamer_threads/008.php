...
class gossamer_threads_008 extends gossamer_threads_000
{
	var $_dependent = '004';

	function gossamer_threads_008(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}Message", 'msg_id', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $pm)
		{
			...
			$userid 	= $idcache->get_id('user', $pm['to_user_id_fk']);
			$username	= $idcache->get_id('username', $pm['to_user_id_fk']);
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$idcache->get_id('user', $pm['from_user_id_fk']));
			$vB_pm_text->set_value('mandatory', 'title',			$pm['msg_subject']);
			$vB_pm_text->set_value('mandatory', 'message',			$this->gossamer_threads_html($pm['msg_body']));
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$idcache->get_id('username', $pm['from_user_id_fk']));
			$vB_pm_text->set_value('nonmandatory', 'iconid',		'');
			$vB_pm_text->set_value('nonmandatory', 'dateline',		$pm['msg_time']);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',    			$idcache->get_id('user', $pm['to_user_id_fk']));
					...
					$vB_pm_from->set_value('mandatory', 'userid',   		$idcache->get_id('user', $pm['from_user_id_fk']));
