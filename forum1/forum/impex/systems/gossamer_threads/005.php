...
class gossamer_threads_005 extends gossamer_threads_000
{
	var $_dependent = '004';

	function gossamer_threads_005(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}Category", 'cat_id', 0, 0, 500);

			$displayobject->display_now("<h4>{$displayobject->phrases['importing']} " . count($data_array['data']) . " {$displayobject->phrases['categories']}</h4>");
			...
			foreach ($data_array['data'] as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'title', 				$cat['cat_name']);
				$try->set_value('mandatory', 'displayorder',		$cat['cat_sort_rank']+1);
		...
		{
			// Sort out the forums
			$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}Forum", 'forum_id', 0, $start_at, $per_page);
			...
			foreach ($data_array['data'] as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'parentid',			$cat_ids["$forum[cat_id_fk]"]);
				$try->set_value('mandatory', 'title', 				$forum['forum_name']);
				$try->set_value('mandatory', 'displayorder',		$forum['forum_sort_rank']);
