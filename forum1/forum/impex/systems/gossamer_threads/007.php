...
class gossamer_threads_007 extends gossamer_threads_000
{
	var $_dependent = '006';

	function gossamer_threads_007(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}Post", 'post_id', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'userid',				$idcache->get_id('user', $data['user_id_fk']));

			if ($data['post_root_id'] == 0)
			{
				$try->set_value('mandatory', 'importthreadid',	$import_id);
				$try->set_value('mandatory', 'threadid',		$idcache->get_id('thread', $import_id));
			}
			else
			{
				$try->set_value('mandatory', 'importthreadid',	$data['post_root_id']);
				$try->set_value('mandatory', 'threadid',		$idcache->get_id('thread', $data['post_root_id']));
			}

			// Non mandatory
			$try->set_value('nonmandatory', 'importpostid',		$import_id);
			$try->set_value('nonmandatory', 'visible',			($data['post_deleted'] == 0 ? 1 : 0));
			$try->set_value('nonmandatory', 'ipaddress',		$data['post_ip']);
			$try->set_value('nonmandatory', 'showsignature',	"1");
			$try->set_value('nonmandatory', 'allowsmilie',		"1");
			$try->set_value('nonmandatory', 'pagetext',			$this->gossamer_threads_html($data['post_message']));
			$try->set_value('nonmandatory', 'dateline',			$data['post_time']);
			$try->set_value('nonmandatory', 'title',			htmlspecialchars($data['post_subject']));
			...
			if($try->is_valid())
			{
				if($try->import_post($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['post'] . ' -> ' .  $try->get_value('nonmandatory','title'));
					}
