...
class openBB_006 extends openBB_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_openBB_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			$try->set_value('nonmandatory', 'postuserid',		$thread_details['posterid']);
			$try->set_value('nonmandatory', 'lastposter',		$thread_details['lpuser']);
			...
			if($thread_details['mode'] == 'Sticky')				{$try->set_value('nonmandatory', 'sticky',		'1'); }
