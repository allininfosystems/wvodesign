...
\*======================================================================*/
/**
* aspnet_004 Import Forum module
*
...
*
*/
class aspnet_004 extends aspnet_000
{
	var $_dependent 	= '003';

	function aspnet_004(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_aspnet_categories_details($Db_source, $source_database_type, $source_table_prefix);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_aspnet_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'parentid',			$cat_ids["$forum[F_Cat]"]);
				$try->set_value('mandatory', 'title', 				$forum['F_Name']);
				$try->set_value('mandatory', 'displayorder',		$forum['Forum_ID']);
