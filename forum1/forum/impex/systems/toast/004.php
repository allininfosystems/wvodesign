...
class toast_004 extends toast_000
{
	var $_dependent = '003';

	function toast_004(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}Forum", 'ID', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('mandatory', 'importforumid',		$data['ID']);
			$try->set_value('mandatory', 'importcategoryid',	$cat_ids_array[1]);
			$try->set_value('mandatory', 'displayorder',		"1");
			$try->set_value('mandatory', 'options',				$this->_default_forum_permissions);
			$try->set_value('mandatory', 'title',				$data['ForumName']);

			// Non mandatory
			$try->set_value('nonmandatory', 'description',		$data['Description']);
			$try->set_value('nonmandatory', 'threadcount',		$data['TopicCount']);
			$try->set_value('nonmandatory', 'replycount',		$data['PostCount']);
			...
			if($try->is_valid())
			{
				if($try->import_forum($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $data['ID'] . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['forum'] . ' -> ' . $data['ForumName']);
					}
