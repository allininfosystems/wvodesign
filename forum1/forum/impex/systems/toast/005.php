...
class toast_005 extends toast_000
{
	var $_dependent = '004';

	function toast_005(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Non mandatory
			$try->set_value('nonmandatory', 'visible',			($data['Deleted'] == 0 ? 1 : 0));
			$try->set_value('nonmandatory', 'sticky',			"0");
			$try->set_value('nonmandatory', 'views',			$data['Hits']);
			$try->set_value('nonmandatory', 'open',				$data['Active']);
			$try->set_value('nonmandatory', 'replycount',		$data['ReplyCount']);

			$try->set_value('nonmandatory', 'postusername',		$idcache->get_id('username', $data['MemberID']));
			$try->set_value('nonmandatory', 'postuserid',		$idcache->get_id('user', $data['MemberID']));
			...
			if($try->is_valid())
			{
				if($try->import_thread($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $data['ID'] . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['thread'] . ' -> ' . $data['Subject']);
					}
