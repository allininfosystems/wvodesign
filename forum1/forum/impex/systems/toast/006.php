...
class toast_006 extends toast_000
{
	var $_dependent = '005';

	function toast_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$Data_object 	= new ImpExData($Db_target, $sessionobject, 'post');
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}Post", 'ID', 0, $start_at, $per_page);
		...
		{
			$try = (phpversion() < '5' ? $Data_object : clone($Data_object));
			...
			$try->set_value('mandatory', 'threadid',			$idcache->get_id('thread', $threadid));
			$try->set_value('mandatory', 'importthreadid',		$threadid);
			$try->set_value('mandatory', 'userid',				$idcache->get_id('user', $data['MemberID']));

			// Non mandatory
			$try->set_value('nonmandatory', 'visible',			$data['Active']);
			$try->set_value('nonmandatory', 'showsignature',	$data['1']);
			$try->set_value('nonmandatory', 'allowsmilie',		$data['1']);
			$try->set_value('nonmandatory', 'pagetext',			$this->html_2_bb($data['Message']));
			$try->set_value('nonmandatory', 'dateline',			strtotime($data['PostDate']));
			$try->set_value('nonmandatory', 'title',			$data['Subject']);
			$try->set_value('nonmandatory', 'username',			$idcache->get_id('username', $data['MemberID']));
			...
			if($try->is_valid())
			{
				if($try->import_post($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $data['ID'] . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['post'] . ' -> ' . $data['Subject']);
					}
...
# Autogenerated on : May 26, 2007, 8:53 am
