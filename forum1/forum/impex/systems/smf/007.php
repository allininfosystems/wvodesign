...
*
*/
class smf_007 extends smf_000
{
	var $_dependent 	= '006';

	function smf_007(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br />' . $post_id . ' :: <span class="isucc"><b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['post'] . ' -> ' . $post_details['subject']);
