...
class fusionbb_003 extends fusionbb_000
{
	var $_dependent = '001';

	function fusionbb_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'importusergroupid',	$data['group_id']);

			$try->set_value('nonmandatory', 'title',			$data['group_name']);
			$try->set_value('nonmandatory', 'description',		$data['group_description']);
			...
			if($try->is_valid())
			{
				if($try->import_usergroup($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $data['group_id'] .' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['usergroup'] . ' -> ' . $data['group_name']);
					}
