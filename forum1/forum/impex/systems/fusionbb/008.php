...
class fusionbb_008 extends fusionbb_000
{
	var $_dependent = '007';

	function fusionbb_008(&$displayobject)
	{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			// Ask some questions
			$displayobject->update_html($displayobject->make_input_code($displayobject->phrases['units_per_page'], 'perpage', 50));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Strip off preceding timestamp for readable name
			$filename = substr($data['file_name'], 11);

			if(!is_file($dir . '/' . $data['file_name']))
			{
				$displayobject->display_now("<br /><b>{$displayobject->phrases['source_file_not']} </b> :: {$filename}");
				$sessionobject->add_error($Db_target, 'warning', $class_num, $attachment_id, $displayobject->phrases['attachment_not_imported'], $data['file_name'] . ' - ' . $displayobject->phrases['attachment_not_imported_rem_1']);
			...
			$try->set_value('mandatory', 'filename',				$filename);
			...
			$try->set_value('nonmandatory', 'postid',				$data['post_id']); // importid that is looked up in post table
			$try->set_value('nonmandatory', 'filesize',				$data['file_size']);
			$try->set_value('nonmandatory', 'counter',				$data['file_downloads']);
			$try->set_value('nonmandatory', 'visible',				1);
			$try->set_value('nonmandatory', 'dateline',				$data['file_created']);

			#$try->set_value('nonmandatory', 'userid',		$data['userid']); Done by the import function now
			...
			if($try->is_valid())
			{
				if($try->import_attachment($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">$import_id :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['attachment'] . ' -> ' . $data['']);
					}
