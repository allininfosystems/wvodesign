...
class fusionbb_007 extends fusionbb_000
{
	var $_dependent = '006';

	function fusionbb_007(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Non mandatory
			$try->set_value('nonmandatory', 'importpostid',		$data['post_id']);
			$try->set_value('nonmandatory', 'ipaddress',		$data['post_ip']);
			$try->set_value('nonmandatory', 'visible',			'1');
			$try->set_value('nonmandatory', 'pagetext',			$this->html_2_bb($data['post_raw_body']));
			$try->set_value('nonmandatory', 'dateline',			$data['post_posted']);
			$try->set_value('nonmandatory', 'username',			$idcache->get_id('username', $data['user_id']));
			$try->set_value('nonmandatory', 'parentid',			$idcache->get_id('post', $data['post_parent']));
			...
			#$try->set_value('nonmandatory', 'showsignature',	$data['showsignature']);
			...
			if($try->is_valid())
			{
				if($try->import_post($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['post'] . ' -> ' . $idcache->get_id('username', $data['user_id']));
					}
