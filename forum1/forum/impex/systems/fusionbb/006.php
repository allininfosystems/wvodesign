...
class fusionbb_006 extends fusionbb_000
{
	var $_dependent = '005';

	function fusionbb_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'title',				$data['topic_subject']);
			...
			$try->set_value('nonmandatory', 'sticky',			$data['topic_is_sticky']);
			$try->set_value('nonmandatory', 'views',			$data['topic_views']);
			$try->set_value('nonmandatory', 'open',				($data['topic_is_closed'] == 0 ? 1 : 0));
			$try->set_value('nonmandatory', 'replycount',		$data['topic_replies']);
			$try->set_value('nonmandatory', 'postusername',		$data['topic_username']);
			$try->set_value('nonmandatory', 'postuserid',		$idcache->get_id('user', $data['user_id']));
			$try->set_value('nonmandatory', 'lastposter',		$data['topic_last_poster']);
			$try->set_value('nonmandatory', 'dateline',			$data['topic_first_post_time']);
			...
			if($try->is_valid())
			{
				if($try->import_thread($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['thread'] . ' -> ' . $data['topic_subject']);
					}
