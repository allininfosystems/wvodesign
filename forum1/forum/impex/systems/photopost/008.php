...
\*======================================================================*/
/**
* photopost_008 Import Attachment module
*
...
*
*/
class photopost_008 extends photopost_000
{
	var $_dependent 	= '007';

	function photopost_008(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$attachment_array 		= $this->get_photopost_attachment_details($Db_source, $source_database_type, $source_table_prefix, $attachment_start_at, $attachment_per_page);
		...
		foreach ($attachment_array as $attachment_id => $attachment_details)
		{
			...
			$full_path = $dir . '/' . $attachment_details['cat'] . '/' . $attachment_details['userid'] .   $attachment_details['bigimage'];
	
			if(!is_file($full_path))
			{
				$displayobject->display_now("<br /><b>{$displayobject->phrases['source_file_not']} </b> :: {$attachment_details['bigimage']}");
				$sessionobject->add_error($attachment_id, $displayobject->phrases['attachment_not_imported'], $attachment_details['bigimage'] . ' - ' . $displayobject->phrases['attachment_not_imported_rem_1']);
			...
			$try->set_value('mandatory', 'filename',				addslashes($attachment_details['bigimage']));
			$try->set_value('mandatory', 'filedata',				$file);

			// Non Mandatory
			$try->set_value('nonmandatory', 'filehash',				md5($file));
			$try->set_value('nonmandatory', 'postid',				$attachment_id); # Because the photo is the thread and first post at the same time
			$try->set_value('nonmandatory', 'filesize',				$attachment_details['filesize']);
			$try->set_value('nonmandatory', 'counter',				$attachment_details['views']);
			$try->set_value('nonmandatory', 'visible',				'1');
			$try->set_value('nonmandatory', 'dateline',				$attachment_details['date']);
			$try->set_value('nonmandatory', 'userid',				$user_ids_array["$attachment_details[id]"]);
