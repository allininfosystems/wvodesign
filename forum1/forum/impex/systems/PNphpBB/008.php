...
\*======================================================================*/
/**
* PNphpBB_008 Import Post module
*
...
*
*/
class PNphpBB_008 extends PNphpBB_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '006';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 			= $this->get_PNphpBB_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		$truncated_smilies		= $this->get_PNphpBB_truncated_smilies($Db_source, $source_database_type, $source_table_prefix);

		$last_pass = $sessionobject->get_session_var('last_pass');
		$displayobject->display_now("<h4>Importing " . count($post_array) . " posts</h4><p><b>From</b> : " . $post_start_at . " ::  <b>To</b> : " . ($post_start_at + count($post_array)) ."</p><p><b>Last pass took</b> : " . $last_pass . " seconds</p>");
		...
		foreach ($post_array as $post_id => $post)
		{
			...
			unset($page_text);
			$page_text = $this->get_PNphpBB_post_text($Db_source, $source_database_type, $source_table_prefix, $post['post_id']);
			$try->set_value('nonmandatory', 'title', 			$page_text['post_subject']);
			$try->set_value('nonmandatory', 'pagetext', 		$this->html_2_bb($this->PNphpBB_html($page_text['post_text'],$truncated_smilies)));
			...
			*/
			...
			if($try->is_valid())
			{
				...
				else
				{
					...
					$displayobject->display_now("<br />Got post " . $cat['cat_title'] . " and <b>DID NOT</b> imported to the " . $target_database_type . " database");
				}
			}
			else
			{
