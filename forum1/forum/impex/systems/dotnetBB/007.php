...
\*======================================================================*/
/**
* dotnetBB_007 Import Post module
*
...
*
*/
class dotnetBB_007 extends dotnetBB_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '006';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_dotnetBB_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			// Mandatory
			if ($post_details['threadid'])
			{
				$try->set_value('mandatory', 'threadid',			$thread_ids_array["$post_details[threadid]"]);
			}
			else
			{
				// Its the thread starting post $post_id = $threadid
				$try->set_value('mandatory', 'threadid',			$thread_ids_array[$post_id]);				
			}
			
			$try->set_value('mandatory', 'userid',				$user_ids_array["$post_details[userid]"]);
			$try->set_value('mandatory', 'importthreadid',		$try->get_value('mandatory', 'threadid'));
			...
			$try->set_value('nonmandatory', 'pagetext',			$this->dotnetBB_html($this->html_2_bb($post_details['body'])));
