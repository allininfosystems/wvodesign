...
*
*/
class ttcms_005 extends ttcms_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_ttcms_cat_details($Db_source, $source_database_type, $source_table_prefix);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_ttcms_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'displayorder',		($forum['boardOrder'] = 0 ? 1 : $forum['boardOrder']));
				$try->set_value('mandatory', 'parentid',			$cat_ids["$forum[ID_CAT]"]);
				...
				/*
				$try->set_value('nonmandatory', 'visible', 			$forum['']);
				$try->set_value('nonmandatory', 'styleid',			$forum['']);
				$try->set_value('nonmandatory', 'threadcount',		$forum['']);
				$try->set_value('nonmandatory', 'daysprune',		$forum['']);
				$try->set_value('nonmandatory', 'password',			$forum['']);
				$try->set_value('nonmandatory', 'replycount',		$forum['']);
				...
				*/
				...
				if($try->is_valid())
				{
					...
					else
					{
						$sessionobject->add_error('warning',
												 $this->_modulestring,
												 get_class($this) . "::import_forum failed for " . $cat['name'] . " get_ttcms_categories_details was ok.",
												 'Check database permissions and user table');
						$sessionobject->set_session_var($class_num . '_objects_failed',$sessionobject->get_session_var($class_num . '_objects_failed') + 1 );
						$displayobject->display_now("<br />Got forum " . $cat['name'] . " and <b>DID NOT</b> imported to the " . $target_database_type . " database");
					}
				}
				else
				{
