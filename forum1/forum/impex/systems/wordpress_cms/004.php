...
class wordpress_cms_004 extends wordpress_cms_000
{
    ...
    function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
    {
        ...
        foreach ($article_array['data'] as $article_id => $article)
        {
            ...
            $try->set_value('mandatory', 'pagetext',      $article['post_content']);
            $try->set_value('mandatory', 'title',         $article['post_title']);
            $try->set_value('mandatory', 'postauthor',    $idcache->get_id('user', $article['post_author']));
            $try->set_value('mandatory', 'previewtext',   substr($article['post_content'], 0, 2040));

            $try->set_value('nonmandatory', 'created',    strtotime($article['post_date']));
