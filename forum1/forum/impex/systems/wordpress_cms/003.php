...
class wordpress_cms_003 extends wordpress_cms_000
{
    var $_dependent     = '001';

    function wordpress_cms_003(&$displayobject)
    {
    ...
    function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
    {
        ...
        $user_array  =  $this->get_wordpress_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
        ...
        foreach ($user_array['data'] as $user_id => $user)
        {
            ...
            $try->set_value('mandatory', 'username',     $user['user_login']);
            ...
            $try->set_value('nonmandatory', 'lastactivity', strtotime($user['user_registered']));
            $try->set_value('nonmandatory', 'joindate',     strtotime($user['user_registered']));
