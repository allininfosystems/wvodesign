...
class cfbb_005 extends cfbb_000
{
	...
	function cfbb_005(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_cfbb_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			$try->set_value('nonmandatory', 'open',				($thread_details['locked'] == null ? 1 : 0));
			$try->set_value('nonmandatory', 'replycount',		$thread_details['replies']);
			$try->set_value('nonmandatory', 'dateline',			strtotime($thread_details['createdOn']));
