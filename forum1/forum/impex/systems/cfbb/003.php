...
*
*/
class cfbb_003 extends cfbb_000
{
	var $_dependent 	= '001';

	function cfbb_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array = $this->get_cfbb_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			$try->set_value('nonmandatory', 'msn',			$user_details['msn_number']);
			$try->set_value('nonmandatory', 'icq',			$user_details['icq_number']);
			$try->set_value('nonmandatory', 'yahoo',		$user_details['yahoo_number']);
			$try->set_value('nonmandatory', 'aim',			$user_details['aim_number']);
			
			#$try->set_value('nonmandatory', 'avatar',		$user_details['avatar']); It's there
			$try->_password_md5_already = false;
			$try->set_value('nonmandatory', 'password',		$user_details['password']); // Is it plain text ?
			$try->set_value('nonmandatory', 'posts',		$user_details['num_posts']);
			
			$try->set_value('nonmandatory', 'lastactivity',	strtotime($user_details['last_visit']));
			$try->set_value('nonmandatory', 'lastvisit',	strtotime($user_details['last_visit']));
			
			$try->set_value('nonmandatory', 'joindate',		strtotime($user_details['createdOn']));
			$try->set_value('nonmandatory', 'customtitle',	$user_details['custom_title']);
