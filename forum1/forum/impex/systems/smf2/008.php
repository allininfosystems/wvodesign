...
class smf2_008 extends smf2_000
{
	var $_dependent = '004';

	function smf2_008($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}personal_messages", 'id_pm', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$to_userid = $this->get_smf2_pm_toid($Db_source, $s_db_type, $s_tb_prefix, $import_id);
			...
			$vB_pm_text->set_value('mandatory', 'fromuserid',		$idcache->get_id('user', $data['id_member_from']));
			$vB_pm_text->set_value('mandatory', 'title',			$data['subject']);
			$vB_pm_text->set_value('mandatory', 'message',			$this->html_2_bb($data['body']));
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$idcache->get_id('username', $data['id_member_from']));
			$vB_pm_text->set_value('nonmandatory', 'iconid',		'');
			$vB_pm_text->set_value('nonmandatory', 'dateline',		$data['msgtime']);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',    			$idcache->get_id('user', $to_userid));
					...
					$vB_pm_from->set_value('mandatory', 'userid',   		$idcache->get_id('user', $data['id_member_from']));
