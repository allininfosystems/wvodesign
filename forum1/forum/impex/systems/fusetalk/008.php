...
*
*/
class fusetalk_008 extends fusetalk_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '003';
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			// Ask some questions
			$displayobject->update_html($displayobject->make_input_code('Pms to import per cycle (must be greater than 1)','pmperpage',250));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$pm_array 	= $this->get_fusetalk_pm_details($Db_source, $source_database_type, $source_table_prefix, $pm_start_at, $pm_per_page);
	...
	#	$user_name_array = $this->get_username($Db_target, $target_database_type, $target_table_prefix);
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			$userid 	= $idcache->get_id('user', $pm['reciverid']);
			$username	= $idcache->get_id('username', $pm['reciverid']);
			unset($touserarray);
			...
			{
				$vB_pm_text->set_value('mandatory', 'title',		'Imported PM');
			}
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$idcache->get_id('user', $pm['reciverid']));
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$idcache->get_id('user', $pm['sender']));
					...
					if($vB_pm_text->is_valid())
					{
						...
						{
							$displayobject->display_now('<br /><span class="isucc"><b>' . $vB_pm_text->how_complete() . '%,</b></span> Imported from : <b>' . $user_name_array["$pm[senderid]"] . '</b> , to : <b>' . $user_name_array["$pm[reciverid]"] . '</b>');
		...
		if (count($pm_array) == 0 OR count($pm_array) < $pm_per_page)
		{
			...
			$displayobject->display_now('User PM count ...');
