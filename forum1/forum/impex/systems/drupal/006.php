...
*
*/
class drupal_006 extends drupal_000
{
	var $_dependent 	= '005';

	function drupal_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_drupal_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			$try->set_value('mandatory', 'importthreadid',		$post_details['nid']);
			$try->set_value('mandatory', 'threadid',			$thread_ids_array["$post_details[nid]"]);

			// Non Mandatory
			$try->set_value('nonmandatory', 'dateline',			$post_details['timestamp']);
			$try->set_value('nonmandatory', 'pagetext',			htmlspecialchars($this->html_2_bb($this->drupal_html($post_details['comment']))));
			$try->set_value('nonmandatory', 'allowsmilie',		'1');
			$try->set_value('nonmandatory', 'showsignature',	'1');
			$try->set_value('nonmandatory', 'ipaddress',		$post_details['hostname']);
