...
class xmb_006 extends xmb_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_xmb_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			if($user_name_2_id_array["$post_details[author]"])
			{
				$try->set_value('mandatory', 'userid',				$user_name_2_id_array["$post_details[author]"]);
			}
			else
			{
			...
			// Non Mandatory
			$try->set_value('nonmandatory', 'parentid',			'0'); #$this->get_vb_post_id($Db_target, $target_database_type, $target_table_prefix, $post_details['pid']));
			...
			$try->set_value('nonmandatory', 'pagetext',			$this->xmb_html($this->html_2_bb(stripslashes($post_details['message']))));

			if($post_details['smileyoff'] == 'yes')
			{
			...
			$try->set_value('nonmandatory', 'showsignature',	$post_details['usesig']);
