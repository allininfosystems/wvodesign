...
class yabbse_005 extends yabbse_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_yabbse_cat_details($Db_source, $source_database_type, $source_table_prefix);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_yabbse_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'importforumid',		$forum['ID_BOARD']);
...
# Autogenerated on : May 17, 2005, 3:28 pm
