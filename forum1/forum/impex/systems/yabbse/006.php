...
\*======================================================================*/
/**
* yabbse_006 Import Thread module
*
...
*
*/
class yabbse_006 extends yabbse_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_yabbse_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			$thread_info = $this->get_thread_info($Db_source, $source_database_type, $source_table_prefix, $thread_details['ID_FIRST_MSG']);

			// Mandatory
			$try->set_value('mandatory', 'title',				$thread_info['subject']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: thread -> ' . $thread_info['subject']);
