...
class fireboard_006 extends fireboard_000
{
	var $_dependent = '005';

	function fireboard_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_fireboard_threads($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('mandatory', 'importforumid',	$data['catid']);
			$try->set_value('mandatory', 'importthreadid',	$import_id);
			$try->set_value('mandatory', 'forumid',			$forum_ids_array["$data[catid]"]);

			// Non mandatory
			$try->set_value('nonmandatory', 'sticky',		$data['locked']);
			...
			$try->set_value('nonmandatory', 'postusername',	$idcache->get_id('username', $data['userid']));
			$try->set_value('nonmandatory', 'postuserid',	$idcache->get_id('user', $data['userid']));
