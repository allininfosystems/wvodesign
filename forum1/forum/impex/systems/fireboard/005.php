...
class fireboard_005 extends fireboard_000
{
	var $_dependent = '004';

	function fireboard_005($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_fireboard_cat_details($Db_source, $s_db_type, $s_tb_prefix);
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$cat['ordering']);
				...
				$try->set_value('nonmandatory', 'description', 		$cat['name']);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_fireboard_forum_details($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
			...
			foreach ($forum_array as $import_id => $data)
			{
				...
				// Mandatory
				if ($cat_ids["$data[parent]"])
				{
					$try->set_value('mandatory', 'parentid',			$cat_ids["$data[parent]"]);
				}
				else 
				{
					$forum_ids_array = $this->get_forum_ids($Db_target, $t_db_type, $t_tb_prefix);
					$try->set_value('mandatory', 'parentid',			$forum_ids_array["$data[parent]"]);
				}
				$try->set_value('mandatory', 'importforumid',		$import_id);
				$try->set_value('mandatory', 'importcategoryid',	$data['parent']);
				$try->set_value('mandatory', 'displayorder',		$data['ordering']);
