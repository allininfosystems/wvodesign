...
class wbb3_007 extends wbb3_000
{
	var $_dependent = '006';

	function wbb3_007($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$forum_prefix}post", 'postID', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Non mandatory
			$try->set_value('nonmandatory', 'visible',		($data['isDeleted'] == 0 ? 1 : 0));
			...
			$try->set_value('nonmandatory', 'allowsmilie',	$data['enableSmilies']);
			$try->set_value('nonmandatory', 'title',		$data['subject']);
			$try->set_value('nonmandatory', 'username',		$data['username']);
			$try->set_value('nonmandatory', 'parentid',		$idcache->get_id('post', $data['parentPostID']));
