...
\*======================================================================*/
/**
* thwboard_005 Import Forum module
*
...
*
*/
class thwboard_005 extends thwboard_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '003';
	var $_modulestring 	= 'Import Forum';

	function thwboard_005()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_thwboard_cat_details($Db_source, $source_database_type, $source_table_prefix);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_thwboard_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'title', 				$forum['boardname']);
				...
				$try->set_value('nonmandatory', 'description', 		$forum['boarddescription']);
				$try->set_value('nonmandatory', 'visible', 			'1');
				$try->set_value('nonmandatory', 'replycount',		$forum['threadreplies']);
				$try->set_value('nonmandatory', 'lastpost',			$forum['boardlastpost']);
