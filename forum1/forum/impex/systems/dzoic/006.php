...
class dzoic_006 extends dzoic_000
{
	var $_dependent = '005';

	function dzoic_006($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}posts", 'pst_id', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'threadid',		$idcache->get_id('thread', $data['top_id']));
			$try->set_value('mandatory', 'importthreadid',	$data['top_id']);
			$try->set_value('mandatory', 'userid',			$idcache->get_id('user', $data['mem_id']));
			...
			$try->set_value('nonmandatory', 'dateline',		$data['posted']);
			$try->set_value('nonmandatory', 'title',		'');
			$try->set_value('nonmandatory', 'username',		$idcache->get_id('username', $data['mem_id']));
			...
			if($try->is_valid())
			{
				if($try->import_post($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['post'] . ' -> ' . $idcache->get_id('username', $data['mem_id']));
					}
...
# Autogenerated on : May 12, 2008, 3:48 pm
