...
class dzoic_003 extends dzoic_000
{
	var $_dependent = '001';

	function dzoic_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}members", 'mem_id', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			{
				case 'a':
					$user_group_id = $user_group_ids_array['69'];		
					break;
				case 'm':
					$user_group_id = $user_group_ids_array['70'];
					break;
				default :
					$user_group_id = $user_group_ids_array['71'];
					
			}
			
			$try->set_value('mandatory', 'usergroupid',			$user_group_id);
			...
			$try->set_value('nonmandatory', 'joindate',			$Db_source->query_first("SELECT joined FROM {$s_tb_prefix}profiles WHERE mem_id={$import_id}"));
