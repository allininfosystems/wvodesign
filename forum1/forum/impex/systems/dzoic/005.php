...
class dzoic_005 extends dzoic_000
{
	var $_dependent = '004';

	function dzoic_005($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}topics", 'top_id', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('mandatory', 'importforumid',	$data['frm_id']);
			$try->set_value('mandatory', 'importthreadid',	$import_id);
			$try->set_value('mandatory', 'forumid',			$forum_ids_array["$data[frm_id]"]);
			...
			$try->set_value('nonmandatory', 'postusername',	$idcache->get_id('username', $data['mem_id']));
			$try->set_value('nonmandatory', 'postuserid',	$idcache->get_id('user', $data['mem_id']));
