...
class ultraboard_003 extends ultraboard_000
{
	var $_dependent = '002';

	function ultraboard_003($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_ultraboard_users($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('nonmandatory', 'usertitle',			$data['LEVEL_TEXT']);
			$try->set_value('nonmandatory', 'posts',				$data['TOTAL_POSTS']);
			$try->set_value('nonmandatory', 'joindate',				$data['REGISTERED_SECOND']);
			$try->set_value('nonmandatory', 'lastpost',				$data['LAST_POST_SECOND']);
			$try->set_value('nonmandatory', 'lastvisit',			$data['LAST_LOGIN_SECOND']);
			...
			$try->add_default_value('Interests',					$data['INTEREST']);
			$try->add_default_value('signature', 					$this->html_2_bb($user['SIGNATURE']));
