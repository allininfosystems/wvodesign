...
*
*/
class trubb_004 extends trubb_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '003';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_trubb_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'usergroupid',				$user_group_ids_array["$user_details[membercode]"]);
			...
			$try->set_value('nonmandatory', 'joindate',				$user_details['joineddate']);
			$try->set_value('nonmandatory', 'timezoneoffset',		$user_details['timeoffset']);
			$try->set_value('nonmandatory', 'ipaddress',			$user_details['lastip']);
			...
			$try->add_default_value('signature', 					$this->html_2_bb(addslashes($user_details['sig'])));
