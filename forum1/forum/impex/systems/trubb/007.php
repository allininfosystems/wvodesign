...
*
*/
class trubb_007 extends trubb_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '006';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 		= $this->get_trubb_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			$try->set_value('mandatory', 'userid',				$user_ids_array["$post_details[author]"]);
			$try->set_value('mandatory', 'importthreadid',		$post_details['tid']);


			// Non Mandatory
			$try->set_value('nonmandatory', 'parentid',			'0');
			$try->set_value('nonmandatory', 'username',			$user_name_array["$post_details[author]"]);
			$try->set_value('nonmandatory', 'title',			$post_details['subject']);
			$try->set_value('nonmandatory', 'dateline',			$post_details['postdate']);
			$try->set_value('nonmandatory', 'pagetext',			stripslashes($this->html_2_bb($post_details['message'])));
			$try->set_value('nonmandatory', 'allowsmilie',		$this->iif($post_details['smiliesoff'],'0','1'));
			$try->set_value('nonmandatory', 'ipaddress',		$post_details['ip']);
			$try->set_value('nonmandatory', 'visible',			'1');
			$try->set_value('nonmandatory', 'importpostid',		$post_id);

			// Needs import smilie mapping
