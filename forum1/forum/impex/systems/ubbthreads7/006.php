...
class ubbthreads7_006 extends ubbthreads7_000
{
	var $_dependent = '005';

	function ubbthreads7_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			if($data['TOPIC_SUBJECT'])
			{
				$try->set_value('mandatory', 'title',			$data['TOPIC_SUBJECT']);
			}
			else
			{
				$try->set_value('mandatory', 'title',			'Blank title');
			}
			...
			$try->set_value('nonmandatory', 'dateline',		$data['TOPIC_CREATED_TIME']);
			...
			$try->set_value('nonmandatory', 'postusername',	$idcache->get_id('username', $data['USER_ID']));
			...
			*/
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['thread'] . ' -> ' . $data['TOPIC_SUBJECT']);
