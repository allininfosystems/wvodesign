...
class ubbthreads7_010 extends ubbthreads7_000
{
	var $_dependent = '004';

	function ubbthreads7_010(&$displayobject)
	{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$sessionobject->add_session_var("012_objects_done", '0');
			$sessionobject->add_session_var("012_objects_failed", '0');
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}PRIVATE_MESSAGE_POSTS", 'POST_ID', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$pm_title = $this->get_ubb7_PM_topic($Db_source, $s_db_type, $s_tb_prefix, $data['TOPIC_ID']);
			$to_users = $this->get_ubb7_PM_tousers($Db_source, $s_db_type, $s_tb_prefix, $data['TOPIC_ID']);
			unset($touserarray);
			...
			{
				$userid = $idcache->get_id('user', $id);				
				$username	= $idcache->get_id('username', $id);
				$touserarray[$userid] = $username;	
			}

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$idcache->get_id('user', $data['USER_ID']));
			$vB_pm_text->set_value('mandatory', 'title',			$pm_title);
			$vB_pm_text->set_value('mandatory', 'message',			$this->html_2_bb($data['POST_BODY']));
			...
			$vB_pm_text->set_value('nonmandatory', 'dateline',		$data['POST_TIME']);
