...
class ubbthreads7_007 extends ubbthreads7_000
{
	var $_dependent = '006';

	function ubbthreads7_007(&$displayobject)
	{
...
$per_page=10000;
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('nonmandatory', 'pagetext',		$this->html_2_bb($data['POST_BODY']));
			$try->set_value('nonmandatory', 'dateline',		$data['POST_POSTED_TIME']);
			$try->set_value('nonmandatory', 'visible',		$data['POST_IS_APPROVED']);
			$try->set_value('nonmandatory', 'showsignature',$data['POST_ADD_SIGNATURE']);
			$try->set_value('nonmandatory', 'ipaddress',	$data['POST_POSTER_IP']);
			...
			if($data['POST_PARENT_ID'])
			{
				$try->set_value('nonmandatory', 'parentid',		$this->get_vb_post_id($Db_target, $t_db_type, $t_tb_prefix, $data['POST_PARENT_ID']));
			}
			else
			{
			...
			*/
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['post'] . ' -> ' . $data['POST_SUBJECT']);
