...
class wordpress_006 extends wordpress_000
{
	var $_dependent = '005';

	function wordpress_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}comments", 'comment_ID', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory blog_text
			$try->set_value('mandatory', 'blogid',				$idcache->get_id('blog', $data['comment_post_ID']));
			$try->set_value('mandatory', 'bloguserid',			$idcache->get_id('user', $data['user_id']));
			$try->set_value('mandatory', 'title',				$data['comment_author']);
			$try->set_value('mandatory', 'pagetext',			$data['comment_content']);
			$try->set_value('mandatory', 'dateline',			strtotime($data['comment_date']));
			...
			$try->set_value('nonmandatory', 'username',			$idcache->get_id('username', $data['user_id']));
			$try->set_value('nonmandatory', 'ipaddress',		$data['comment_author_IP']);
			$try->set_value('nonmandatory', 'reportthreadid',	"0");
			$try->set_value('nonmandatory', 'state',			($data['comment_approved'] == 1 ? 'visible' : 'moderation')); // enum('moderation', 'visible', 'deleted')
			...
			if($try->is_valid())
			{
				...
				if($try->import_blog_text($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">$import_id :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['blog_comment'] . ' -> ' . $data['comment_author']);
					}
