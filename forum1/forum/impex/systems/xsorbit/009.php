...
\*======================================================================*/
/**
* xsorbit_009 Import Poll module
*
...
*
*/
class xsorbit_009 extends xsorbit_000
{
	var $_dependent 	= '006';


	function xsorbit_009(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$poll_array 	= $this->get_xsorbit_poll_details($Db_source, $source_database_type, $source_table_prefix, $poll_start_at, $poll_per_page);
		...
		foreach ($poll_array as $poll_id => $poll_details)
		{
			...
			$try->set_value('nonmandatory', 'multiple',			($poll_details['maxVotes'] == 1 ? 0 : 1));
			$try->set_value('nonmandatory', 'timeout',			$poll_details['expireTime']);
			$try->set_value('nonmandatory', 'numberoptions',	$poll_details['numberoptions']);
			$try->set_value('nonmandatory', 'active',			($poll_details['votingLocked'] == 1 ? 0 : 1));

			
			$poll_voters_array = array();
			
			foreach($poll_details['poll_voters'] AS $xsorbit_user_id => $vote)
			{
				$voter = $user_ids[$xsorbit_user_id];
			...
			if($try->is_valid())
			{
				...
				if($result)
				{
					if($try->import_poll_to_thread($Db_target, $target_database_type, $target_table_prefix, $vb_poll_id, $poll_details['ID_TOPIC']))
					{
