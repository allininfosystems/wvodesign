...
\*======================================================================*/
/**
* phpMyForum_004 Import User module
*
...
*
*/
class phpMyForum_004 extends phpMyForum_000
{
	var $_dependent 	= '003';

	function phpMyForum_004(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_phpMyForum_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			$try->set_value('nonmandatory', 'joindate',			$user_details['reg']);
			
			if ($user_details['geb'])
			{
				$bits = explode('-', $user_details['geb']); // YYYY-MM-DD
			
				$try->set_value('nonmandatory', 'birthday',				$bits[1] . '-' . $bits[2] . '-' . $bits[0]); // MM-DD-YYYY
				$try->set_value('nonmandatory', 'birthday_search',		$user_details['geb']); // YYYY-MM-DD
			}
			...
			$try->set_value('nonmandatory', 'lastactivity',		$user_details['lastaction']);
			$try->set_value('nonmandatory', 'options',			$this->_default_user_permissions);
			$try->set_value('nonmandatory', 'timezoneoffset',	$user_details['time_zone']);
			
			
			if($user['sign'] != '')
			{
				$try->add_default_value('signature', 		addslashes($this->html_2_bb($user['sign'])));
			}			
			
			if ($sessionobject->get_session_var('get_avatars') AND $user_details['avatar'] != NULL)
			{
				if(strstr($moo,'url:'))
				{
					$try->set_value('nonmandatory', 'avatar',	substr($user_details['avatar'], 4));
				}
				elseif (strstr($moo,'upload:'))
				{
					$try->set_value('nonmandatory', 'avatar',	$sessionobject->get_session_var('get_avatars_path') . substr($user_details['avatar'], 7));
				}
				else
				{
