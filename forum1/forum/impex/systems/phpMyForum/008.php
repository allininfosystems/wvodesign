...
\*======================================================================*/
/**
* phpMyForum_008 Import Smilie module
*
...
*
*/
class phpMyForum_008 extends phpMyForum_000
{
	var $_dependent 	= '001';

	function phpMyForum_008(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$smilie_array 	= $this->get_phpMyForum_smilie_details($Db_source, $source_database_type, $source_table_prefix, $smilie_start_at, $smilie_per_page);
		...
		foreach ($smilie_array as $smilie_id => $smilie_details)
		{
			...
			$it_is_a_duplicate = $this->does_smilie_exists($Db_target, $target_database_type, $target_table_prefix, addslashes($smilie_details['code']));
			...
			$try->set_value('nonmandatory', 'smiliepath',		substr($smilie_details['filename'], 9));
			$try->set_value('nonmandatory', 'imagecategoryid',	$smilie_group_id);
			$try->set_value('nonmandatory', 'displayorder',		$smilie_details['view']);
