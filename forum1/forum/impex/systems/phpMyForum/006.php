...
\*======================================================================*/
/**
* phpMyForum_006 Import Thread module
*
...
*
*/
class phpMyForum_006 extends phpMyForum_000
{
	var $_dependent 	= '005';

	function phpMyForum_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_phpMyForum_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'importforumid',	$thread_details['board_id']);
			$try->set_value('mandatory', 'title',			$thread_details['name']);
			$try->set_value('mandatory', 'importthreadid',	$thread_id);
			$try->set_value('mandatory', 'forumid',			$forum_ids_array["$thread_details[board_id]"]);

			// Non Mandatory
			$try->set_value('nonmandatory', 'visible',		$thread_details['show_topic']);
			$try->set_value('nonmandatory', 'sticky',		$thread_details['top']);
			$try->set_value('nonmandatory', 'views',		$thread_details['views']);
			$try->set_value('nonmandatory', 'open',			($thread_details['closed'] == 0 ? 1 : 0));
			$try->set_value('nonmandatory', 'replycount',	$thread_details['posts']);
			$try->set_value('nonmandatory', 'postusername',	$user_name_array["$thread_details[user_id]"]);
			$try->set_value('nonmandatory', 'postuserid',	$user_ids_array["$thread_details[user_id]"]);
			$try->set_value('nonmandatory', 'dateline',		$thread_details['post_date']);
