...
\*======================================================================*/
/**
* phorum5_006 Import Post module
*
...
*
*/
class phorum5_006 extends phorum5_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_phorum5_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			$try->set_value('nonmandatory', 'dateline',			$post_details['datestamp']);
			...
			$try->set_value('nonmandatory', 'importpostid',		$post['message_id']);
			...
			if($try->is_valid())
			{
				...
				else
				{
					...
					$displayobject->display_now("<br />Found post and <b>DID NOT</b> imported to the  {$target_database_type} database");
				}
			}
			else
			{
