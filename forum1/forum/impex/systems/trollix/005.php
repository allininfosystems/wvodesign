...
\*======================================================================*/
/**
* trollix_005 Import Thread module
*
...
*
*/
class trollix_005 extends trollix_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	var $_modulestring 	= 'Import Thread';

	function trollix_005()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_trollix_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			$try->set_value('nonmandatory', 'open',				($thread_details['closed'] == 'yes' ? 0 : 1));
			...
			if($try->is_valid())
			{
				if($threadid = $try->import_thread($Db_target, $target_database_type, $target_table_prefix))
				{
					...
					$post->set_value('mandatory', 'userid',				$user_name_array["$thread_details[author]"]);
		
					// Non Mandatory
					$post->set_value('nonmandatory', 'importpostid',	$thread_id);
					$post->set_value('nonmandatory', 'visible',			'1');
					$post->set_value('nonmandatory', 'ipaddress',		($thread_details['useip'] == 'unknowen' ? '' : $thread_details['useip']));
					$post->set_value('nonmandatory', 'showsignature',	'1');
					$post->set_value('nonmandatory', 'allowsmilie',		'1');
					$post->set_value('nonmandatory', 'pagetext',		$thread_details['message']);
					$post->set_value('nonmandatory', 'dateline',		$thread_details['dateline']);
					$post->set_value('nonmandatory', 'title',			$thread_details['subject']);
					$post->set_value('nonmandatory', 'username',		$thread_details['author']);
					$post->set_value('nonmandatory', 'parentid',		'0');
					
					if($post->import_post($Db_target, $target_database_type, $target_table_prefix))
					{
						$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: post from -> ' . $thread_details['author']);
