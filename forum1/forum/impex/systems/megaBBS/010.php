...
*
*/
class megaBBS_010 extends megaBBS_000
{
	var $_dependent 	= '004';
	var $_modulestring 	= 'Import Private Message';

	function megaBBS_010()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$pm_array 	= $this->get_megaBBS_pm_details($Db_source, $source_database_type, $source_table_prefix, $pm_start_at, $pm_per_page);
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			$userid 	= $idcache->get_id('usernametoid', $pm['toname']);
			$username	= $pm['toname'];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$idcache->get_id('usernametoid', $pm['fromname']));
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$pm['fromname']);
			$vB_pm_text->set_value('nonmandatory', 'iconid',		'');
			$vB_pm_text->set_value('nonmandatory', 'dateline',		strtotime($pm['datesent']));
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$idcache->get_id('usernametoid', $pm['toname']));
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$idcache->get_id('usernametoid', $pm['fromname']));
					...
					if($vB_pm_text->is_valid())
					{
						...
						{
							$displayobject->display_now('<br /><span class="isucc"><b>' . $vB_pm_text->how_complete() . '%,</b></span> Imported from : <b>' . $pm['fromname'] . '</b> , to : <b>' . $pm['toname'] . '</b>');
