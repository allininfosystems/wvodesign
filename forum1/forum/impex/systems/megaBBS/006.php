...
\*======================================================================*/
/**
* megaBBS_006 Import Thread module
*
...
*
*/
class megaBBS_006 extends megaBBS_000
{
	var $_dependent 	= '005';
	var $_modulestring 	= 'Import Thread';

	function megaBBS_006()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_megaBBS_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'title',				$thread_details['threadsubject']);
			...
			// Non Mandatory
			$try->set_value('nonmandatory', 'dateline',			strtotime($thread_details['datecreated']));
			$try->set_value('nonmandatory', 'open',				$this->iif($thread_details['closed'],0,1));
			$try->set_value('nonmandatory', 'views',			$thread_details['timesviewed']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: thread -> ' . $thread_details['threadsubject']);
