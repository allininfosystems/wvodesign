...
*
*/
class MyBB_004 extends MyBB_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '003';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_MyBB_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			$try->set_value('nonmandatory', 'lastactivity',			$user_details['lastactive']);
			...
			if(is_int($user_details['referrer']))
			{
				echo "<h1>'" . $user_details['referrerid'] . "'</h1>";
				$try->set_value('nonmandatory', 'referrerid',		$this->get_vb_userid($Db_target, $target_database_type, $target_table_prefix, $user_details['referrerid']));
			}

			$try->set_value('nonmandatory', 'timezoneoffset',		$user_details['timezone']);
			$try->set_value('nonmandatory', 'pmpopup',				$this->option2bin($user_details['pmpopup']));
			$try->set_value('nonmandatory', 'options',				'2135');

			$b_bits = explode('-', $user_details['birthday']);
