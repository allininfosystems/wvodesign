...
*
*/
class MyBB_007 extends MyBB_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '006';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_MyBB_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			// Non Mandatory
			// Non Mandatory
			if($post_details['replyto'] != 0)
			{
				// If there is a parent id get it.
				$try->set_value('nonmandatory', 'parentid',
					$this->get_vb_post_id($Db_target, $target_database_type, $target_table_prefix, $post_details['replyto']));
			}

			$try->set_value('nonmandatory', 'username',			$post_details['username']);
			$try->set_value('nonmandatory', 'title',			stripslashes($post_details['subject']));
			$try->set_value('nonmandatory', 'dateline',			$post_details['dateline']);
			$try->set_value('nonmandatory', 'pagetext',			$this->MyBB_html($post_details['message']));
			$try->set_value('nonmandatory', 'allowsmilie',		$this->iif($this->option2bin($post_details['smilieoff']),0,1));
			$try->set_value('nonmandatory', 'showsignature',	$this->iif($this->option2bin($post_details['includesig']),0,1));
			$try->set_value('nonmandatory', 'ipaddress',		$post_details['ipaddress']);
			$try->set_value('nonmandatory', 'iconid',			$post_details['icon']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: post -> ' . stripslashes($post_details['subject']));
