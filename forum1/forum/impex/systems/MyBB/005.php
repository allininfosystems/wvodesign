...
*
*/
class MyBB_005 extends MyBB_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_MyBB_categories_details($Db_source, $source_database_type, $source_table_prefix);
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$cat['disporder']);
				...
				$try->set_value('nonmandatory', 'description', 		stripslashes($cat['description']));
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_MyBB_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$forum['disporder']);

				$paret_ids = explode(',', $forum['parentlist']);
				$try->set_value('mandatory', 'parentid',			$cat_ids[$paret_ids[0]]);
				$try->set_value('mandatory', 'importforumid',		$forum['fid']);
				$try->set_value('mandatory', 'importcategoryid',	"0");



				$try->set_value('nonmandatory', 'description', 		stripslashes($forum['description']));
				$try->set_value('nonmandatory', 'visible', 			'1');
				$try->set_value('mandatory', 'options',				$this->_default_forum_permissions);

				// TODO: options, they are there just need to grab them

				/*
				$try->set_value('mandatory', 'styleid','');
				$try->set_value('mandatory', 'importcategoryid','');
