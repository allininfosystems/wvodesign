...
class MyBB_002 extends MyBB_000
{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if ($proceed)
		{
			...
			$displayobject->update_html($displayobject->make_description('<p>If you want to associate a MyBB user (in the left column) with an existing vBulletin user,
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		{
			// Get a list of the MyBB members in this current selection
			$userarray = $this->get_MyBB_members_list($Db_source, $source_database_type, $source_table_prefix, $associate_start_at, $associate_per_page);
			...
			$displayobject->update_html($displayobject->make_description('Put the exsisting <b>vbulletin user id</b> next to the MyBB user that you wish to associate them with'));
		...
		if ($do_associate == 1)
		{
			...
			foreach ($users_to_associate as $key => $value)
			{
				...
				{
					$displayobject->update_html($displayobject->display_now('<p align="center">MyBB user ' .  substr(key($value),12) . ' done.</p>'));
