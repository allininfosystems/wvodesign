...
class fusionbb2_009 extends fusionbb2_000
{
	var $_dependent = '007';

	function fusionbb2_009($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$actual = substr($data['file_name'], strpos($data['file_name'], "-")+1);
			
			if(!is_file($dir . '/' . $data['file_name']) OR $data['pt_post_id'] > 0) // Don't want to attach private attachments to public threads 
			{
				$displayobject->display_now("<br /><b>{$displayobject->phrases['source_file_not']} </b> :: {$actual}");
				$sessionobject->add_error($Db_target, 'warning', $class_num, $attachment_id, $displayobject->phrases['attachment_not_imported'], $actual . ' - ' . $displayobject->phrases['attachment_not_imported_rem_1']);
			...
			$try->set_value('mandatory', 'filename',				$actual);
			$try->set_value('mandatory', 'filedata',				$file);

			// Non mandatory
			$try->set_value('nonmandatory', 'extension',			substr($actual, strrpos($actual, ".")+1));
			...
			if($try->is_valid())
			{
				if($try->import_attachment($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">$import_id :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['attachment'] . ' -> ' . $actual);
					}
