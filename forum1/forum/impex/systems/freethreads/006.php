...
\*======================================================================*/
/**
* freethreads_006 Import Thread module
*
...
*
*/
class freethreads_006 extends freethreads_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			// Ask some questions
			$displayobject->update_html($displayobject->make_input_code('Threads to import per cycle (must be greater than 1)','postperpage',2000));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('forumget') != 'end')
		{
			...
			$post_array 		= $this->get_freethreads_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page, $sessionobject->get_session_var('forumget'));
			$forum_ids_array 	= $this->get_forum_id_by_name($Db_target, $target_database_type, $target_table_prefix);
			$user_ids 			= $this->get_username_to_ids($Db_target, $target_database_type, $target_table_prefix);
			$forum_and_thread_ids = $this->get_forum_and_thread_ids($Db_target, $target_database_type, $target_table_prefix);
			// Display count and pass time
			$displayobject->display_now('<h4>Importing ' . count($post_array) . ' posts from ' . $forum_name . '</h4><p><b>From</b> : ' . $post_start_at . ' ::  <b>To</b> : ' . ($post_start_at + count($post_array)) . '</p>');
			...
			foreach ($post_array as $post_id => $post_details)
			{
				...
				// Mandatory
				$importforumid = $forum_ids_array[$forum_name]['importforumid'];
				
				$try->set_value('mandatory', 'threadid',			$forum_and_thread_ids[$importforumid][$post_details['MAIN']]);
				$try->set_value('mandatory', 'userid',				$user_ids["$post_details[SENDER]"]);
				$try->set_value('mandatory', 'importthreadid',		$post_details['MAIN']);

				// Non Mandatory
				if($post_details['PARENT'] == $post_details['MAIN'])
				{
					$try->set_value('nonmandatory', 'parentid',		'0');
				}
				else
				{
					//TODO: Need to find the correct parent id for threadded posts
					#$try->set_value('nonmandatory', 'parentid',		$this->get_freethreads_parent_id();
					$try->set_value('nonmandatory', 'parentid',		'0');
				}
				
				$try->set_value('nonmandatory', 'username',			$post_details['SENDER']);
				$try->set_value('nonmandatory', 'title',			$post_details['SUBJECT']);
				$try->set_value('nonmandatory', 'dateline',			$post_details['POSTEDDATE']);
				$try->set_value('nonmandatory', 'pagetext',			$post_details['CONTENT']);
				$try->set_value('nonmandatory', 'allowsmilie',		'1');
				$try->set_value('nonmandatory', 'showsignature',	'1');
				$try->set_value('nonmandatory', 'ipaddress',		$post_details['IPADDR']);
