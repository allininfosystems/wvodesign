...
\*======================================================================*/
/**
* punbb_005 Import Thread module
*
* @package			ImpEx.punbb
...
*
*/
class punbb_006 extends punbb_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_punbb_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			// Non Mandatory
			$try->set_value('nonmandatory', 'dateline',		$thread_details['posted']);
			$try->set_value('nonmandatory', 'lastpost',		$thread_details['last_post']);
			$try->set_value('nonmandatory', 'lastposter',		$thread_details['last_poster']);
			$try->set_value('nonmandatory', 'views',		$thread_details['num_views']);
			$try->set_value('nonmandatory', 'replycount',		$thread_details['num_replies']);
			...
			$try->set_value('nonmandatory', 'postusername',		$thread_details['poster']);
			$try->set_value('nonmandatory', 'postuserid',		$user_name_array["$thread_details[poster]"]);
