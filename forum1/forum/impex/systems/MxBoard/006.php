...
\*======================================================================*/
/**
* MxBoard_006 Import Post module
*
...
*
*/
class MxBoard_006 extends MxBoard_000
{
	var $_dependent 	= '005';

	function MxBoard_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_MxBoard_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			$try->set_value('nonmandatory', 'visible',			($post_details['deleted'] == 1 ? 0 : 1));
			$try->set_value('nonmandatory', 'showsignature',	'1');
			$try->set_value('nonmandatory', 'allowsmilie',		'1');
			$try->set_value('nonmandatory', 'pagetext',			$this->MxBoard_html($this->html_2_bb($post_details['message'])));
			$try->set_value('nonmandatory', 'dateline',			$this->do_date($post_details['post_date']));
			$try->set_value('nonmandatory', 'title',			'');
			$try->set_value('nonmandatory', 'username',			$user_name_array["$post_details[uid]"]);

			if($post_details['rmid'] !==0)
			{
				$try->set_value('nonmandatory', 'parentid',		$this->get_vb_post_id($Db_target, $target_database_type, $target_table_prefix, $post_details['rmid']));
			}
			else
			{
