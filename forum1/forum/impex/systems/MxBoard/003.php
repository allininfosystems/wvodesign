...
\*======================================================================*/
/**
* MxBoard_003 Import User module
*
...
*
*/
class MxBoard_003 extends MxBoard_000
{
	var $_dependent 	= '001';

	function MxBoard_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_MxBoard_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			$try->set_value('mandatory', 'username',			$user_details['handle']);
			$try->set_value('mandatory', 'importuserid',		$user_id);
			$try->set_value('mandatory', 'email',				trim($user_details['email']));

			// Non Mandatory
			$try->set_value('nonmandatory', 'joindate',			$this->do_date($user_details['member_since'])); // YYYYMMDDHHMMSS
