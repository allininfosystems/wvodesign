...
class yaf_006 extends yaf_000
{
	var $_dependent = '005';

	function yaf_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_yaf_threads($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('mandatory', 'importforumid',		$data['ForumID']);
			$try->set_value('mandatory', 'importthreadid',		$import_id);
			$try->set_value('mandatory', 'forumid',				$forum_ids_array["$data[ForumID]"]);
			...
			/*
			$try->set_value('nonmandatory', 'sticky',			$data['sticky']);
			...
			$try->set_value('nonmandatory', 'postusername',		$data['postusername']);
			...
			*/
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['thread'] . ' -> ' . $data['Topic']);
