...
class vBB_003 extends vBB_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_vBB_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'usergroupid',				'69');
			...
			$try->set_value('nonmandatory', 'joindate',				strtotime($user_details['registered']));
			...
			$try->set_value('nonmandatory', 'lastactivity',			strtotime($user_details['lastlogin']));
			#$try->set_value('nonmandatory', 'lastpost',			$user_details['lastpost']);
			$try->set_value('nonmandatory', 'posts',				$user_details['numposts']);
			...
			$bday_bits = explode($user_details['birthday']);
			$try->set_value('nonmandatory', 'birthday',				$bday_bits[2] . "-" . $bday_bits[1] . "-" .  $bday_bits[0]);
