...
class vBB_004 extends vBB_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_vBB_category_details($Db_source, $source_database_type, $source_table_prefix);
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$cat['corder']);

				$try->set_value('mandatory', 'description',			'description');
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_vBB_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'parentid',			$cat_ids[$forum['category_ID']]);
				...
				$try->set_value('nonmandatory', 'description', 		$forum['comment']);
				$try->set_value('nonmandatory', 'visible', 			'1');
				$try->set_value('nonmandatory', 'replycount',		$forum['numposts']);

				/*
				$try->set_value('nonmandatory', 'lastpost','');
				$try->set_value('nonmandatory', 'styleid','');
