...
class vBB_008 extends vBB_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$smilie_array 	= $this->get_vBB_smilie_details($Db_source, $source_database_type, $source_table_prefix, $smilie_start_at, $smilie_per_page);
		...
		foreach ($smilie_array as $smilie_id => $smilie_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'smilietext',			stripslashes($smilie_details['pattern']));
			$try->set_value('mandatory', 'importsmilieid',		$smilie_id);


			// Non Mandatory
			$try->set_value('nonmandatory', 'title',			$smilie_details['smiley']);
			$try->set_value('nonmandatory', 'smiliepath',		$smilie_details['smiley']);
