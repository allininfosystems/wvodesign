...
class vBB_009 extends vBB_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$pms_array 	= $this->get_vBB_pmtext_details($Db_source, $source_database_type, $source_table_prefix, $pmtext_start_at, $pmtext_per_page);
		...
		$displayobject->display_now('<h4>Importing ' . count($pms_array) . ' pmtexts</h4><p><b>From</b> : ' . $pmtext_start_at . ' ::  <b>To</b> : ' . ($pmtext_start_at + count($pms_array)) . '</p>');
		...
		foreach ($pms_array as $pm_id => $pm)
		{
			...
			$userid 	= $user_ids_array["$pm[to_user]"];
			$username	= $user_name_array["$pm[to_user]"];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$user_ids_array["$pm[from_user]"]);
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$user_name_array["$pm[from_user]"]);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$user_ids_array["$pm[to_user]"]);
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$user_ids_array["$pm[from_user]"]);
					...
					if($vB_pm_text->is_valid())
					{
						...
						{
							$displayobject->display_now('<br /><span class="isucc"><b>' . $vB_pm_text->how_complete() . '%,</b></span> Imported from : <b>' . $user_name_array[$pm['from_user']] . '</b> , to : <b>' . $user_name_array[$pm['to_user']] . '</b>');
