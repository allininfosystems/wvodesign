...
class vBJournal_003 extends vBJournal_000
{
	var $_dependent = '001';

	function vBJournal_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_vBJournal_users($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			*/
			...
			$blog_user->set_value('nonmandatory', 'title',				$data['journalname']);
			$blog_user->set_value('nonmandatory', 'description',		$data['journaldesc']);
