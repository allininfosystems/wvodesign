...
class vBJournal_005 extends vBJournal_000
{
	var $_dependent = '004';

	function vBJournal_005(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}journal_comments", 'comment_id', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('mandatory', 'bloguserid',			$idcache->get_id('user', $data['commenter_id']));
			$try->set_value('mandatory', 'title',				$data['comment_title']);
			$try->set_value('mandatory', 'pagetext',			$data['comment_text']);
			$try->set_value('mandatory', 'dateline',			$data['comment_date']);
			$try->set_value('mandatory', 'userid',				$idcache->get_id('user', $data['commenter_id']));
			$try->set_value('mandatory', 'importblogtextid',	$import_id);

			// Non mandatory blog_text
			$try->set_value('nonmandatory', 'allowsmilie',		"1");
			$try->set_value('nonmandatory', 'username',			$data['commenter']);
			...
			if($try->is_valid())
			{
				...
				if($try->import_blog_text($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $data['comment_title']);
					}
