...
\*======================================================================*/
/**
* maxportalweb_005 Import Thread module
*
...
*
*/
class maxportalweb_005 extends maxportalweb_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_maxportalweb_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			$try->set_value('nonmandatory', 'postuserid',		$thread_details['T_AUTHOR']);
			$try->set_value('nonmandatory', 'lastposter',		$user_name_array["$thread_details[T_LAST_POST_AUTHOR]"]);
			$try->set_value('nonmandatory', 'dateline',			$this->clean_date($thread_details['T_DATE']));
			$try->set_value('nonmandatory', 'views',			$thread_details['T_VIEW_COUNT']);
			#$try->set_value('nonmandatory', 'iconid',			$thread_details['iconid']);
			$try->set_value('nonmandatory', 'notes',			'Import thread from maxportalweb');
			...
			if($try->is_valid())
			{
				...
				if($thread_id)
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: thread -> ' . $thread_details['T_SUBJECT']);
			...
			// *****************************
			// And do the first post for each as its here.
			// *****************************

			$try = (phpversion() < '5' ? $post_object : clone($post_object));
			// Mandatory
			$try->set_value('mandatory', 'threadid',			$thread_id);
			$try->set_value('mandatory', 'userid',				$user_ids_array["$thread_details[T_AUTHOR]"]);
			$try->set_value('mandatory', 'importthreadid',		$thread_details['TOPIC_ID']);

			// Non Mandatory
			$try->set_value('nonmandatory', 'parentid',			'0');
			$try->set_value('nonmandatory', 'username',			$user_name_array["$thread_details[T_AUTHOR]"]);
			$try->set_value('nonmandatory', 'title',			$thread_details['T_SUBJECT']);
			$try->set_value('nonmandatory', 'dateline',			$this->clean_date($thread_details['T_DATE']));
			$try->set_value('nonmandatory', 'pagetext',			$this->html_2_bb($this->maxportalweb_html($thread_details['T_MESSAGE'])));
			$try->set_value('nonmandatory', 'allowsmilie',		'1');
			$try->set_value('nonmandatory', 'showsignature',	'1');
			$try->set_value('nonmandatory', 'ipaddress',		$thread_details['T_IP']);
			#$try->set_value('nonmandatory', 'iconid',			$post_details['iconid']);
			$try->set_value('nonmandatory', 'visible',			'1');
			#$try->set_value('nonmandatory', 'attach',			$post_details['attach']);
			$try->set_value('nonmandatory', 'importpostid',		$thread_details['TOPIC_ID']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: post -> ' . $thread_details['T_SUBJECT']);
					$sessionobject->add_session_var('006_objects_done',intval($sessionobject->get_session_var('006_objects_done')) + 1 );
				}
				else
				{
					$sessionobject->set_session_var('006_objects_failed',$sessionobject->get_session_var('006_objects_failed') + 1 );
					$sessionobject->add_error('warning', $this->_modulestring, get_class($this) . '::import_custom_profile_pic failed.', 'Check database permissions and database table');
