...
/*======================================================================*\
|| #################################################################### ||
|| # vBulletin Blog 4.1.2 - Licence Number VBF6025393
|| # ---------------------------------------------------------------- # ||
	...
	private function getThreads($type)
	{
		...
		if (!empty($forumchoice))
		{
			...
			while ($thread = $vbulletin->db->fetch_array($threads))
			{
				...
				if ($VB_API_REQUESTS['api_version'] > 1)
				{
					$array[$i]['lastposttime'] = $thread['lastpost'];
				}
				else
				{
					$array[$i]['lastpostdate'] = date($vbulletin->options['dateformat'], $thread['lastpost']);
					$array[$i]['lastposttime'] = date($vbulletin->options['timeformat'], $thread['lastpost']);
				}
