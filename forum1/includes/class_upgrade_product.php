...
class vB_Upgrade_Product
{
	...
	*/
	public function import_dependencies($dependencylist = null)
	{
		...
		// ############## import dependencies
		if (isset($this->productobj['dependencies']['dependency']) AND is_array($this->productobj['dependencies']['dependency']))
		{
	...
	*/
	public function install()
	{
		...
		if (isset($this->productobj['codes']['code']) AND is_array($this->productobj['codes']['code']))
		{
	...
	*/
	public function post_install()
	{
		...
		if (!empty($this->productobj['templates']['template']) AND is_array($this->productobj['templates']['template']))
		{
			...
			foreach ($templates AS $template)
			{
				...
				else
				{
					...
					$querybits[] = "(-1, '$template[templatetype]', '$title', '" . $this->db->escape_string(compile_template($template['value'], false)) . "', '$template[template]', $template[date], '$template[username]', '" . $this->db->escape_string($template['version']) . "', '" . $this->db->escape_string($this->productinfo['productid']) . "')";
				}
		...
		if (isset($this->productobj['templates_mobile']['template']) AND is_array($this->productobj['templates_mobile']['template']))
		{
			$querybits = array();
			$querytemplates = 0;
		...
		if (isset($this->productobj['stylevardfns_mobile']['stylevargroup']) AND is_array($this->productobj['stylevardfns_mobile']['stylevargroup']))
		{
			xml_import_stylevar_definitions($this->productobj['stylevardfns_mobile'], $this->productinfo['productid'], -2);
		}

		if (isset($this->productobj['stylevars_mobile']['stylevar']) AND is_array($this->productobj['stylevars_mobile']['stylevar']))
		{
			xml_import_stylevars($this->productobj['stylevars_mobile'], -2);
		}

		// ############## import hooks/plugins
		if (isset($this->productobj['plugins']['plugin']) AND is_array($this->productobj['plugins']['plugin']))
		{
		...
		// ############## import phrases
		if (isset($this->productobj['phrases']['phrasetype']) AND is_array($this->productobj['phrases']['phrasetype']))
		{
		...
		// ############## import settings
		if (isset($this->productobj['options']['settinggroup']) AND is_array($this->productobj['options']['settinggroup']))
		{
		...
		if ($rebuild['plugins'])
		{
			...
			if ($this->active)
			{
				...
				while ($plugin_info = $this->db->fetch_array($plugin_data))
				{
					...
					{
						$this->registry->pluginlist = vb_unserialize($plugin_info['data']);
					}
					else if ($plugin_info['title'] == 'pluginlistadmin')
					{
						$this->registry->pluginlistadmin = vb_unserialize($plugin_info['data']);
					}
				}
