...
function fetch_visitormessageinfo($vmid)
{
	...
	if (!isset($messagecache["$vmid"]))
	{
		$messagecache["$vmid"] = $vbulletin->db->query_first("
			...
			LEFT JOIN " . TABLE_PREFIX . "ipdata AS ipdata ON (ipdata.ipid = visitormessage.ipaddress)
...
function fetch_vm_ajax_query($userinfo, $vmid, $type = 'wall', $userinfo2 = null)
{
	...
	if ($type != 'wall')
	{
		...
		$sql = "
			SELECT
				visitormessage.*, user.*, ipdata.ip AS messageipaddress, visitormessage.userid AS profileuserid, ipdata.ip AS ipaddress
	...
	else
	{
		...
		$sql = "
			(
				SELECT
					visitormessage.*, visitormessage.dateline AS pmdateline, user.*, ipdata.ip AS messageipaddress, visitormessage.userid AS profileuserid, ipdata.ip AS ipaddress
					$delsql1
			...
			(
				SELECT
					visitormessage.*, visitormessage.dateline AS pmdateline, user.*, ipdata.ip, visitormessage.userid AS profileuserid, ipdata.ip AS ipaddress
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 10:21, Mon Dec 11th 2017 : $Revision: 92673 $
|| # $Date: 2017-01-29 17:34:48 -0800 (Sun, 29 Jan 2017) $
|| ####################################################################
\*======================================================================*/
?>
