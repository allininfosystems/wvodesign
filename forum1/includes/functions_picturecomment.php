	...
	do
	{
		...
		$messages = $vbulletin->db->query_read("
			SELECT SQL_CALC_FOUND_ROWS
				picturecomment.*, user.*, ipdata.ip AS messageipaddress, ipdata.ip AS ipaddress
			...
			LEFT JOIN " . TABLE_PREFIX . "ipdata AS ipdata ON (ipdata.ipid = picturecomment.ipaddress)
...
function fetch_picturecommentinfo($filedataid, $userid, $commentid)
{
	...
	return $vbulletin->db->query_first("
		...
		LEFT JOIN " . TABLE_PREFIX . "ipdata AS ipdata ON (ipdata.ipid = picturecomment.ipaddress)
		WHERE
			picturecomment.commentid = " . intval($commentid) . "
				AND
			picturecomment.filedataid = " . intval($filedataid) . "
				AND
			picturecomment.userid = " . intval($userid) . "
	");
}
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 10:21, Mon Dec 11th 2017 : $Revision: 93087 $
|| # $Date: 2017-02-23 10:23:21 -0800 (Thu, 23 Feb 2017) $
|| ####################################################################
\*======================================================================*/
?>
