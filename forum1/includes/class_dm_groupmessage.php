...
class vB_DataManager_GroupMessage extends vB_DataManager
{
	...
	*/
	function save($doquery = true, $delayed = false, $affected_rows = false, $replace = false, $ignore = false)
	{
		...
		if ($result AND ($this->groupmessage['discussionid'] OR $this->existing['discussionid']))
		{
			...
			$do = (is_bool($result) == true ? (is_numeric($this->existing[$this->primary]) == true ? $this->existing[$this->primary] : $this->existing['discussionid']) : $result);
	...
	*/
	...
	{
		if ($gmid = $this->existing[$this->primary])
		{
			$db =& $this->registry->db;
			...
			if ($this->info['hard_delete'])
			{
				...
				$typeid = vb_Types::instance()->getContentTypeID('vBForum_SocialGroupMessage');

				$db->query_write("
					DELETE FROM " . TABLE_PREFIX . "ipdata WHERE contenttypeid = $typeid AND contentid = $gmid
				");
			}
			else
			{
	...
	*/
	...
	{
		$gmid = intval($this->fetch_field($this->primary));
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 10:21, Mon Dec 11th 2017 : $Revision: 92983 $
