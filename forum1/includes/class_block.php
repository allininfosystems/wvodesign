...
class vB_Block
{
	...
	*/
	...
	public static function create(&$registry, $blockid, &$blockscache = null)
	{
		...
		$blockinfo['configcache'] = vb_unserialize($blockinfo['configcache']);
...
//		require_once(DIR . '/vb/context.php');
	...
	protected function get_pagetext_noquote($pagetext)
	{
		...
		// this previously used process_quote_removal, which is not
		// needed because we always remove the quote here, we don't
		// selectively remove it.
		return preg_replace('#\[quote(=(&quot;|"|\'|)??.*\\2)?\](((?>[^\[]*?|(?R)|.))*)\[/quote\]#siU', '', $pagetext);
	}
	...
	*/
	...
	public function reloadBlockTypes($removemissing = false)
	{
		...
		foreach ($blocks as $block)
		{
			...
			$this->saveBlockConfig($block['blockid'], vb_unserialize($block['configcache']));
		}
	...
	public function rebuildBlockConfigCache($blockid)
	{
		...
		if ($this->registry->db->num_rows($configs) > 0)
		{
			while ($config = $this->registry->db->fetch_array($configs))
			{
				...
				{
					$value = vb_unserialize($config['value']);
				}
				else
				{
					$value = $config['value'];
