...
function switch_css_style($styleid, $templates)
{
	...
	$vbulletin->stylevars = vb_unserialize($style['newstylevars']);
...
function build_style($styleid, $title = '', $actions = array(), $parentlist = '', $indent = '', $resetcache = false)
{
	...
	if ($styleid != -1 AND $styleid != -2)
	{
		...
		if ($actions['docss'] OR $actions['dostylevars'] OR $actions['doreplacements'])
		{
			...
			if ($templateids = implode(',' , vb_unserialize($templatelist)))
			{
				$templates = $vbulletin->db->query_read("
		...
		if ($actions['dostylevars'])
		{
			...
			if ($master_stylevar_cache[$masterstyleid] === null)
			{
				...
				{
					$tmp = vb_unserialize($master_stylevar['value']);
					if (!is_array($tmp))
					{
						$tmp = array('value' => $tmp);
					}
			...
			if (substr(trim($parentlist), 0, -3) != '')
			{
				...
				{
					$newstylevars[$new_stylevar['stylevarid']] = vb_unserialize($new_stylevar['value']);
...
function construct_css($css, $styleid, $styletitle, &$csscolors)
{
	...
	foreach ($css_write_order AS $itemname)
	{
		if (!empty($css["$itemname"]) AND is_array($css["$itemname"]))
		{
...
*/
function compile_template($template, $haltonerror = true, &$errors = array())
{
...
function get_style_export_xml($styleid, $product, $product_version, $title, $mode)
{
	...
	else
	{
		...
		if ($mode == 2)
		{
			...
			$sqlcondition = "templateid IN(" . implode(',', vb_unserialize($style['templatelist'])) . ")";
		...
		else if ($mode == 1)
		{
			...
			$sqlcondition = "styleid <> -1 AND styleid <> -2 AND templateid IN(" . implode(',', vb_unserialize($style['templatelist'])) . ")";
...
)
{
	...
	if ($one_pass OR ($startat == 0))
	{
		...
		if ($master OR $mobilemaster)
		{
			...
			$import_data = vb_unserialize(fetch_adminutil_text("master_style_import_{$product}_{$specialstyleid}"));
