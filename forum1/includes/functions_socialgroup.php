...
function fetch_groupmessageinfo($gmid)
{
	...
	if (!isset($groupmessagecache["$gmid"]))
	{
		$groupmessagecache["$gmid"] = $vbulletin->db->query_first("
			SELECT groupmessage.*, socialgroup.creatoruserid AS group_ownerid, ipdata.ip AS ipaddress
			...
			LEFT JOIN " . TABLE_PREFIX . "ipdata AS ipdata ON (ipdata.ipid = groupmessage.ipaddress)
	...
	if (count($ids_to_lookup))
	{
		$set = $vbulletin->db->query_read ("
			SELECT socialgroup.*,
				...
				(($vbulletin->userinfo['userid']) ? ',groupread.readtime AS readtime' : '') : '') . "
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 10:21, Mon Dec 11th 2017 : $Revision: 92669 $
|| # $Date: 2017-01-29 14:56:34 -0800 (Sun, 29 Jan 2017) $
|| ####################################################################
\*======================================================================*/
