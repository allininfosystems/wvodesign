...
*
* @package	vBulletin
* @version	$Revision: 93210 $
* @date		$Date: 2017-03-03 07:46:25 -0800 (Fri, 03 Mar 2017) $
*/
				...
				if ($this->user_changelog_state AND is_array($this->user_changelog_fields) AND sizeof($this->user_changelog_fields) AND is_array($this->existing) AND sizeof($this->existing) AND is_array($this->user) AND sizeof($this->user))
				{
					...
					foreach($this->user_changelog_fields AS $fieldname)
					{
						...
						if ($doquery)
						{
							...
							$userchangelogid = $this->dbobject->insert_id();

							$ipman->set('rectype', 'other');
							$ipman->set('contentid', $userchangelogid);
							$ipman->set('userid', $this->userchangelog['adminid']);
							$ipman->set('contenttypeid', vB_Types::instance()->getContentTypeID('vBForum_IPAddress'));

							$ipid = $ipman->save();
							$return = $ipman->update_content('userchangelog', 'changeid', $ipid, $userchangelogid);
							unset($ipman);
						}
						else
						{
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 10:21, Mon Dec 11th 2017 : $Revision: 93210 $
|| # $Date: 2017-03-03 07:46:25 -0800 (Fri, 03 Mar 2017) $
|| ####################################################################
\*======================================================================*/
?>
