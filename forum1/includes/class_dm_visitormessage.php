...
class vB_DataManager_VisitorMessage extends vB_DataManager
{
	...
	*/
	...
	{
		if ($vmid = $this->existing[$this->primary])
		{
			$db =& $this->registry->db;
			...
			if ($this->info['hard_delete'])
			{
				...
				$typeid = vb_Types::instance()->getContentTypeID('vBForum_VisitorMessage');

				$db->query_write("
					DELETE FROM " . TABLE_PREFIX . "ipdata WHERE contenttypeid = $typeid AND contentid = $vmid
				");
			}
			else
			{
	...
	*/
	...
	{
		$vmid = intval($this->fetch_field($this->primary));
		vB_Search_Indexcontroller_Queue::indexQueue('vBForum', 'VisitorMessage', 'index', $vmid);
		...
		if (!$this->condition)
		{
			...
			$activity = new vB_ActivityStream_Manage('forum', 'visitormessage');
			$activity->set('contentid', $this->fetch_field($this->primary));
	...
	*/
	...
	{
		$type =& $this->table;
		$contenttypeid = vB_Types::instance()->getContentTypeID('vBForum_VisitorMessage');
