...
class vB_WysiwygHtmlParser
{
	...
	*/
	...
	{
		$text = preg_replace_callback('#<pre(.*)>(.*)</pre>#siU', array($this, 'replace_newline_in_pre'), $text);
		...
		$text = preg_replace_callback('#(<[^<>]+ (src|href))=(\'|"|)??(.*)(\\3)#siU', array($this, 'escape_within_url'), $text);
	...
	*
	* @param	array	Matches from RE, index 2 is the stringtext to be fixed
	*
	* @return	string	Fixed Output
	*/
	protected function replace_newline_in_pre($matches)
	{
		static $find, $replace;
		if (!is_array($find))
		{
		...
		return str_replace($find, $replace, $matches[2]);
	}
	...
	*
	* @param array $matches PCRE match array with the following indexes
	* 	* 1 -- Type of call (tag name and src/href)
	* 	* 2 -- Not used
	* 	* 3 -- Delimiter for the attribute
	* 	* 4 -- URL that will be escaped
	*
	* @return	string	Escaped output.
	*/
	protected function escape_within_url($matches)
	{
		static $find, $replace;
		if (!is_array($find))
		{
		...
		$type = $matches[1];
		$url = $matches[4];
		return $type . '=' . $delimiter . str_replace($find, $replace, $url) . $delimiter;
	}
	...
	*/
	protected function filter_bbcode($text)
	{
		...
		//$text = preg_replace_callback('', array($this, 'replace_newline_in_pre'), $text);
		
		// strip html from php tags
		$text = preg_replace_callback('#\[(html|php)\]((?>[^\[]+?|(?R)|.))*\[/\\1\]#siU', array($this, 'strip_html_from_bbcode'), $text);

		// strip linked URLs from manually entered [url] tags (generic)
		$text = preg_replace(
			'#\[url=(\'|"|&quot;|)<A href="(.*)/??">\\2/??</A>#siU', 
			'[URL=$1$2', 
			$text
	...
	*
	* @param array $matches PCRE match string
	*
	* @param	return	Text without the HTML
	*/
	protected function strip_html_from_bbcode($matches)
	{
		return strip_tags($matches[0], '<p><br>');
	}
	...
	*/
	...
	{
		$text = preg_replace_callback('#<img[^>]+smilieid="(\d+)".*>#siU', array($this, 'translate_smilie_id_text'), $text);
		$text = preg_replace_callback('#<img[^>]+src=(\'|")(.*)(\\1).*>#siU', array($this, 'handle_wysiwyg_img'), $text);
	...
	*
	* @param array $matches PCRE match array with the following indexes
	* 	* 0 -- The full opening image tag
	* 	* 1 -- unused
	* 	* 2 -- image url
	*
	* @return	string	img bbcode
	*/
	protected function handle_wysiwyg_img($matches)
	{
		$img_url = $matches[2];
		$fulltag = $matches[0];

		// handle image attachments stored locally in our db
		if (preg_match('#attachment.php\?attachmentid=(\d+)#si', $img_url, $matches_id) AND 
			preg_match('#class=(\'|"|)([-_a-z0-9\s]+)?\s*previewthumb\s*([-_a-z0-9\s]+)?(\\1)#si', $fulltag))
		{
			return '[ATTACH=CONFIG]' . $matches_id[1] . '[/ATTACH]';
		}
	...
	*/
	protected function translate_smilie_id_text($matches)
	{
		static $smilies;
		$smilieid = $matches[1];
