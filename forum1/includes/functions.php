...
* @ignore
*/
define('COOKIE_SALT', 'rcIuLPnY1OkFRntz2UOf4Roe5');
define('EDITOR_INDENT', 40);

/* Navbar stuff, Dont use 
'DIR' here unless its defined */
$dir = defined('DIR') ? DIR : '.';
...
function mark_content_read($contenttype, $contentid, $type = 'read', $contenttypeid = 0, $time = 0, $userid = 0)
{
	...
	$ipman = datamanager_init('IPData', $vbulletin, ERRTYPE_STANDARD);
	$crman = datamanager_init('ContentRead', $vbulletin, ERRTYPE_STANDARD);
...
function standard_redirect($message = '', $forceredirect = false, $bypasswhitelist = false)
{
	...
	if ($vbulletin->options['useheaderredirect'] AND !$forceredirect AND !headers_sent() AND empty($vbulletin->GPC['postvars']) AND !VB_API)
	{
...
function is_browser($browser, $version = 0)
{
	...
	if (!is_array($is))
	{
		...
		// detect chrome
			...
			# Mozilla/5.0 (Macintosh; Intel Mac OS X 10.18.6) AppleWebKit/535.12 (KHTML, like Gecko) Chrome/54.0.2741.60 Safari/535.12
...
function init_language()
{
	...
	$vbphrasegroup = array();
	$tmp = vb_unserialize($vbulletin->userinfo["lang_phrasegroupinfo"]);
	if (is_array($tmp))
	{
		$vbphrasegroup = $tmp;
	}
...
function build_datastore($title = '', $data = '', $unserialize = 0)
{
	...
	if ($title != '')
	{
		...
		if (is_callable(array($vbulletin->datastore, 'build')))
		{
...
{
	if (defined('VB_FRAMEWORK') AND VB_FRAMEWORK === true)
	{
		return;
	}
...
{
	// Bug was fixed in PHP 5.3 so we dont need this (its unsed anyway).
	trigger_error('Function removed, use PHP var_export() instead.', E_USER_ERROR);
}
...
{
	if (is_string($list) 
	AND substr($list,0,2) == 'a:')
	{
		$list = vb_unserialize($list);
	}
...
/**
 * Used as callback of preg_replace_callback to convert_unicode_char_to_charset function 
 */
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 10:21, Mon Dec 11th 2017 : $Revision: 93880 $
