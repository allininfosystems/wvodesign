...
* @date		$Date: 2017-01-15 06:08:51 -0800 (Sun, 15 Jan 2017) $
*/
	...
	*/
	function sql_select_core($cond_value, $cond_type, $time_start, $time_end, $page, $limit)
	{
		...
		if ($where)
		{
			...
			// build the query
			$query = "
				SELECT ".($this->just_count ? "COUNT(userchangelog.changeid) AS change_count" : 
					"user.*, userchangelog.*, adminuser.username AS admin_username, ipdata.* ") . " 
				...
				JOIN " . TABLE_PREFIX . "ipdata AS ipdata ON(ipdata.ipid = userchangelog.ipaddress)
				WHERE $where 
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 10:21, Mon Dec 11th 2017 : $Revision: 92424 $
|| # $Date: 2017-01-15 06:08:51 -0800 (Sun, 15 Jan 2017) $
|| ####################################################################
\*======================================================================*/
?>
