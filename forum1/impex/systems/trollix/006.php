...
\*======================================================================*/
/**
* trollix_006 Import Post module
*
...
*
*/
class trollix_006 extends trollix_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	var $_modulestring 	= 'Import Post';

	function trollix_006()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_trollix_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			$try->set_value('mandatory', 'userid',				$user_name_array["$post_details[author]"]);

			// Non Mandatory
			$try->set_value('nonmandatory', 'importpostid',		$post_id);
			$try->set_value('nonmandatory', 'visible',			'1');
			$try->set_value('nonmandatory', 'ipaddress',		$post_details['useip']);
			$try->set_value('nonmandatory', 'showsignature',	($post_details['usesig'] == 'yes' ? '1' : '0'));
			$try->set_value('nonmandatory', 'allowsmilie',		($post_details['smileyoff'] == 'yes' ? '0' : '0'));
