...
\*======================================================================*/
/**
* trollix_007 Import Smilie module
*
...
*
*/
class trollix_007 extends trollix_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '001';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$smilie_array 	= $this->get_trollix_smilie_details($Db_source, $source_database_type, $source_table_prefix, $smilie_start_at, $smilie_per_page);
		...
		foreach ($smilie_array as $smilie_id => $smilie_details)
		{
			...
			$try->set_value('nonmandatory', 'title',			substr($smilie_details['url'], strpos($smilie_details['url'], '/')+1, -4));
			
			$try->set_value('nonmandatory', 'smiliepath', 		substr($smilie_details['url'], strpos($smilie_details['url'], '/')+1));
			...
			if($try->is_valid())
			{
				...
				else
				{
					$sessionobject->add_error('warning', $this->_modulestring,
											 get_class($this) . "::import_smilie failed for " . $smilie_details['code'] . ".",
											 'Check database permissions and smilie table');
					$sessionobject->set_session_var($class_num . '_objects_failed',$sessionobject->get_session_var($class_num. '_objects_failed') + 1 );
					$displayobject->display_now("<br />Got smilie " . $smilie_details['code'] . " and <b>DID NOT</b> imported to the " . $target_database_type . " database");
				}
			}
			else
			{
