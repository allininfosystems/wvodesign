...
*
*/
class ldu_008 extends ldu_000
{
	var $_dependent 	= '004';

	function ldu_008(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$pm_array 	= $this->get_ldu_pm_details($Db_source, $source_database_type, $source_table_prefix, $pm_start_at, $pm_per_page);
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			$userid 	= $users_ids["$pm[pm_touserid]"];
			$username	= $user_names["$pm[pm_touserid]"];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$users_ids["$pm[pm_fromuserid]"]);
			$vB_pm_text->set_value('mandatory', 'title',			$pm['pm_title']);
			$vB_pm_text->set_value('mandatory', 'message',			$pm['pm_text']);
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$user_names["$pm[pm_fromuserid]"]);
			$vB_pm_text->set_value('nonmandatory', 'iconid',		'');
			$vB_pm_text->set_value('nonmandatory', 'dateline',		$pm['pm_date']);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$users_ids["$pm[pm_touserid]"]);
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$users_ids["$pm[pm_fromuserid]"]);
