...
*
*/
class ldu_006 extends ldu_000
{
	var $_dependent 	= '005';

	function ldu_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_ldu_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'importforumid',		$thread_details['ft_sectionid']);
			$try->set_value('mandatory', 'title',				$thread_details['ft_title']);
			$try->set_value('mandatory', 'importthreadid',		$thread_id);
			$try->set_value('mandatory', 'forumid',				$forum_ids_array["$thread_details[ft_sectionid]"]);

			// Non Mandatory
			$try->set_value('nonmandatory', 'notes',			'Imported thread');
			$try->set_value('nonmandatory', 'visible',			'1');
			$try->set_value('nonmandatory', 'sticky',			$thread_details['ft_sticky']);
			$try->set_value('nonmandatory', 'views',			$thread_details['ft_viewcount']);
			$try->set_value('nonmandatory', 'open',				'1');
			$try->set_value('nonmandatory', 'replycount',		$thread_details['ft_postcount']);
			$try->set_value('nonmandatory', 'postusername',		$thread_details['ft_firstpostername']);
			$try->set_value('nonmandatory', 'dateline',			trim($thread_details['ft_creationdate']));
