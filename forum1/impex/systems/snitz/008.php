...
class snitz_008 extends snitz_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			$userid 	= $idcache->get_id('user', $pm['M_TO']);
			$username	= $idcache->get_id('username', $pm['M_TO']);
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$userid);
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$idcache->get_id('username', $pm['M_FROM']));
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$idcache->get_id('user', $pm['M_TO']));
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$idcache->get_id('user', $pm['M_FROM']));
