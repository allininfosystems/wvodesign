...
*
*/
class snitz_004 extends snitz_000
{
	var $_dependent 	= '003';


	function snitz_004(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_snitz_cat_details($Db_source, $source_database_type, $source_table_prefix);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_snitz_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'title', 				$forum['F_SUBJECT']);
				$try->set_value('mandatory', 'displayorder',		$forum['F_ORDER']);
				...
				$try->set_value('nonmandatory', 'description', 		$forum['F_DESCRIPTION']);
				$try->set_value('nonmandatory', 'visible', 			'1');
				$try->set_value('nonmandatory', 'replycount',		$forum['F_COUNT']);
				$try->set_value('nonmandatory', 'threadcount',		$forum['F_TOPICS']);
				$try->set_value('nonmandatory', 'lastpost',			$forum['F_LAST_POST']);
				$try->set_value('nonmandatory', 'lastposter',		$user_ids_array["$forum[F_LAST_POST_REPLY_ID]"]);
