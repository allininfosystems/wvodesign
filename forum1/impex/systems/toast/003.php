...
class toast_003 extends toast_000
{
	var $_dependent = '001';

	function toast_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('mandatory', 'importuserid',		$data['ID']);
			...
			$try->set_value('nonmandatory', 'posts',			$data['PostCount']);
			$try->set_value('nonmandatory', 'lastvisit',		strtotime($data['LastLoginDate']));
			$try->set_value('nonmandatory', 'lastpost',			strtotime($data['LastPostDate']));

			// Have to randomize it as can't import
			...
			if($try->is_valid())
			{
				if($try->import_user($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $data['ID'] . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['user'] . ' -> ' . $data['Username']);
					}
