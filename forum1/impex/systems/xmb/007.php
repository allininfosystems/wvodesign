...
class xmb_007 extends xmb_000
{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$displayobject->update_html($displayobject->make_yesno_code("Would you like the XMB smilies to over write the vB ones if there is a duplication ?","over_write_smilies",1));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$smilie_array 	= $this->get_xmb_smilie_details($Db_source, $source_database_type, $source_table_prefix, $smilie_start_at, $smilie_per_page);
		...
		foreach ($smilie_array as $smilie_id => $smilie_details)
		{
			...
			// Non Mandatory
			$try->set_value('nonmandatory', 'title',			$smilie_details['url']);
			...
			if($try->is_valid())
			{
				if($import_smilie)
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: smilie -> ' . $smilie_details['code']);
					...
					else
					{
						...
						$displayobject->display_now("<br />Found  smilie and <b>DID NOT</b> imported to the  {$target_database_type} database " . $smilie_details['code']);
					}
				}
			}
			else
