...
\*======================================================================*/
/**
* fudforum_007 Import Post module
*
...
*
*/
class fudforum_007 extends fudforum_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '006';
	var $_modulestring 	= 'Import Post';

	function fudforum_007()
	{
		// Constructor
	}
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$displayobject->update_html($displayobject->make_input_code('Full Path to msg (where the msg_1, msg_2, etc files are located) no trailing slash','msgpath',$sessionobject->get_session_var('msgpath'),1,60));
			// End the table
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 		= $this->get_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page, 'msg', 'id');
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			if ($user_ids_array["$post_details[poster_id]"])
			{
				$try->set_value('mandatory', 'userid',			$user_ids_array["$post_details[poster_id]"]);
			}
			else
			{
			...
			$try->set_value('nonmandatory', 'showsignature',	$this->option2bin($post_details['show_sig']));
			$try->set_value('nonmandatory', 'allowsmilie',		($post_details['smiley_disabled'] == 'N' ? 1 : 0));
			$try->set_value('nonmandatory', 'importpostid',		$post_id);


			$file = $sessionobject->get_session_var('msgpath') . '/' . 'msg_' . $post_details['file_id'];
			...
			$try->set_value('nonmandatory', 'dateline',			$post_details['post_stamp']);
			$try->set_value('nonmandatory', 'title',			$post_details['subject']);
			$try->set_value('nonmandatory', 'username',			$user_name_array["$post_details[username]"]);
			if ($post_details['reply_to'])
			{
				$try->set_value('nonmandatory', 'parentid',		$this->get_vb_post_id($Db_target, $target_database_type, $target_table_prefix, $post_details['reply_to']));
			}
			else
			{
