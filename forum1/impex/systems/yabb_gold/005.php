...
\*======================================================================*/
/**
* yabb_gold_005 Import Forum module
*
...
*
*/
class yabb_gold_005 extends yabb_gold_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	var $_modulestring 	= 'Import Forum';

	function yabb_gold_005()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_yabb_gold_cat_details($cat_dir);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_yabb_gold_forum_details($cat_dir, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$forum_id); // Where is this hiding ??
				$try->set_value('mandatory', 'parentid',			$cat_ids[ucfirst($forum['category'])]);
