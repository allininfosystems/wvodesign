...
*
*/
class jForum_004 extends jForum_000
{
	var $_dependent 	= '003';

	function jForum_004(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_jForum_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			if ($old_id = $user_details['usergroups'][0])
			{
				// There is at least 1 so use the first
				$try->set_value('mandatory', 'usergroupid',		$user_group_ids_array[$old_id]);
			}

			// Non Mandatory
			if (count($user_details['usergroups'][0]) > 1)
			{
				$new_ids = array();
				
				if (is_array($old_group_ids))
				{
					foreach ($old_group_ids as $id => $old_id)
					{
						if ($id == 0)
						{
							// Ignore the primary group
							continue;
						}
			...
			$try->set_value('nonmandatory', 'pmunread',		$user_details['user_unread_privmsg']);
			...
			$try->set_value('nonmandatory', 'yahoo',		$user_details['user_yim']);
			...
			// Looks like a local uploaded file ?
