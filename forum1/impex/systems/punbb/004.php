...
*
*/
class punbb_004 extends punbb_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '003';
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$this->add_custom_field($Db_target, $tdt, $ttp, 'realname','The users real name');			
		}
		else
		{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_punbb_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			// Mandatory
			if ($user_group_ids_array["$user_details[group_id]"])
			{
			...
			{			
				$try->set_value('nonmandatory', 'customtitle',	addslashes($user_details['title']));
			}
			
			if($user_details['realname'])
			{	
				$try->add_custom_value('realname', 		$user_details['realname']);
			}
			...
			$try->set_value('nonmandatory', 'joindate',		$user_details['registered']);
			$try->set_value('nonmandatory', 'lastvisit',		$user_details['last_visit']);
			$try->set_value('nonmandatory', 'lastactivity',		$user_details['last_action']);
