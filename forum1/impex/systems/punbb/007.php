...
*
*/
class punbb_007 extends punbb_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '006';
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$displayobject->update_html($displayobject->make_input_code('Default title for impoter posts ','defaulttitle',$sessionobject->get_session_var('defaulttitle'),1,60));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_punbb_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			$try->set_value('nonmandatory', 'username',		$post_details['poster']);
			$try->set_value('nonmandatory', 'title',		addslashes($sessionobject->get_session_var('defaulttitle')));
			$try->set_value('nonmandatory', 'dateline',		$post_details['posted']);
			$try->set_value('nonmandatory', 'pagetext',		$this->html_2_bb($post_details['message']));
			$try->set_value('nonmandatory', 'allowsmilie',		$post_details['smilies']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: post from -> ' . $post_details['poster']);
