...
*
*/
class oxygen_004 extends oxygen_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '001';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_oxygen_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			// Mandatory
			if ($user_group_ids_by_name_array["$user_details[status]"])
			{
				$try->set_value('mandatory', 'usergroupid',			$user_group_ids_by_name_array["$user_details[status]"]);
			}
			else
			{
			...
			$try->set_value('nonmandatory', 'homepage',				$user_details['site']);
			...
			$try->set_value('nonmandatory', 'birthday',				$user_details['bday']);
