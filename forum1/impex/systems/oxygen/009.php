...
*
*/
class oxygen_009 extends oxygen_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$pm_array 	= $this->get_oxygen_pm_details($Db_source, $source_database_type, $source_table_prefix, $pm_start_at, $pm_per_page);
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			$userid 	= $user_name_array["$pm[msgto]"];
			$username	= $pm['msgto'];
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$user_name_array["$pm[msgfrom]"]);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$user_name_array["$pm[msgto]"]);
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$user_name_array["$pm[msgfrom]"]);
