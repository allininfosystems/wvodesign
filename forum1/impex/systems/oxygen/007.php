...
*
*/
class oxygen_007 extends oxygen_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '006';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_oxygen_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);

		$thread_ids_array = $this->get_threads_ids($Db_target, $target_database_type, $target_table_prefix);
		$user_name2ids_array = $this->get_username_to_ids($Db_target, $target_database_type, $target_table_prefix);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			$try->set_value('mandatory', 'userid',				$user_name2ids_array["$post_details[author]"]);
			...
			$try->set_value('nonmandatory', 'pagetext',			$this->html_2_bb(stripslashes($post_details['message'])));
			if ($post_details['smilieyoff'] == 'yes')
			{
