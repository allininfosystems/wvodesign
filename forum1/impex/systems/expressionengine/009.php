...
class expressionengine_009 extends expressionengine_000
{
	var $_dependent = '007';

	function expressionengine_009($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}forum_attachments", 'attachment_id', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$full_path = $dir . '/' . $data['filehash'] . $data['extension'];
			...
			// Non mandatory
			$try->set_value('nonmandatory', 'extension',		 substr($data['extension'], 1));
			...
			$try->set_value('nonmandatory', 'counter',			$data['hits']);
			$try->set_value('nonmandatory', 'visible',			'1');
			$try->set_value('nonmandatory', 'dateline',			$data['attachment_date']);
			$try->set_value('nonmandatory', 'userid',			$data['member_id']);
