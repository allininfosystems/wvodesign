...
class phpfusion_009 extends phpfusion_000
{
	...
	function phpfusion_009(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$pm_array 	= $this->get_phpfusion_pm_details($Db_source, $source_database_type, $source_table_prefix, $pm_start_at, $pm_per_page);
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			$userid 	= $users_ids["$pm[message_to]"];
			$username	= $user_names["$pm[message_to]"];

			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',			$users_ids["$pm[message_from]"]);
			$vB_pm_text->set_value('mandatory', 'title',				$pm['message_subject']);
			$vB_pm_text->set_value('mandatory', 'message',				$this->phpfusion_html($pm_text['message_message']));
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',		$user_names["$pm[message_from]"]);
			$vB_pm_text->set_value('nonmandatory', 'iconid',			'');
			$vB_pm_text->set_value('nonmandatory', 'dateline',			$pm['message_datestamp']);
			$vB_pm_text->set_value('nonmandatory', 'showsignature',		'1');
			$vB_pm_text->set_value('nonmandatory', 'allowsmilie',		$pm['message_smileys']);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					$vB_pm = (phpversion() < '5' ? $pm_object : clone($pm_object));

					$vB_pm->set_value('mandatory', 'pmtextid',			$pm_text_id);
					$vB_pm->set_value('mandatory', 'userid',			$users_ids["$pm[message_to]"]);
					$vB_pm->set_value('mandatory', 'importpmid',		'1');
					$vB_pm->set_value('nonmandatory', 'folderid',		$pm['message_folder']-1);
					$vB_pm->set_value('nonmandatory', 'messageread',	$pm['message_read']);
					...
					if($vB_pm->is_valid())
					{
						...
						{
							$displayobject->display_now('<br /><span class="isucc"><b>' . $vB_pm->how_complete() . '%</b></span> ' . $displayobject->phrases['pm'] . ' -> ' . $vB_pm_text->get_value('nonmandatory', 'fromusername'));
