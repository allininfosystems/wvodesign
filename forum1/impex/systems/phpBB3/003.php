...
class phpBB3_003 extends phpBB3_000
{
	var $_dependent = '001';

	function phpBB3_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if ($s_db_type == 'mysql')
		{
			$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}groups", 'group_id', 0, $start_at, $per_page);
		}
		else 
		{
			$data_array = $this->get_phpbb3_usergroups($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		}
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('nonmandatory', 'title',				$data["group_name"]);
			$try->set_value('nonmandatory', 'pmquota',				$data["group_message_limit"]);
			$try->set_value('nonmandatory', 'avatarmaxwidth',		$data["group_avatar_width"]);
			$try->set_value('nonmandatory', 'avatarmaxheight',		$data["group_avatar_height"]);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['usergroup'] . ' -> ' . $data['group_name']);
