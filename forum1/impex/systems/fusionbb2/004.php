...
class fusionbb2_004 extends fusionbb2_000
{
	var $_dependent = '003';

	function fusionbb2_004($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_fusionbb2_user_data($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('nonmandatory', 'lastpost',			$data['info_last_post_time']);
			$try->set_value('nonmandatory', 'lastactivity',		$data['info_last_online']);
			$try->set_value('nonmandatory', 'lastvisit',		$data['info_login_time']);		
			...
			if ($data['info_birthday'])
			{
				#  1954-05-19

			   $year 	= substr($data['info_birthday'], 0, 4);
			   $month 	= substr($data['info_birthday'], 4, 2);
			   $day 	= substr($data['info_birthday'], 6, 2);
			...
			if($data['info_raw_signature'])
			{
				$try->add_default_value('signature', 			$data['info_raw_signature']);
			}			

			
			if ($sessionobject->get_session_var('get_avatars') AND $data['info_avatar'])
			{
