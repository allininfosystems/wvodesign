...
\*======================================================================*/
/**
* simpleboard_003 Import User module
*
...
*
*/
class simpleboard_003 extends simpleboard_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '001';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_simpleboard_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			if($user_details['registerDate'])
			{
				$try->set_value('nonmandatory', 'joindate',			strtotime($user_details['registerDate']));
			}
			if($user_details['lastvisitDate'])
			{
				$try->set_value('nonmandatory', 'lastactivity',		strtotime($user_details['lastvisitDate']));
			}
			$try->add_default_value('Interests',					$user_details['name']);			
			if($user['signature'])
			{
