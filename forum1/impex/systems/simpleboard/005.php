...
\*======================================================================*/
/**
* simpleboard_005 Import Thread module
*
...
*
*/
class simpleboard_005 extends simpleboard_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_simpleboard_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			$try->set_value('mandatory', 'forumid',				$forum_ids_array["$thread_details[catid]"]);
			$try->set_value('mandatory', 'importthreadid',		$thread_id);
			$try->set_value('mandatory', 'importforumid',		$thread_details['catid']);


			// Non Mandatory
			$try->set_value('nonmandatory', 'dateline',			$thread_details['time']);
			$try->set_value('nonmandatory', 'open',				$this->iif($thread_details['locked'],0,1));
			$try->set_value('nonmandatory', 'views',			$thread_details['hits']);
			$try->set_value('nonmandatory', 'sticky',			$thread_details['locked']);
			$try->set_value('nonmandatory', 'postusername',		$thread_details['name']);
