...
*
*/
class discus_file_002 extends discus_file_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '001';
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if ($proceed)
		{
			...
			$displayobject->update_html($displayobject->make_description('<p>If you want to associate a Discuss user (in the left column) with an existing vBulletin user,
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if ($associate_users == 1)
		{
			...
			$userarray = $this->get_discus_file_members_list($sessionobject->get_session_var('adminpath'), $associate_start_at, $associate_per_page);
			...
			$displayobject->update_html($displayobject->make_description('Put the existing <b>vbulletin user id</b> next to the Discus user that you wish to associate them with'));
		...
		if ($do_associate == 1)
		{
			...
			foreach ($users_to_associate as $key => $value)
			{
				...
				{
					$displayobject->update_html($displayobject->display_now('<p align="center">phpBB user ' .  substr(key($value),12) . ' done.</p>'));
