...
class smf2_004 extends smf2_000
{
	var $_dependent = '003';

	function smf2_004(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}members", 'id_member', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			if($user_group_ids_array["$data[id_group]"])
			{
				$try->set_value('mandatory', 'usergroupid',				$user_group_ids_array["$data[id_group]"]);
			}
			else 
			{
				$try->set_value('mandatory', 'usergroupid',				$user_group_ids_array["$data[id_post_group]"]);
			}
			$try->set_value('mandatory', 'username',				$data['member_name']);
			$try->set_value('mandatory', 'importuserid',			$data['id_member']);
			$try->set_value('mandatory', 'email',					$data['email_address']);

			// Non mandatory
			$try->set_value('nonmandatory', 'joindate',				$data['date_registered']);
			$try->set_value('nonmandatory', 'options',				$this->_default_user_permissions);
			$try->set_value('nonmandatory', 'lastvisit',			$data['last_login']);
			$try->set_value('nonmandatory', 'pmunread',				$data['unread_messages']);
			$try->set_value('nonmandatory', 'pmtotal',				$data['instant_messages']);
			...
			$try->set_value('nonmandatory', 'homepage',				$data['website_url']);
			...
			$try->set_value('nonmandatory', 'ipaddress',			$data['member_ip']);
			
			# YYYY-MM-DD
			$year 	= substr($data['birthdate'], 0, 4);
			$month 	= substr($data['birthdate'], 5, 2);
			$day 	= substr($data['birthdate'], 8, 2);

			$try->set_value('nonmandatory', 'birthday',				"{$month}-{$day}-{$year}");
			$try->set_value('nonmandatory', 'birthday_search',		$data['birthdate']);
			...
			if ($sessionobject->get_session_var('get_avatars') AND $data['avatar'] != NULL)
			{
				...
				else
				{
					...
					if(strstr($data['avatar'], '-upload-'))
					{
						$text = substr($data['avatar'], 8);
			
						$try->set_value('nonmandatory', 'avatar',$ava_path . $text);
					}
				}
			}
			...
			if($try->is_valid())
			{
				if($try->import_user($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['user'] . ' -> ' . $data['member_name']);
					}
