...
class smf2_006 extends smf2_000
{
	var $_dependent = '005';

	function smf2_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}topics", 'id_topic', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'title',				$this->get_smf2_thread_title($Db_source, $s_db_type, $s_tb_prefix, $data['id_first_msg']));
			
			$try->set_value('mandatory', 'importforumid',		$data['id_board']);
			$try->set_value('mandatory', 'importthreadid',		$import_id);
			$try->set_value('mandatory', 'forumid',				$forum_ids_array["$data[id_board]"]);

			// Non mandatory
			$try->set_value('nonmandatory', 'visible',			'1');
			$try->set_value('nonmandatory', 'sticky',			$data['is_sticky']);
			$try->set_value('nonmandatory', 'views',			$data['num_views']);
			$try->set_value('nonmandatory', 'open',				($data['locked'] == 0 ? 1 : 0));
			$try->set_value('nonmandatory', 'replycount',		$data['num_replies']);
			$try->set_value('nonmandatory', 'postusername',		$idcache->get_id('username', $data['id_member_started']));
			$try->set_value('nonmandatory', 'postuserid',		$idcache->get_id('user', $data['id_member_started']));
			$try->set_value('nonmandatory', 'lastposter',		$idcache->get_id('username', $data['id_member_updated']));
