...
class smf2_007 extends smf2_000
{
	var $_dependent = '006';

	function smf2_007(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}messages", 'id_msg', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'threadid',			$idcache->get_id('thread', $data['id_topic']));
			$try->set_value('mandatory', 'importthreadid',		$data['id_topic']);
			$try->set_value('mandatory', 'userid',				$idcache->get_id('user', $data['id_member']));
			...
			$try->set_value('nonmandatory', 'allowsmilie',		$data['smileys_enabled']);
			$try->set_value('nonmandatory', 'pagetext',			$this->html_2_bb($data['body']));
			$try->set_value('nonmandatory', 'dateline',			$data['poster_time']);
			$try->set_value('nonmandatory', 'title',			$data['subject']);
			$try->set_value('nonmandatory', 'username',			$data['poster_name']);
			...
			if($try->is_valid())
			{
				if($try->import_post($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['post'] . ' -> ' . $data['poster_name']);
					}
