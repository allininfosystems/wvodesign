...
\*======================================================================*/
/**
* thwboard_007 Import Post module
*
...
*
*/
class thwboard_007 extends thwboard_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '006';
	var $_modulestring 	= 'Import Post';

	function thwboard_007()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array			= $this->get_thwboard_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			// Non Mandatory
			$try->set_value('nonmandatory', 'parentid',			'0'); // Not a threadded system
			if ($post_details['userid'] != 0)
			{
				$try->set_value('nonmandatory', 'username',		$user_name_array["$post_details[username]"]);
			}
			else
			{
				$try->set_value('nonmandatory', 'username',		$post_details['postguestname']);
			}

			$try->set_value('nonmandatory', 'title',			''); // There isn't one
			$try->set_value('nonmandatory', 'dateline',			$post_details['posttime']);
			$try->set_value('nonmandatory', 'pagetext',			$this->html_2_bb($post_details['posttext']));
			$try->set_value('nonmandatory', 'allowsmilie',		$post_details['postsmilies']);
			$try->set_value('nonmandatory', 'showsignature',	'1');
			$try->set_value('nonmandatory', 'ipaddress',		$post_details['postip']);
		...
		{
			$displayobject->display_now('<br>Updateing parent ids to allow for a threaded view....');
