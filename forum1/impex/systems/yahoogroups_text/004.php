...
class yahoogroups_text_004 extends yahoogroups_text_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array = $this->get_yahoogroups_text_thread_details(
							$sessionobject->get_session_var('filespath'),
							$sessionobject->get_session_var('datafile'),
							$sessionobject->get_session_var('groupname'),
							$thread_start_at,
							$thread_per_page
						);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			// Might be thread titles on diffrent page passes, if there is a duplicate, just skipp it and all the posts
			// will go under one thread title that is unique
			if($threadid = $this->yahoo_title_search($Db_target, $target_database_type, $target_table_prefix,$thread_details['thread']['subject']))
			{
				continue;
			}

			$try->set_value('mandatory', 'title',				$thread_details['thread']['subject']);
			$try->set_value('mandatory', 'forumid',				$forum_ids_array[1]);
			...
			$try->set_value('nonmandatory', 'dateline',			$thread_details['thread']['dateline']);
			/*
			...
			*/
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: thread -> ' . $thread_details['thread']['subject']);
