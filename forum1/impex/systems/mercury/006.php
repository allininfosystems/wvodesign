...
\*======================================================================*/
/**
* mercury_006 Import Thread module
*
...
*
*/
class mercury_006 extends mercury_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	var $_modulestring 	= 'Import Thread';

	function mercury_006()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_mercury_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			$try->set_value('mandatory', 'forumid',				$forum_ids_array["$thread_details[topic_forum]"]);
			$try->set_value('mandatory', 'importthreadid',		$thread_id);
			$try->set_value('mandatory', 'importforumid',		$thread_details['topic_forum']);

			// Non Mandatory
			$try->set_value('nonmandatory', 'open',				'1'); // Seems to be a 1,2,4,8,16,32 permissions going ong
			$try->set_value('nonmandatory', 'replycount',		$thread_details['topic_replies']);
			$try->set_value('nonmandatory', 'postusername',		$user_name_array["$thread_details[topic_starter]"]);
			$try->set_value('nonmandatory', 'postuserid',		$user_ids_array["$thread_details[topic_starter]"]);
			$try->set_value('nonmandatory', 'lastposter',		$user_ids_array["$thread_details[topic_last_poster]"]);
			$try->set_value('nonmandatory', 'dateline',			$this->get_mercury_first_post_time($Db_source, $source_database_type, $source_table_prefix, $thread_id));
			$try->set_value('nonmandatory', 'views',			$thread_details['topic_views']);
			#$try->set_value('nonmandatory', 'iconid',			$thread_details['iconid']); 
			$try->set_value('nonmandatory', 'notes',			'Imported');
			$try->set_value('nonmandatory', 'visible',			'1');
			#$try->set_value('nonmandatory', 'sticky',			$thread_details['sticky']); going to be in topic_modes
