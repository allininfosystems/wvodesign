...
class mercury_008 extends mercury_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$pms_array 	= $this->get_mercury_pmtext_details($Db_source, $source_database_type, $source_table_prefix, $pmtext_start_at, $pmtext_per_page);
		...
		foreach ($pms_array as $pm_id => $pm)
		{
			...
			$userid 	= $users_ids["$pm[pm_to]"];
			$username	= $user_names["$pm[pm_to]"];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$users_ids["$pm[pm_from]"]);
			$vB_pm_text->set_value('mandatory', 'title',			$pm['pm_title']);
			$vB_pm_text->set_value('mandatory', 'message',			$this->html_2_bb($pm_text['pm_message']));
			...
			$vB_pm_text->set_value('nonmandatory', 'dateline',		$pm['pm_time']);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$users_ids["$pm[pm_to]"]);
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$users_ids["$pm[pm_from]"]);
