...
class digipost_003 extends digipost_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$usergroup_array 	= $this->get_digipost_usergroup_details($Db_source, $source_database_type, $source_table_prefix, $usergroup_start_at, $usergroup_per_page);
		...
		foreach ($usergroup_array as $usergroup_id => $usergroup_details)
		{
			...
			// Non Mandatory
			$try->set_value('nonmandatory', 'title',				$usergroup_details['userGroupID']);
			$try->set_value('nonmandatory', 'description',			'Imported usergroup #' . $usergroup_details['userGroupID']);

			// There isn't anything else to import ........
			/*
			...
			*/
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: usergroup -> Imported usergroup #' . $usergroup_details['userGroupID']);
