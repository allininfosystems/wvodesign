...
*
*/
class vb_36_016 extends vb_36_000
{
	var $_dependent = '004';

	function vb_36_016(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
			...
			foreach ($avatar_array as $avatar_id => $avatar)
			{
				...
				if($try->is_valid())
				{
					...
					else
					{
						...
						$sessionobject->add_error($Db_target, 'invalid', $class_num, $avatar_id, $displayobject->phrases['avatar_not_imported'], $displayobject->phrases['avatar_not_imported_rem']);
				...
				else
				{
					...
					$sessionobject->add_error($Db_target, 'invalid', $class_num, $avatar_id, $displayobject->phrases['invalid_object'], $try->_failedon);
				}
				unset($try);
			}
		...
		foreach ($custom_avatar_array as $avatar_id => $avatar)
		{
			...
			$try->set_value('nonmandatory', 'userid',				$idcache->get_id('user', $avatar['userid']));
