...
\*======================================================================*/
/**
* minibb_004 Import Forum module
*
...
*
*/
class minibb_004 extends minibb_000
{
	var $_dependent 	= '003';

	function minibb_004(&$displayobject)
	{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$try->set_value('mandatory', 'importcategoryid',	'999');
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$forum_array 	= $this->get_minibb_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
		...
		foreach ($forum_array as $forum_id => $forum_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'parentid',			$cat_ids_array[999]);
			$try->set_value('mandatory', 'importforumid',		$forum_id);
			$try->set_value('mandatory', 'displayorder',		$forum_details['forum_order']);
			$try->set_value('mandatory', 'options',				$this->_default_forum_permissions);
			$try->set_value('mandatory', 'title',				$forum_details['forum_name']);
			$try->set_value('mandatory', 'importcategoryid',	'1');

			// Non Mandatory
			$try->set_value('nonmandatory', 'description',		$forum_details['forum_name']);
