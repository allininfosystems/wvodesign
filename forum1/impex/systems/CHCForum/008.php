...
\*======================================================================*/
/**
* CHCForum_008 Import Pmtext module
*
...
*
*/
class CHCForum_008 extends CHCForum_000
{
	var $_dependent 	= '004';

	function CHCForum_008(&$displayobject)
	{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			if ($this->_restart)
			{
				...
				{
					$displayobject->display_now("<h4>{$displayobject->phrases['pmtexts_cleared']}</h4>");
					$this->_restart = true;
				}
				else
				{
					$sessionobject->add_error(substr(get_class($this) , -3), $displayobject->phrases['pmtext_restart_failed'], $displayobject->phrases['check_db_permissions']);
				}
			}

			// Start up the table
			$displayobject->update_basic('title', $displayobject->phrases['import_pmtext']);
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$pm_array	= $this->get_CHCForum_pmtext_details($Db_source, $source_database_type, $source_table_prefix, $pm_start_at, $pm_per_page);
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			$userid 	= $users_ids["$pm[fe_users_uid]"];
			$username	= $user_names["$pm[fe_users_uid]"];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$users_ids["$pm[cruser_id]"]);
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$user_names["$pm[cruser_id]"]);
			$vB_pm_text->set_value('nonmandatory', 'iconid',		'');
			$vB_pm_text->set_value('nonmandatory', 'dateline',		$pm['crddate']);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$users_ids["$pm[fe_users_uid]"]);
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$users_ids["$pm[cruser_id]"]);
