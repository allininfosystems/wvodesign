...
class mysmartbb_011 extends mysmartbb_000
{
	var $_dependent = '005';

	function mysmartbb_011(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}sectionadmin", 'id', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'userid',				$user_ids_array["$data[member_id]"]);
			$try->set_value('mandatory', 'forumid',				$forum_ids_array["$data[section_id]"]);
