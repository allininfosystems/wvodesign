...
class mysmartbb_005 extends mysmartbb_000
{
	var $_dependent = '004';

	function mysmartbb_005(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_mysmart_categories_details($Db_source, $s_db_type, $s_tb_prefix);
			...
			foreach ($categories_array as $import_id => $data)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$data['sort']);
				...
				if($try->is_valid())
				{
					...
					{
							$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['forum'] . ' -> ' . $data['title']);
		...
		{
			// Get an array data
			$data_array = $this->get_mysmart_forum_details($Db_source, $s_db_type, $s_tb_prefix);
			...
			foreach ($data_array as $import_id => $data)
			{
				...
				// Mandatory
				if ($cat_ids_array["$data[from_main_section]"])
				{
					$try->set_value('mandatory', 'parentid',			$cat_ids_array["$data[from_main_section]"]);
				}
				else
				{
					$forum_ids_array = $this->get_forum_ids($Db_target, $t_db_type, $t_tb_prefix);
					$try->set_value('mandatory', 'parentid',			$forum_ids_array["$data[from_main_section]"]);
				}
