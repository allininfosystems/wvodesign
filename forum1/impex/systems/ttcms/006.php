...
\*======================================================================*/
/**
* ttcms_006 Import Thread module
*
* @package			ImpEx.ttcms
...
*
*/
class ttcms_006 extends ttcms_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_ttcms_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			// Non Mandatory
			$try->set_value('nonmandatory', 'open',				($thread_details['locked'] ? 0 : 1));
			$try->set_value('nonmandatory', 'views',			$thread_details['numViews']);
			$try->set_value('nonmandatory', 'sticky',			$thread_details['isSticky']);
			$try->set_value('nonmandatory', 'postusername',		$user_name_array["$thread_details[ID_MEMBER_STARTED]"]);
			$try->set_value('nonmandatory', 'postuserid',		$user_ids_array["$thread_details[ID_MEMBER_STARTED]"]);
