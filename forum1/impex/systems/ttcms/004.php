...
class ttcms_004 extends ttcms_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_ttcms_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			$try->set_value('nonmandatory', 'joindate',				strtotime($user_details['date']));
			...
			if($user_details['websiteUrl'] != 'http://')
			{
				$try->set_value('nonmandatory', 'homepage',			$user_details['websiteUrl']);
			}


			$try->set_value('nonmandatory', 'avatar',				trim($user['avatar']));
