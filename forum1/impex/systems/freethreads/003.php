...
\*======================================================================*/
/**
* freethreads_003 Import User module
*
...
*
*/
class freethreads_003 extends freethreads_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '001';
	var $_modulestring 	= 'Import User';

	function freethreads_003()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_freethreads_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			$try->set_value('mandatory', 'email',					$user_details['PRIVATE_EMAIL']);
			...
			$try->set_value('nonmandatory', 'lastactivity',			$user_details['LOGINTIME']);
			...
			$try->add_default_value('Interests', 					substr(addslashes($user_details['HOBBIES']), 0, 250));
			$try->add_default_value('Occupation', 					substr(addslashes($user_details['OCCUPATION']), 0, 250));			

			//TODO: Need to add the admin
			/*
				ADMIN
