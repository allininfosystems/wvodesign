...
class bbBoardv2_007 extends bbBoardv2_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_bbBoardv2_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'threadid',			$thread_ids_array["$post_details[startofthread]"]);
			$try->set_value('mandatory', 'userid',				$user_ids_array["$post_details[postedbyid]"]);
			$try->set_value('mandatory', 'importthreadid',		$post_details['startofthread']);

			// Non Mandatory
			// TODO: do they have threadding ?
			$try->set_value('nonmandatory', 'parentid',			'0');
			$try->set_value('nonmandatory', 'username',			$user_name_array["$post_details[postedbyid]"]);
			$try->set_value('nonmandatory', 'title',			$post_details['subject']);
			$try->set_value('nonmandatory', 'dateline',			$post_details['postedbytime']);
			$try->set_value('nonmandatory', 'pagetext',			$this->vb_parse_url($this->bbBoardv2_html($post_details['content'])));
			...
			$try->set_value('nonmandatory', 'importpostid',		$post_details['gpid']);
