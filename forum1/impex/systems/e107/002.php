...
*
*/
class e107_002 extends e107_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '001';
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if ($proceed)
		{
			...
			$displayobject->update_html($displayobject->make_description('<p>If you want to associate a e107 user (in the left column) with an existing vBulletin user,
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		{
			// Get a list of the e107 members in this current selection
			$userarray = $this->get_e107_members_list($Db_source, $source_database_type, $source_table_prefix, $associate_start_at, $associate_per_page);
			...
			$displayobject->update_html($displayobject->make_description('Put the exsisting <b>vbulletin user id</b> next to the e107 user that you wish to associate them with'));
		...
		if ($do_associate == 1)
		{
			...
			foreach ($users_to_associate as $key => $value)
			{
				...
				{
					$displayobject->update_html($displayobject->display_now('<p align="center">e107 user ' .  substr(key($value),12) . ' done.</p>'));
