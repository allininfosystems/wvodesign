...
*
*/
class e107_005 extends e107_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_e107_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'title',			$thread_details['thread_name']);
			$try->set_value('mandatory', 'forumid',			$forum_ids_array["$thread_details[thread_forum_id]"]);
			$try->set_value('mandatory', 'importthreadid',		$thread_id);
			$try->set_value('mandatory', 'importforumid',		$thread_details['thread_forum_id']);


			// Non Mandatory
			
			$try->set_value('nonmandatory', 'dateline',		$thread_details['thread_datestamp']);
			$try->set_value('nonmandatory', 'views',		$thread_details['thread_views']);
			$try->set_value('nonmandatory', 'open',			$thread_details['thread_active']);
			$try->set_value('nonmandatory', 'visible',		'1');
			$try->set_value('nonmandatory', 'postusername',		$thread_details['thread_user']);
			$try->set_value('nonmandatory', 'postuserid',		$user_name_array["$thread_details[thread_user]"]);
			...
			*/
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: thread -> ' . $thread_details['thread_name']);
