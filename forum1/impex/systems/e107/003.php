...
*
*/
class e107_003 extends e107_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '001';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_e107_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			$try->set_value('nonmandatory', 'customtitle',		$user_details['user_customtitle']);
			...
			$user = explode('-', $user_details['user_birthday']); // YYYY-MM-DD
			$try->set_value('nonmandatory', 'birthday',		$user[1] . "-" . $user[2] . "-" . $user[0]);
			$try->set_value('nonmandatory', 'birthday_search',	$user_details['user_birthday']);
			$try->set_value('nonmandatory', 'timezoneoffset',	$user_details['user_timezone']);
			$try->set_value('nonmandatory', 'joindate',		$user_details['user_join']);
			$try->set_value('nonmandatory', 'lastvisit',		$user_details['user_lastvisit']);
			$try->set_value('nonmandatory', 'lastactivity',		$user_details['user_currentvisit']);
			$try->set_value('nonmandatory', 'lastpost',		$user_details['user_lastpost']);
