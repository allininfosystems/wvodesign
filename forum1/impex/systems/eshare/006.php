...
*
*/
class eshare_006 extends eshare_000
{
	var $_dependent 	= '005';

	function eshare_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 		= $this->get_eshare_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			{
				$try->set_value('mandatory', 'threadid',			$thread_ids_array["$post_details[ParentID]"]);
			}
			else if ($post_details['ThreadID'])
			{
