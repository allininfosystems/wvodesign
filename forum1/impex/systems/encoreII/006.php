...
\*======================================================================*/
/**
* encoreII_006 Import Post module
*
...
*
*/
class encoreII_006 extends encoreII_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_encoreII_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'threadid',			$thread_ids_array[$post_details['SubtopicID']+1]);
			$try->set_value('mandatory', 'userid',				$user_name_array["$post_details[RegUser]"]);
			$try->set_value('mandatory', 'importthreadid',		$post_details['SubtopicID']);


			// Non Mandatory
			if($post_details['InReplyTo'])
			{
				$try->set_value('nonmandatory', 'parentid',		$this->get_vb_post_id($Db_target, $target_database_type, $target_table_prefix, $post_details['InReplyTo']+1));
			}
			else
			{
			...
			$try->set_value('nonmandatory', 'username',			$post_details['RegUser']);
			
			$try->set_value('nonmandatory', 'title',			$post_details['MessageTitle']);
			$try->set_value('nonmandatory', 'dateline',			$post_details['DatePosted']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: post from -> ' . $post_details['RegUser']);
