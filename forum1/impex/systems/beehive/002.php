...
class beehive_002 extends beehive_000
{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if ($proceed)
		{
			...
			$displayobject->update_html($displayobject->make_description('<p>If you want to associate a beehive user (in the left column) with an existing vBulletin user,
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		{
			// Get a list of the beehive members in this current selection
			$userarray = $this->get_beehive_members_list($Db_source, $source_database_type, $source_table_prefix, $associate_start_at, $associate_per_page);
			...
			$displayobject->update_html($displayobject->make_description('Put the exsisting <b>vbulletin user id</b> next to the beehive user that you wish to associate them with'));
		...
		if ($do_associate == 1)
		{
			...
			foreach ($users_to_associate as $key => $value)
			{
				...
				{
					$displayobject->update_html($displayobject->display_now('<p align="center">beehive user ' .  substr(key($value),12) . ' done.</p>'));
