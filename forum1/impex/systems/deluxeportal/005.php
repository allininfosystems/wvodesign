...
\*======================================================================*/
/**
* deluxeportal_005 Import Forum module
*
...
*
*/
class deluxeportal_005 extends deluxeportal_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_deluxeportal_categories_details($Db_source, $source_database_type, $source_table_prefix);
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$cat['ordered']);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_deluxeportal_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				*/
				...
				$try->set_value('mandatory', 'displayorder',		$forum['ordered']);
				...
				$try->set_value('mandatory', 'lastpost',			$forum['lastpostdate']);
				#$try->set_value('mandatory', 'lastposter',			$forum['posts']);
				#$try->set_value('mandatory', 'lastthread',			$forum['posts']);
				#$try->set_value('mandatory', 'lastthreadid',		$forum['posts']);
				#$try->set_value('mandatory', 'lasticonid',			$forum['posts']);
				$try->set_value('mandatory', 'threadcount',			$forum['threads']);
				#$try->set_value('mandatory', 'daysprune',			$forum['posts']);
				#$try->set_value('mandatory', 'newpostemail',		$forum['posts']);
				#$try->set_value('mandatory', 'newthreademail',		$forum['posts']);
				#$try->set_value('mandatory', 'parentlist',			$forum['posts']);
				#$try->set_value('mandatory', 'password',			$forum['posts']);
				#$try->set_value('mandatory', 'link',				$forum['posts']);
				#$try->set_value('mandatory', 'childlist',			$forum['posts']);
