...
\*======================================================================*/
/**
* deluxeportal_006 Import Thread module
*
...
*
*/
class deluxeportal_006 extends deluxeportal_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			// Ask some questions
			$displayobject->update_html($displayobject->make_input_code('Threads to import per cycle (must be greater than 1)','threadperpage',250));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_deluxeportal_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'title',				$thread_details['name']);
			...
			$try->set_value('nonmandatory', 'postusername',		$thread_details['username']);
			$try->set_value('nonmandatory', 'postuserid',		$thread_details['lastuserid']);
			$try->set_value('nonmandatory', 'lastposter',		$user_ids_array["$thread_details[lastposter]"]);
			// Can we get away with out this and update it on the post import thread update ?
			...
			$try->set_value('nonmandatory', 'notes',			'An imported thread');
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: thread -> ' . $thread_details['name']);
