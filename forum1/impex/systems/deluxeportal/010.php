...
\*======================================================================*/
/**
* deluxeportal_010 Import Attachment module
*
...
*
*/
class deluxeportal_010 extends deluxeportal_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '007';
			...
			if ($this->_restart)
			{
				...
				else
				{
					$sessionobject->add_error('fatal',
											 $this->_modulestring,
											 get_class($this) . '::restart failed , no_function','Check database permissions');
				}
			}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$attachment_array 	= $this->get_deluxeportal_attachment_details($Db_source, $source_database_type, $source_table_prefix, $attachment_start_at, $attachment_per_page);
		...
		foreach ($attachment_array as $attachment_id => $attachment_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'filename',				$attachment_details['name']);
			...
			$try->set_value('nonmandatory', 'dateline',				$post_details['postdate']);
			$try->set_value('nonmandatory', 'visible',				'1');
			$try->set_value('nonmandatory', 'counter',				'0');
			$try->set_value('nonmandatory', 'filesize',				strlen($attachment_details['attachment']));
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: attachment -> ' . $attachment_details['name']);
