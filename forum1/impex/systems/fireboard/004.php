...
class fireboard_004 extends fireboard_000
{
	var $_dependent = '003';

	function fireboard_004($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_fireboard_users($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			if ($user_group_ids_array["$data[group_id]"] > 0)
			{
			...
			$try->set_value('nonmandatory', 'homepage',				$data['websiteurl']);
			$try->set_value('nonmandatory', 'password',				$data['password']); // Random			
			$try->set_value('nonmandatory', 'joindate',				strtotime($data['registerDate']));
			$try->set_value('nonmandatory', 'posts',				$data['posts']);
			$try->set_value('nonmandatory', 'lastactivity',			strtotime($data['lastvisitDate']));
			$try->set_value('nonmandatory', 'lastvisit',			strtotime($data['lastvisitDate']));
...
# Autogenerated on : January 30, 2008, 2:32 pm
