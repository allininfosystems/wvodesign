...
class wBB_013 extends wBB_000
{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$displayobject->update_html($displayobject->make_input_code('Full Path to wBB uploads folder.','attachmentsfolder',$sessionobject->get_session_var('attachmentsfolder'),1,60));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$attachment_array 	= $this->get_wBB_attachment_details($Db_source, $source_database_type, $source_table_prefix, $attachment_start_at, $attachment_per_page);
		...
		foreach ($attachment_array as $attachment_id => $attachment_details)
		{
			...
			$whole = $dir . "/attachment-{$attachment_id}." . $attachment_details['attachmentextension'];

			if(!is_file($whole))
			{
				$displayobject->display_now('<br /><b>Source file not found </b> :: attachment -> ' . $whole);
				$sessionobject->set_session_var($class_num . '_objects_failed',$sessionobject->get_session_var($class_num. '_objects_failed') + 1 );
				continue;
			}

			$file = $this->vb_file_get_contents($whole);

			// Mandatory
			$try->set_value('mandatory', 'filename',				$attachment_details['attachmentname'] . '.' . $attachment_details['attachmentextension']);
			$try->set_value('mandatory', 'filedata',				$file);
			$try->set_value('mandatory', 'importattachmentid',		$attachment_id);


			// Non Mandatory
			$try->set_value('nonmandatory', 'userid',				'1'); // Could be looked up from postid
			$try->set_value('nonmandatory', 'dateline',				time());
			$try->set_value('nonmandatory', 'visible',				'1');
			$try->set_value('nonmandatory', 'counter',				$attachment_details['counter']);
			$try->set_value('nonmandatory', 'filesize',				$attachment_details['attachmentsize']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: attachment -> ' . $attachment_details['attachmentname'] . '.' . $attachment_details['attachmentextension']);
