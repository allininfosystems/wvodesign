...
*
*/
class MyBB_009 extends MyBB_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$pmtext_array 	= $this->get_MyBB_pmtext_details($Db_source, $source_database_type, $source_table_prefix, $pmtext_start_at, $pmtext_per_page);
		...
		foreach ($pmtext_array as $pm_id => $pm)
		{
			...
			$userid 	= $user_ids_array["$pm[toid]"];
			$username	= $user_name_array["$pm[toid]"];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$user_ids_array["$pm[fromid]"]);
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$user_name_array["$pm[fromid]"]);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$user_ids_array["$pm[toid]"]);
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$user_ids_array["$pm[fromid]"]);
					...
					if($vB_pm_text->is_valid())
					{
						...
						{
							$displayobject->display_now('<br /><span class="isucc"><b>' . $vB_pm_text->how_complete() . '%,</b></span> Imported : ' . $pm['subject']);
