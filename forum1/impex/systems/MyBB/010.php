...
\*======================================================================*/
/**
* MyBB_010 Import Moderator module
*
* @package			ImpEx.MyBB
...
*
*/
class MyBB_010 extends MyBB_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$moderator_array 	= $this->get_MyBB_moderator_details($Db_source, $source_database_type, $source_table_prefix, $moderator_start_at, $moderator_per_page);
		...
		foreach ($moderator_array as $moderator_id => $moderator_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'userid',					$user_ids_array["$moderator_details[uid]"]);
			$try->set_value('mandatory', 'forumid',					$forum_ids_array["$moderator_details[fid]"]);
			$try->set_value('mandatory', 'importmoderatorid',		$moderator_details['mid']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: moderator -> ' . $user_ids_array["$moderator_details[uid]"]);
