...
*
*/
class MyBB_006 extends MyBB_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_MyBB_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			$try->set_value('mandatory', 'importthreadid',		$thread_details['tid']);
			...
			//$try->set_value('nonmandatory', 'pollid',			$thread_details['poll']);
			if($thread_details['closed'] == 'yes')
			{
			...
			$try->set_value('nonmandatory', 'iconid',			$thread_details['icon']);
			...
			$try->set_value('nonmandatory', 'votenum',			$thread_details['numratings']);
			$try->set_value('nonmandatory', 'votetotal',		$thread_details['totalratings']);
