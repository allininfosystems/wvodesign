...
*
*/
class bac_003 extends bac_000
{
	var $_dependent 	= '001';

	function bac_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_bac_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			#$try->set_value('nonmandatory', 'birthday',		$user_details['birthday']);  They only have the month......
			$try->set_value('nonmandatory', 'options',			$this->_default_user_permissions);
			$try->set_value('nonmandatory', 'joindate',			@strtotime($user_details['RegistrationDate']));
