...
class bac_005 extends bac_000
{
	...
	function bac_005(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 		= $this->get_bac_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			// Non Mandatory
			$try->set_value('nonmandatory', 'visible',			($thread_details['Hidden'] == 0 ? 1 : 1));
			$try->set_value('nonmandatory', 'notes',			'Imported thread');
			#$try->set_value('nonmandatory', 'iconid',			$thread_details['iconid']); Posttype table
			#$try->set_value('nonmandatory', 'pollid',			$thread_details['pollid']); Polls are there, though no data
			$try->set_value('nonmandatory', 'open',				'1'); # Suspended ?
			$try->set_value('nonmandatory', 'postusername',		$thread_details['AuthorName']);
			$try->set_value('nonmandatory', 'postuserid',		$user_ids_array["$thread_details[AuthorName]"]);
			$try->set_value('nonmandatory', 'dateline',			strtotime($thread_details['DateStarted']));
