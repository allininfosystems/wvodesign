...
*
*/
class yabbse_007 extends yabbse_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '006';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		#$post_array 	= $this->get_yabbse_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		$post_array = $this->get_source_data($Db_source, $source_database_type, "{$source_table_prefix}messages", 'ID_MSG', 0, $post_start_at, $post_per_page);

		// Display count and pass time
		$displayobject->display_now('<h4>Importing ' . $post_array['count'] . ' posts</h4><p><b>From</b> : ' . $post_start_at . ' ::  <b>To</b> : ' . $post_array['lastid'] . '</p>');

		foreach ($post_array['data'] as $post_id => $post_details)
		{
			$try = (phpversion() < '5' ? $post_object : clone($post_object));
			// Mandatory
			$try->set_value('mandatory', 'threadid',			$idcache->get_id('thread', $post_details['ID_TOPIC']));
			$try->set_value('mandatory', 'userid',				$idcache->get_id('user', $post_details['ID_MEMBER']));
			$try->set_value('mandatory', 'importthreadid',		$post_details['ID_TOPIC']);

			// Non Mandatory
			$try->set_value('nonmandatory', 'username',			$idcache->get_id('username', $post_details['ID_MEMBER']));
			$try->set_value('nonmandatory', 'title',			$post_details['subject']);
			$try->set_value('nonmandatory', 'dateline',			$post_details['posterTime']);
			$try->set_value('nonmandatory', 'pagetext',			$this->html_2_bb($post_details['body']));
			$try->set_value('nonmandatory', 'allowsmilie',		$post_details['smiliesEnabled']);
			$try->set_value('nonmandatory', 'ipaddress',		$post_details['posterIP']);
