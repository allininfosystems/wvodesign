...
*
*/
class wowBB_009 extends wowBB_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$pm_array 	= $this->get_wowBB_pm_details($Db_source, $source_database_type, $source_table_prefix, $pm_start_at, $pm_per_page);
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			$userid 	= $user_ids["$pm[pm_to]"];
			$username	= $user_names["$pm[pm_to]"];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$user_ids["$pm[pm_from]"]);
			$vB_pm_text->set_value('mandatory', 'title',			$pm['pm_subject']);
			$vB_pm_text->set_value('mandatory', 'message',			$this->html_2_bb($this->wowBB_html($pm['text'])));
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$user_names["$pm[pm_from]"]);
			$vB_pm_text->set_value('nonmandatory', 'iconid',		'');
			$vB_pm_text->set_value('nonmandatory', 'dateline',		strtotime($pm['pm_date_time']));
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$user_ids["$pm[pm_to]"]);
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$user_ids["$pm[pm_from]"]);
					...
					if($vB_pm_text->is_valid())
					{
						...
						{
							$displayobject->display_now('<br /><span class="isucc"><b>' . $vB_pm_text->how_complete() . '%,</b></span> Imported from : <b>' . $user_names["$pm[pm_from]"] . '</b> , to : <b>' . $user_names["$pm[pm_to]"] . '</b>');
