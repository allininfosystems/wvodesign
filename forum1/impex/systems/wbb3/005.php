...
class wbb3_005 extends wbb3_000
{
	var $_dependent = '004';

	function wbb3_005($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_wbb3_categories_details($Db_source, $s_db_type, $forum_prefix);
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$cat['boardID']);
				...
				$try->set_value('mandatory', 'importcategoryid',	$cat['boardID']);
			...
			$sessionobject->add_session_var('categoriesfinished','TRUE');
						...
						// Private Category
			$try = (phpversion() < '5' ? $category_object : clone($category_object));
			$try->set_value('mandatory', 'title', 				'ImpEx - Default');
			$try->set_value('mandatory', 'displayorder',		'1');
			$try->set_value('mandatory', 'parentid',			'-1');
			$try->set_value('mandatory', 'importforumid',		'0');
			$try->set_value('mandatory', 'importcategoryid',	'99999');
			$try->set_value('mandatory', 'options',				$this->_default_cat_permissions);
			$try->set_value('nonmandatory', 'description', 		'This is a default category for lost forums');
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_wbb3_forum_details($Db_source, $s_db_type, $forum_prefix, $start_at, $per_page);
			$cat_ids 		=  $this->get_category_ids($Db_target, $t_db_type, $t_tb_prefix);

			$displayobject->display_now("<h4>{$displayobject->phrases['importing']} " . count($forum_array) . " {$displayobject->phrases['forums']}</h4><p><b>{$displayobject->phrases['from']}</b> : " . $start_at . " ::  <b>{$displayobject->phrases['to']}</b> : " . ($start_at + count($forum_array)) . "</p>");
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$forum['boardID']);
