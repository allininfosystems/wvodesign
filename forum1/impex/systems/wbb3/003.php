...
class wbb3_003 extends wbb3_000
{
	var $_dependent = '001';

	function wbb3_003($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}group", 'groupID', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'importusergroupid',	$data['groupID']);
			
			// Non mandatory
			$try->set_value('nonmandatory', 'title',			$data['groupName']);
			$try->set_value('nonmandatory', 'description',		$data['groupDescription']);	
			...
			if($try->is_valid())
			{
				if($try->import_usergroup($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['usergroup'] . ' -> ' . $data['groupName']);
					}
