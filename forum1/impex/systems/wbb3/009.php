...
class wbb3_009 extends wbb3_000
{
	var $_dependent = '007';

	function wbb3_009(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		// TODO: all depends on the packageID 
		
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}attachment", 'attachmentID', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			{
				$displayobject->display_now("<br /><b>{$displayobject->phrases['source_file_not']} </b> :: {$data['attachmentName']}");
			...
			$try->set_value('mandatory', 'filename',				$data['attachmentName']);
			$try->set_value('mandatory', 'filedata',				$file);

			// Non mandatory
			$try->set_value('nonmandatory', 'extension',			substr($data['attachmentName'], strrpos($data['attachmentName'], '.')+1));
			$try->set_value('nonmandatory', 'posthash',				$data['posthash']);
			$try->set_value('nonmandatory', 'postid',				$data['messageID']);
			$try->set_value('nonmandatory', 'filesize',				$data['attachmentSize']);
			$try->set_value('nonmandatory', 'counter',				$data['downloads']);
			$try->set_value('nonmandatory', 'visible',				'1');
			$try->set_value('nonmandatory', 'userid',				$data['userID']);
			...
			if($try->is_valid())
			{
				if($try->import_attachment($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['attachment'] . ' -> ' . $data['attachmentName']);
					}
