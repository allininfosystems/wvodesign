...
*
*/
class webwiz_008 extends webwiz_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '006';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$poll_array = $this->get_webwiz_poll_details($Db_source, $source_database_type, $source_table_prefix, $poll_start_at, $poll_per_page);
		...
		{
			$import_thread_id = $this->get_poll_thread_id($Db_source, $source_database_type, $source_table_prefix, $poll_id);
			...
			$voters 				= 0;
			$no_voters				= false;
			unset($webwiz_voters_array);

			$webwiz_voters_array = $this->get_webwiz_poll_questions($Db_source, $source_database_type, $source_table_prefix, $poll_id);

			foreach ($webwiz_voters_array as $vote_option_id => $option)
			{
			...
			$try->set_value('nonmandatory', 'multiple',			$poll_details['Multiple_votes']);
			...
			if($try->is_valid())
			{
				...
				if($result)
				{
					if($try->import_poll_to_thread($Db_target, $target_database_type, $target_table_prefix, $vb_poll_id, $import_thread_id))
					{
					...
					else
					{
						...
						$displayobject->display_now("<br />Got poll " . $poll['vote_text'] . " and <b>DID NOT</b> attach to the correct thread import thread id = <b>'{$import_thread_id}'</b> and poll is <b>{$poll_id}</b>");
					}
				}
				else
				{
