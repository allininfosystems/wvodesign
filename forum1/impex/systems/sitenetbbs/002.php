...
* sitenetbbs
/**
* sitenetbbs_001 Associate Users
*
...
*
*/
class sitenetbbs_002 extends sitenetbbs_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '001';
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if ($proceed)
		{
			...
			$displayobject->update_html($displayobject->make_description('<p>If you want to associate a sitenetbbs user (in the left column) with an existing vBulletin user,
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		{
			// Get a list of the sitenetbbs members in this current selection
			$userarray = $this->get_members_list($sessionobject->get_session_var('admindata'),$associate_start_at, $associate_per_page);
			...
			$displayobject->update_html($displayobject->make_description('Put the exsisting <b>vbulletin user id</b> next to the sitenetbbs user that you wish to associate them with'));
			...
			{
				$displayobject->update_html($displayobject->make_input_code("$counter) User name - " . $userid . " :: " . $username ,'user_to_ass_' . $userid,'',10));
				$any_more = true;
				$counter++;
			}
		...
		if ($do_associate == 1)
		{
			...
			foreach ($users_to_associate as $key => $value)
			{
				...
				{
					$displayobject->update_html($displayobject->display_now('<p align="center">sitenetbbs user ' .  substr(key($value),12) . ' done.</p>'));
