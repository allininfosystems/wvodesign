...
\*======================================================================*/
/**
* sitenetbbs_003 Import User module
*
...
*
*/
class sitenetbbs_003 extends sitenetbbs_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '001';
	var $_modulestring 	= 'Import User';

	function sitenetbbs_003()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_members_list($path, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $username => $userfile)
		{
			// Get the user file and file() it.
			if(!$userarray = file("$path/$userfile"))           
			{
				$displayobject->display_now("<p>Can't open user file {$username}, skipping</p>");
				continue;
			}
			...
			$try->set_value('mandatory', 'importuserid',		'1');
			...
			$try->set_value('nonmandatory', 'joindate',			trim($userarray[13]));
			$try->set_value('nonmandatory', 'homepage',			addslashes(trim($userarray[6])));
			$try->set_value('nonmandatory', 'customtitle',		addslashes(trim($userarray[4])));
			
			$try->add_default_value('Location', 				addslashes(trim($userarray[8])));
			$try->add_default_value('Interests', 				addslashes(trim($userarray[9])));
			
			if($userarray[5] != '')
			{
				$try->add_default_value('signature', 			$this->html_2_bb(addslashes(trim($userarray[5]))));
			}

			$try->set_value('nonmandatory', 'usertitle',		trim($user_details['usertitle']));
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: user -> ' . $userarray[0]);
