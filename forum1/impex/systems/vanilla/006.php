...
class vanilla_006 extends vanilla_000
{
	var $_dependent = '005';

	function vanilla_006($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_vanilla_threads($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('mandatory', 'importforumid',		$data['CategoryID']);
			$try->set_value('mandatory', 'importthreadid',		$import_id);
			$try->set_value('mandatory', 'forumid',				$forum_ids_array["$data[CategoryID]"]);

			// Non mandatory
			$try->set_value('nonmandatory', 'visible',			$data['Active']);
			$try->set_value('nonmandatory', 'sticky',			$data['Sticky']);
			$try->set_value('nonmandatory', 'views',			$data['CountComments']);
			$try->set_value('nonmandatory', 'open',				($data['Closed'] == 0 ? 1 : 0));
			$try->set_value('nonmandatory', 'replycount',		$data['CountComments']);
			$try->set_value('nonmandatory', 'postusername',		$idcache->get_id('username', $data['AuthUserID']));
			$try->set_value('nonmandatory', 'postuserid',		$idcache->get_id('user', $data['AuthUserID']));
			$try->set_value('nonmandatory', 'lastposter',		$idcache->get_id('username', $data['LastUserID']));
			$try->set_value('nonmandatory', 'dateline',			strtotime($data['DateCreated']));
			...
			if($try->is_valid())
			{
				if($try->import_thread($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['thread'] . ' -> ' . $data['Name']);
					}
