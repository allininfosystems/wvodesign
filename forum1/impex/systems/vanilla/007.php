...
class vanilla_007 extends vanilla_000
{
	var $_dependent = '006';

	function vanilla_007(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_vanilla_posts($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'threadid',			$idcache->get_id('thread', $data['DiscussionID']));
			$try->set_value('mandatory', 'importthreadid',		$data['DiscussionID']);
			$try->set_value('mandatory', 'userid',				$idcache->get_id('user', $data['AuthUserID']));

			// Non mandatory
			$try->set_value('nonmandatory', 'visible',			($data['Deleted'] == 1 ? 0 : 1));
			$try->set_value('nonmandatory', 'ipaddress',		$data['RemoteIp']);
			$try->set_value('nonmandatory', 'showsignature',	'1');
			$try->set_value('nonmandatory', 'allowsmilie',		'1');
			$try->set_value('nonmandatory', 'pagetext',			$data['Body']);
			$try->set_value('nonmandatory', 'dateline',			strtotime($data['DateCreated']));
			$try->set_value('nonmandatory', 'title',			'');
			$try->set_value('nonmandatory', 'username',			$idcache->get_id('username', $data['AuthUserID']));
			...
			if($try->is_valid())
			{
				if($try->import_post($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['post'] . ' -> ' . $idcache->get_id('username', $data['AuthUserID']));
					}
