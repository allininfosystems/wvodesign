...
class vanilla_004 extends vanilla_000
{
	var $_dependent = '003';

	function vanilla_004(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'usergroupid',			$user_group_ids_array["$data[RoleID]"]);
			...
			$try->set_value('nonmandatory', 'joindate',			strtotime($data['DateFirstVisit']));
			$try->set_value('nonmandatory', 'lastactivity',		strtotime($data['DateLastActive']));
			...
			if($try->is_valid())
			{
				if($try->import_user($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['user'] . ' -> ' . $data['Name']);
					}
