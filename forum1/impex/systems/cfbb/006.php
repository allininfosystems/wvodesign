...
class cfbb_006 extends cfbb_000
{
	...
	function cfbb_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_cfbb_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'threadid',			$thread_ids_array["$post_details[topicID]"]);
			$try->set_value('mandatory', 'importthreadid',		$post_details['topicID']);
			$try->set_value('mandatory', 'userid',				$user_ids_array["$post_details[createdBy]"]);
			...
			$try->set_value('nonmandatory', 'pagetext',			$this->html_2_bb($this->cfbb_html($post_details['message'])));
			$try->set_value('nonmandatory', 'dateline',			strtotime($post_details['createdOn']));
			#$try->set_value('nonmandatory', 'title',			$post_details['title']);
			$try->set_value('nonmandatory', 'username',			$user_name_array["$post_details[createdBy]"]);
