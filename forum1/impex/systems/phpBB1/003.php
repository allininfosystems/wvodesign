...
*
*/
class phpBB1_003 extends phpBB1_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '001';
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$displayobject->update_html($displayobject->make_yesno_code("Would you like to import the avatars, this can take some time as they are remotely linked","get_avatars",1));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_phpBB1_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			{
				$try->add_default_value('signature', 		addslashes($this->phpbb_html($this->html_2_bb($user_details['user_sig']))));
			}
			...
			$try->add_default_value('Interests',			$user['user_interest']);
