...
class jive_003 extends jive_000
{
	var $_dependent = '002';

	function jive_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_jive_user_details($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			if ($data['UserGroup'])
			{
				$try->set_value('mandatory', 'usergroupid',			$data['usergroupid']);
			}
			else 
			{
			...
			$try->set_value('nonmandatory', 'password',		$data['passwordHash']);
			$try->set_value('nonmandatory', 'options',		$this->_default_user_permissions);
			$try->set_value('nonmandatory', 'joindate',		substr($data['creationDate'], 0, 10));
