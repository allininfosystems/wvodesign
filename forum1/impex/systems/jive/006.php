...
class jive_006 extends jive_000
{
	var $_dependent = '005';

	function jive_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_jive_post_details($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('nonmandatory', 'dateline',			substr($data['creationDate'], 0, 10));
			$try->set_value('nonmandatory', 'title',			$data['subject']);
			$try->set_value('nonmandatory', 'username',			$idcache->get_id('username', $data['userID']));
			if ($data['parentMessageID'])
			{
				$try->set_value('nonmandatory', 'parentid',		$idcache->get_id('post', $data['parentMessageID']));
			}
			else 
			{
