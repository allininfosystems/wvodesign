...
\*======================================================================*/
/**
* mytopix_005 Import Forum module
*
...
*
*/
class mytopix_005 extends mytopix_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_mytopix_cat_details($Db_source, $source_database_type, $source_table_prefix);
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$cat['forum_position']);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_mytopix_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				else
				{
					...
					$try->set_value('mandatory', 'parentid',		$forum_ids_array["$forum[forum_parent]"]);
				}
				...
				$try->set_value('nonmandatory', 'description', 		$forum['forum_description']);
				$try->set_value('nonmandatory', 'visible', 			$this->iif($forum['forum_closed'],0,1));
