...
\*======================================================================*/
/**
* mytopix_006 Import Thread module
*
...
*
*/
class mytopix_006 extends mytopix_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array		= $this->get_mytopix_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'title',				$thread_details['topics_title']);
			$try->set_value('mandatory', 'forumid',				$forum_ids_array["$thread_details[topics_forum]"]);
			$try->set_value('mandatory', 'importthreadid',		$thread_id);
			$try->set_value('mandatory', 'importforumid',		$thread_details['topics_forum']);


			// Non Mandatory
			$try->set_value('nonmandatory', 'dateline',			$thread_details['topics_date']);
			$try->set_value('nonmandatory', 'postusername',		$user_name_array["$thread_details[topics_author]"]);
			$try->set_value('nonmandatory', 'postuserid',		$user_ids_array["$thread_details[topics_author]"]);
			$try->set_value('nonmandatory', 'views',			$thread_details['topics_views']);
			$try->set_value('nonmandatory', 'replycount',		$thread_details['topics_posts']);
			$try->set_value('nonmandatory', 'lastposter',		$thread_details['topics_last_poster_name']);
			$try->set_value('nonmandatory', 'sticky',			$thread_details['topics_pinned']);
			$try->set_value('nonmandatory', 'open',				'1');
			$try->set_value('nonmandatory', 'visible',			$this->iif($thread_details['topics_hidden'],0,1));
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: thread -> ' . $thread_details['topics_title']);
