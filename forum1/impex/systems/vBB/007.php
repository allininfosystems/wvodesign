...
class vBB_007 extends vBB_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$poll_array 	= $this->get_vBB_poll_details($Db_source, $source_database_type, $source_table_prefix, $poll_start_at, $poll_per_page);
		...
		foreach ($poll_array as $poll_id => $poll_details)
		{
			...
			$options = $this->get_vBB_poll_options($Db_source, $source_database_type, $source_table_prefix, $poll_id);


			// Mandatory
			$try->set_value('mandatory', 'question',			$poll_details['text']);
			$try->set_value('mandatory', 'dateline',			time());
			$try->set_value('mandatory', 'options',				$options['options']);
			$try->set_value('mandatory', 'votes',				$poll_details['votes']);
			$try->set_value('mandatory', 'importpollid',		$poll_id);


			// Non Mandatory
			$try->set_value('nonmandatory', 'active',			$this->option2bin($poll_details['active']));
			$try->set_value('nonmandatory', 'numberoptions',	$options['numberoptions']);
			...
			if($result)
			{
				if($try->import_poll_to_thread($Db_target, $target_database_type, $target_table_prefix, $vb_poll_id, $poll_details['message_ID']))
				{
