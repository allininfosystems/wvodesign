...
class vBB_006 extends vBB_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_vBB_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'threadid',			$thread_ids_array["$post_details[reply]"]);
			$try->set_value('mandatory', 'userid',				$user_ids_array["$post_details[user_ID]"]);
			$try->set_value('mandatory', 'importthreadid',		$post_details['reply']);
			...
			$try->set_value('nonmandatory', 'dateline',			strtotime($post_details['date']));
			$try->set_value('nonmandatory', 'pagetext',			$this->vBB_2_bb($this->html_2_bb($post_details['content'])));
			$try->set_value('nonmandatory', 'allowsmilie',		'1');
			#$try->set_value('nonmandatory', 'showsignature',	$post_details['showsignature']);
			$try->set_value('nonmandatory', 'ipaddress',		$post_details['user_IP']);
