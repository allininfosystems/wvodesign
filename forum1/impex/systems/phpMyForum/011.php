...
\*======================================================================*/
/**
* phpMyForum_011 Import Attachment module
*
...
*
*/
class phpMyForum_011 extends phpMyForum_000
{
	var $_dependent 	= '007';

	function phpMyForum_011(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$attachment_array 	= $this->get_phpMyForum_attachment_details($Db_source, $source_database_type, $source_table_prefix, $attachment_start_at, $attachment_per_page);
		...
		foreach ($attachment_array as $attachment_id => $attachment_details)
		{
			...
			$try->set_value('nonmandatory', 'filehash',			md5($attachment_details['filename']));
			$try->set_value('nonmandatory', 'postid',			$attachment_details['post_id']);
			$try->set_value('nonmandatory', 'filesize',			strlen($file));
