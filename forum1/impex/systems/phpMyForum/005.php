...
\*======================================================================*/
/**
* phpMyForum_005 Import Forum module
*
...
*
*/
class phpMyForum_005 extends phpMyForum_000
{
	var $_dependent 	= '004';

	function phpMyForum_005(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_phpMyForum_cat_details($Db_source, $source_database_type, $source_table_prefix);
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'displayorder',		($cat['rang'] == 0 ? 1 : $cat['rang']));
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_phpMyForum_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				else
				{
					...
					{
						die("No forum parent id");
					}
				}
				$try->set_value('mandatory', 'title', 				$forum['name']);
				$try->set_value('mandatory', 'displayorder',		($forum['rang'] == 0 ? 1 : $forum['rang']));
