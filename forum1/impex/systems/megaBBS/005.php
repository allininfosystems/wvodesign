...
\*======================================================================*/
/**
* megaBBS_005 Import Forum module
*
...
*
*/
class megaBBS_005 extends megaBBS_000
{
	var $_dependent 	= '004';
	var $_modulestring 	= 'Import Forums';

	function megaBBS_005()
	{
		// Constructor
	}
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_megaBBS_cat_details($Db_source, $source_database_type, $source_table_prefix);
		...
		{
			// Sort out the forums
			$forum_array  		= $this->get_megaBBS_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$forum['sortorder']);
				...
				$try->set_value('nonmandatory', 'description', 		$forum['forumdescription']);
