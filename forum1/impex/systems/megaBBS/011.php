...
class megaBBS_011 extends megaBBS_000
{
	...
	function megaBBS_011(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$attachment_array 	= $this->get_megabbs_attachment_details($Db_source, $source_database_type, $source_table_prefix, $attachment_start_at, $attachment_per_page);
		...
		if(is_array($attachment_array))
		{
			foreach ($attachment_array as $attachment_id => $attachment_details)
			{
				...
				if ($attachment_details['infilesystem'])
				{
				...
				$try->set_value('nonmandatory', 'counter',				$attachment_details['downloadcount']);
				$try->set_value('nonmandatory', 'filesize',				$attachment_details['filesize']);
				$try->set_value('nonmandatory', 'postid',				$attachment_details['messageid']);
