...
\*======================================================================*/
/**
* phpwind_010 Import Pmtext module
*
...
*
*/
class phpwind_010 extends phpwind_000
{
	var $_dependent = '004';

	function phpwind_010(&$displayobject)
	{
...
$pm_per_page=2000;
		...
		$pm_array 	= $this->get_phpwind_pmtext_details($Db_source, $source_database_type, $source_table_prefix, $pm_start_at, $pm_per_page);

		$users_ids = $this->get_user_ids($Db_target, $target_database_type, $target_table_prefix, $do_int_val = false);
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			// If no value, set it to 1 (probally the admin)
			$pm['fromuid'] = ($pm['fromuid'] == 0 ? 1 : $pm['fromuid']);
			
			$userid 	= $users_ids["$pm[touid]"];
			$username	= $user_names["$pm[touid]"];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$users_ids["$pm[fromuid]"]);
			$vB_pm_text->set_value('mandatory', 'title',			$pm['title']);
			$vB_pm_text->set_value('mandatory', 'message',			$pm_text['content']);
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$user_names["$pm[fromuid]"]);
			$vB_pm_text->set_value('nonmandatory', 'iconid',		'');
			$vB_pm_text->set_value('nonmandatory', 'dateline',		$pm['mdate']);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$users_ids["$pm[touid]"]);
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$users_ids["$pm[fromuid]"]);
