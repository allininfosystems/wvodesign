...
class zeroforum_001 extends zeroforum_000
{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$displayobject->update_html($displayobject->make_input_code('Full Path to folder with JUST the forum XML files in.','forumpath',$sessionobject->get_session_var('forumpath'),1,60));
		$displayobject->update_html($displayobject->make_input_code('Full Path and filename to the users XML file','userfile',$sessionobject->get_session_var('userfile'),1,60));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$databasedone =	$this->check_path($displayobject, $sessionobject, $sessionobject->get_session_var('forumpath'));
		$databasedone =	$this->check_file($displayobject, $sessionobject, $sessionobject->get_session_var('userfile'));
