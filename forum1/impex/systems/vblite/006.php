...
*
*/
class vblite_006 extends vblite_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_vblite_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			if($user_names_to_ids["$post_details[username]"])
			{
				$try->set_value('mandatory', 'userid',			$user_names_to_ids["$post_details[username]"]);
			}
			else
			{
