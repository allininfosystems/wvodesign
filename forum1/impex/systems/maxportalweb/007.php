...
\*======================================================================*/
/**
* maxportalweb_007 Import Poll module
*
...
*
*/
class maxportalweb_007 extends maxportalweb_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$poll_array 	= $this->get_maxportalweb_poll_details($Db_source, $source_database_type, $source_table_prefix, $poll_start_at, $poll_per_page);
		...
		foreach ($poll_array as $poll_id => $poll_details)
		{
			...
			for($i = 1; $i < 13; $i++)
			{
				if($poll_details["ANSWER{$i}"])
				{
					$options 	.= $poll_details["ANSWER{$i}"] . '|||';
					$votes 		.= $poll_details["RESULT{$i}"] . '|||';
					$numberoptions++;
					$voters 	+= $poll_details["RESULT{$i}"];
				}
				else
				{
					break;
			...
			$maxportalweb_voters_array = $this->get_maxportalweb_vote_voters($Db_source, $source_database_type, $source_table_prefix, $poll['POLL_ID']);
			$importthreadid = $this->get_maxportalweb_poll_threadid($Db_source, $source_database_type, $source_table_prefix, $poll_details['POLL_ID']);


			foreach ($maxportalweb_voters_array AS $vid => $info)
			{
				$memberid = $user_ids_array["$info[MEMBER_ID]"];
				$poll_voters_array[$memberid] =  $info['ANS_VALUE'];
			}


			$try->set_value('mandatory', 'importpollid',		$poll_id);
			$try->set_value('mandatory', 'question',			$poll_details['POLL_QUESTION']);
			$try->set_value('mandatory', 'dateline',			$this->clean_date($poll_details['POST_DATE']));
			...
			if($try->is_valid())
			{
				...
				if($result)
				{
					if($try->import_poll_to_thread($Db_target, $target_database_type, $target_table_prefix, $vb_poll_id, $importthreadid))
					{
