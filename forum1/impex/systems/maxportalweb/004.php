...
\*======================================================================*/
/**
* maxportalweb_004 Import Forum module
*
...
*
*/
class maxportalweb_004 extends maxportalweb_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '003';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_maxportalweb_categories_details($Db_source, $source_database_type, $source_table_prefix);
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('nonmandatory', 'description', 		$forum['CAT_NAME']);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_maxportalweb_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('nonmandatory', 'visible', 			$forum['F_STATUS']);
				#$try->set_value('mandatory', 'options',				$forumoptions);

				$try->set_value('mandatory', 'threadcount',			$forum['F_TOPICS']);
				$try->set_value('mandatory', 'replycount',			$forum['F_COUNT']);
