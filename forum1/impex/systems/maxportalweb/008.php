...
\*======================================================================*/
/**
* maxportalweb_008 Import Pmtext module
*
...
*
*/
class maxportalweb_008 extends maxportalweb_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '003';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$pmtext_array 	= $this->get_maxportalweb_pmtext_details($Db_source, $source_database_type, $source_table_prefix, $pmtext_start_at, $pmtext_per_page);
		...
		{
			$vB_pm_text = $pmtext_object;


			$userid 	= $user_ids_array["$pm[M_TO]"];
			$username	= $user_name_array["$pm[M_TO]"];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$user_ids_array["$pm[M_FROM]"]);
			$vB_pm_text->set_value('mandatory', 'title',			$pm['M_SUBJECT']);
			$vB_pm_text->set_value('mandatory', 'message',			$this->html_2_bb($pm_text['M_MESSAGES']));
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$user_name_array["$pm[M_FROM]"]);
			$vB_pm_text->set_value('nonmandatory', 'iconid',		'');
			$vB_pm_text->set_value('nonmandatory', 'dateline',		$this->clean_date($pm['M_SENT']));
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_object)
				{
					...
					if($vB_pm_text->is_valid())
					{
						...
						{
							$displayobject->display_now('<br /><span class="isucc"><b>' . $vB_pm_text->how_complete() . '%,</b></span> Imported : <b>' . $pm['M_SUBJECT'] . '</b>');
