...
\*======================================================================*/
/**
* maxportalweb_003 Import User module
*
...
*
*/
class maxportalweb_003 extends maxportalweb_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '001';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_maxportalweb_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			$try->set_value('nonmandatory', 'homepage',				$user_details['M_HOMEPAGE']);
			$try->set_value('nonmandatory', 'icq',					$user_details['M_ICQ']);
			$try->set_value('nonmandatory', 'aim',					$user_details['M_AIM']);
			$try->set_value('nonmandatory', 'yahoo',				$user_details['M_YAHOO']);
			$try->set_value('nonmandatory', 'showvbcode',			'1');
			$try->set_value('nonmandatory', 'usertitle',			$user_details['M_TITLE']);
			#$try->set_value('nonmandatory', 'customtitle',			$user_details['customtitle']);
			$try->set_value('nonmandatory', 'joindate',				$this->clean_date($user_details['M_DATE']));
			#$try->set_value('nonmandatory', 'daysprune',			$user_details['daysprune']);
			$try->set_value('nonmandatory', 'lastvisit',			$this->clean_date($user_details['M_LASTHEREDATE']));
			$try->set_value('nonmandatory', 'lastactivity',			$this->clean_date($user_details['M_LASTPOSTDATE']));
			$try->set_value('nonmandatory', 'lastpost',				$this->clean_date($user_details['M_LASTPOSTDATE']));
			...
			$try->add_default_value('signature', 					addslashes($this->html_2_bb($user_details['M_SIG'])));

			$try->add_custom_value('loginname', 	$user_details['M_USERNAME']);
			$try->add_custom_value('forename', 		$user_details['M_FIRSTNAME']);
			$try->add_custom_value('surname', 		$user_details['M_LASTNAME']);

			$try->add_default_value('Location',		$user_details['M_CITY'] . ',' . $user_details['M_STATE']);
			$try->add_default_value('Interests',	$user_details['M_HOBBIES']);
			$try->add_default_value('Occupation',	$user_details['M_OCCUPATION']);
