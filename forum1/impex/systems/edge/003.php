...
*
*/
class edge_003 extends edge_000
{
	var $_dependent 	= '001';


	function edge_003(&$displayobject)
	{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$this->add_custom_field($Db_target, $tdt, $ttp, 'fullname','the full name');
			$this->add_custom_field($Db_target, $tdt, $ttp, 'likes','likes');
			$this->add_custom_field($Db_target, $tdt, $ttp, 'dislikes','dislikes');
			$this->add_custom_field($Db_target, $tdt, $ttp, 'address','dislikes');
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_edge_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			// Non Mandatory
			$try->set_value('nonmandatory', 'ipaddress',		$user_details['ip']);
			$try->_password_md5_already = true;
			$try->set_value('nonmandatory', 'password',			substr($user_details['password'], 3));
			$try->set_value('nonmandatory', 'joindate',			$user_details['joindate']);
			$try->set_value('nonmandatory', 'icq',				$user_details['icq']);
			$try->set_value('nonmandatory', 'aim',				substr(addslashes($this->html_2_bb($user_details['aim'])), 18));
			$try->set_value('nonmandatory', 'msn',				substr(addslashes($this->html_2_bb($user_details['msn'])), 18));
			$try->set_value('nonmandatory', 'yahoo',			substr(addslashes($this->html_2_bb($user_details['yah'])), 18));
			$try->set_value('nonmandatory', 'options',			$this->_default_user_permissions);
			$try->set_value('nonmandatory', 'posts',			$user_details['nrposts']);

			// Whats the encoding
			#$try->set_value('nonmandatory', 'birthday',		$user_details['birthday']);

			$try->add_default_value('Occupation', 				addslashes($this->html_2_bb($user_details['occupation'])));
			$try->add_default_value('Interests', 				addslashes($this->html_2_bb($user_details['interests'])));


			$try->add_custom_value('fullname', 					substr(addslashes($user_details['name']), 0 , 240));
			$try->add_custom_value('likes', 					substr(addslashes($this->html_2_bb($user_details['likes'])), 0 , 240));
			$try->add_custom_value('dislikes', 					substr(addslashes($this->html_2_bb($user_details['dislikes'])), 0 , 240));
#			$try->add_custom_value('address', 					substr(addslashes($this->html_2_bb($user_details['address'])), 0 , 240));
