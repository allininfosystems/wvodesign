...
*
*/
class eve_001 extends eve_000
{
	function eve_001(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('added_default_group') != 'yup')
		{
			...
			$try->set_value('mandatory', 'importusergroupid',		'2');
			$try->set_value('nonmandatory', 'title',				"{$displayobject->phrases['imported']} banned {$displayobject->phrases['users']}");
			$try->import_usergroup($Db_target, $target_db_type, $target_table_prefix);
