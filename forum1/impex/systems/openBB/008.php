...
*
*/
class openBB_008 extends openBB_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '006';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$poll_array 	= $this->get_openBB_poll_details($Db_source, $source_database_type, $source_table_prefix, $poll_start_at, $poll_per_page);
		...
		foreach ($poll_array as $poll_id => $poll)
		{
			...
			$user_voters_names = $poll['total'];
			$all_names = explode(',',$user_voters_names);

			foreach ($all_names AS $name)
			{
				$poll_voters_array["$user_name_array[$name]"] = 0;
			}

			$time_and_id = $this->get_openBB_thread_poll_info($Db_source, $source_database_type, $source_table_prefix, $poll['id']);

			if(empty($time_and_id['id']))
			{
				// The poll has been deleted
				$displayobject->display_now("<br />Poll or thread deleted, skipping.");
				continue;
			}

			$try = (phpversion() < '5' ? $poll_object : clone($poll_object));
			// Mandatory
			$try->set_value('mandatory', 'question',			'Imported Poll');
			...
			$try->set_value('mandatory', 'importpollid',		$poll_details['id']);
			...
			if($try->is_valid())
			{
				...
				if($result)
				{
					if($try->import_poll_to_thread($Db_target, $target_database_type, $target_table_prefix, $vb_poll_id, $time_and_id['id']))
					{
