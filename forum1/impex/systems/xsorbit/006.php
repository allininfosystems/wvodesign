...
\*======================================================================*/
/**
* xsorbit_006 Import Thread module
*
...
*
*/
class xsorbit_006 extends xsorbit_000
{
	var $_dependent 	= '005';


	function xsorbit_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_xsorbit_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'importforumid',		$thread_details['ID_BOARD']);
			$try->set_value('mandatory', 'title',				$thread_details['subject']);
			$try->set_value('mandatory', 'importthreadid',		$thread_details['ID_TOPIC']);
			$try->set_value('mandatory', 'forumid',				$forum_ids_array["$thread_details[ID_BOARD]"]);
			...
			$try->set_value('nonmandatory', 'open',				($thread_details['locked'] == 1 ? 0 : 1));
			$try->set_value('nonmandatory', 'replycount',		$thread_details['numReplies']);
			$try->set_value('nonmandatory', 'postusername',		$thread_details['posterName']);
			$try->set_value('nonmandatory', 'postuserid',		$user_ids_array["$thread_details[ID_MEMBER]"]);
			$try->set_value('nonmandatory', 'lastposter',		$user_ids_array["$thread_details[ID_MEMBER_UPDATED]"]);
			$try->set_value('nonmandatory', 'dateline',			$thread_details['posterTime']);
