...
\*======================================================================*/
/**
* xsorbit_007 Import Post module
*
...
*
*/
class xsorbit_007 extends xsorbit_000
{
	var $_dependent 	= '006';


	function xsorbit_007(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_xsorbit_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'threadid',			$thread_ids_array["$post_details[ID_TOPIC]"]);
			$try->set_value('mandatory', 'importthreadid',		$post_details['ID_TOPIC']);
			$try->set_value('mandatory', 'userid',				$user_ids_array["$post_details[ID_MEMBER]"]);
			...
			$try->set_value('nonmandatory', 'allowsmilie',		$post_details['smileysEnabled']);
			$try->set_value('nonmandatory', 'pagetext',			$this->xsorbit_html($this->html_2_bb($post_details['body'])));
			$try->set_value('nonmandatory', 'dateline',			$post_details['posterTime']);
			$try->set_value('nonmandatory', 'title',			$post_details['subject']);
			$try->set_value('nonmandatory', 'username',			$post_details['posterName']);
