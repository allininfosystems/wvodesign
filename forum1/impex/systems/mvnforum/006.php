...
class mvnforum_006 extends mvnforum_000
{
	var $_dependent = '005';

	function mvnforum_006($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'title',				$data['ThreadTopic']);
			...
			// Non mandatory
			$try->set_value('nonmandatory', 'open',				'1'); //  ThreadStatus?
			$try->set_value('nonmandatory', 'visible',			'1'); //  ThreadStatus?
			$try->set_value('nonmandatory', 'sticky',			'0');
			$try->set_value('nonmandatory', 'views',			$data['ThreadViewCount']);
			$try->set_value('nonmandatory', 'replycount',		$data['ThreadReplyCount']);
			$try->set_value('nonmandatory', 'attach',			$data['ThreadAttachCount']);
			$try->set_value('nonmandatory', 'postusername',		$data['MemberName']);
			$try->set_value('nonmandatory', 'dateline',			strtotime($data['ThreadCreationDate']));
			...
			if($try->is_valid())
			{
				if($try->import_thread($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['thread'] . ' -> ' . $data['ThreadTopic']);
					}
