...
class mvnforum_003 extends mvnforum_000
{
	var $_dependent = '001';

	function mvnforum_003($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}Groups", 'GroupID', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('nonmandatory', 'description',			$data['GroupDesc']);
