...
class siteframe_003 extends siteframe_000
{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$this->add_custom_field($Db_target, $tdt, $ttp, 'nickname','The siteframe nickname');
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_siteframe_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			$properties = $this->get_properties($user_details['user_props']);


			// Mandatory
			$try->set_value('mandatory', 'usergroupid',				$user_group_ids_array[69]);
			$try->set_value('mandatory', 'username',				$user_details['user_firstname'] . ' ' . $user_details['user_lastname']);
			...
			$try->set_value('nonmandatory', 'password',				$user_details['user_passwd']);
			$try->set_value('nonmandatory', 'options',				'2135');
			$try->set_value('nonmandatory', 'joindate',				strtotime($user_details['created']));
			$try->set_value('nonmandatory', 'lastvisit',			strtotime($properties[2][7]));
			$try->set_value('nonmandatory', 'homepage',				addslashes($properties[2][2]));

			$try->add_default_value('Location', 					addslashes($properties[2][3]));
			$try->add_custom_value('nickname', 						$user_details['user_nickname']);
