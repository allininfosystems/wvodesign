...
\*======================================================================*/
/**
* allaire_005 Import Post module
*
...
class allaire_005 extends allaire_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_allaire_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);


		// Get some refrence arrays (use and delete as nessesary).
		// User info
		$user_ids_array = $this->get_email_to_ids($Db_target, $target_database_type, $target_table_prefix, $do_int_val = false);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			$try->set_value('nonmandatory', 'username',			$user_ids_array[$email]['username']);
			$try->set_value('nonmandatory', 'title',			$post_details['Subject']);
			$try->set_value('nonmandatory', 'dateline',			strtotime($post_details['DateCreated']));
			...
			$try->set_value('nonmandatory', 'visible',			($post_details['IsDeleted'] == '1' ? 0 : 1));
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: post from -> ' . $this->iif($user_ids_array[$email]['username'], $user_ids_array[$email]['username'], 'Guest'));
