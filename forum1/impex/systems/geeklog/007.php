...
\*======================================================================*/
/**
* geeklog_007 Import Post module
*
...
*
*/
class geeklog_007 extends geeklog_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '006';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_geeklog_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			if ($post_details['pid'])
			{
				$thread_id = $this->get_geek_thread_id($Db_source, $source_database_type, $source_table_prefix, $post_details['pid']);
			}
			else
			{
				$thread_id = $post_details['id'];
			}

			// Mandatory
			$try->set_value('mandatory', 'threadid',			$thread_ids_array[$thread_id]);
			...
			$try->set_value('nonmandatory', 'username',			$post_details['name']);
			$try->set_value('nonmandatory', 'title',			$post_details['subject']);
			$try->set_value('nonmandatory', 'dateline',			$post_details['date']);
			if (strtolower($post_details['postmode']) == 'html')
			{
				$try->set_value('nonmandatory', 'pagetext',		$this->geeklog_html($this->html_2_bb($post_details['comment'])));
			}
			else
			{
				$try->set_value('nonmandatory', 'pagetext',		$this->geeklog_html($post_details['comment']));
			}
