...
\*======================================================================*/
/**
* ubb_threads_0010 Import Moderator module
*
...
*
*/
class ubb_threads_010 extends ubb_threads_000
{
	var $_dependent 	= '005';

	function ubb_threads_010(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($moderator_array as $id => $details)
		{
			$try = (phpversion() < '5' ? $moderator_object : clone($moderator_object));
			// Mandatory
			$try->set_value('mandatory', 'userid',				$idcache->get_id('user', $details['Mod_Uid']));
			$try->set_value('mandatory', 'forumid',				$forum_ids_array[$forum_word_array[$details['Mod_Board']]]);
			
	
			$try->set_value('mandatory', 'importmoderatorid',	$details['Mod_Uid']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span>' . $displayobject->phrases['moderator'] . ' -> ' . $user_ids_array["$details[Mod_Uid]"]);
