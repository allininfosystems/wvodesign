...
\*======================================================================*/
/**
* ubb_threads_003 Import Usergroup module
*
...
*
*/
class ubb_threads_003 extends ubb_threads_000
{
	var $_dependent 	= '001';


	function ubb_threads_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$usergroup_array 	= $this->get_ubb_threads_usergroup_details($Db_source, $source_database_type, $source_table_prefix, $usergroup_start_at, $usergroup_per_page);
		...
		foreach ($usergroup_array as $usergroup_id => $usergroup_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'importusergroupid',		$usergroup_details['G_Id']);


			// Non Mandatory
			$try->set_value('nonmandatory', 'title',		$usergroup_details['G_Name']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['usergroup'] . ' -> ' . $usergroup_details['G_Name']);
