...
\*======================================================================*/
/**
* ubb_threads_001 Check system module
*
...
*
*/
class ubb_threads_001 extends ubb_threads_000
{
	function ubb_threads_001(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		{
			$try1 = new ImpExData($Db_target, $sessionobject, 'usergroup');
			$try1->set_value('mandatory', 'importusergroupid',		'69');
			$try1->set_value('nonmandatory', 'title',				"{$displayobject->phrases['imported']} {$displayobject->phrases['users']}");
			$try1->import_usergroup($Db_target, $target_db_type, $target_table_prefix);

			$try2 = new ImpExData($Db_target, $sessionobject, 'usergroup');
			$try2->set_value('mandatory', 'importusergroupid',		'70');
			$try2->set_value('nonmandatory', 'title',				"{$displayobject->phrases['banlist']} {$displayobject->phrases['users']}");
			$try2->import_usergroup($Db_target, $target_db_type, $target_table_prefix);
