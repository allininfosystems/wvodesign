	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$cat['CAT_POS']);
		...
		else
		{
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'title', 				addslashes($forum['FORUM_NAME']));
				$try->set_value('mandatory', 'displayorder',		$forum['FORUM_POSITION']);
				...
				$try->set_value('nonmandatory', 'description', 		addslashes($forum['FORUM_DESC']));
				$try->set_value('nonmandatory', 'visible', 			'1');
				$try->set_value('mandatory', 'replycount',			$forum['FORUM_POSTS']);
				$try->set_value('mandatory', 'threadcount',			$forum['FORUM_TOPICS']);
				$try->set_value('mandatory', 'lastpost',			$forum['FORUM_LAST_POST']);
				$try->set_value('mandatory', 'options',				$this->_default_forum_permissions);

				$userid = str_replace('-', '', $forum['FORUM_LAST_POSTER']);
				$try->set_value('mandatory', 'lastposter',			$user_ids_array[$userid]);

				$try->set_value('mandatory', 'daysprune',			$forum['PRUNE_DAYS']);
