...
class photopost_003 extends photopost_000
{
	var $_dependent = '001';

	function photopost_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$usergroup_array 	= $this->get_photopost_usergroup_details($Db_source, $source_database_type, $source_table_prefix, $usergroup_start_at, $usergroup_per_page);
		...
		foreach ($usergroup_array as $usergroup_id => $usergroup_details)
		{
			...
			// Non Mandatory
			$try->set_value('nonmandatory','title',						"Imported: " . $usergroup_details['groupname']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['usergroup'] . ' -> ' . $usergroup_details['groupname']);
