...
class mmforum_006 extends mmforum_000
{
	var $_dependent = '005';

	function mmforum_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}tx_mmforum_topics", 'uid', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Non mandatory
			$try->set_value('nonmandatory', 'visible',		($data['hidden'] == 0 ? 1 : 0));
			#$try->set_value('nonmandatory', 'sticky',		$data['sticky']);
			$try->set_value('nonmandatory', 'views',		$data['topic_views']);
			$try->set_value('nonmandatory', 'open',			($data['closed_flag'] == 0 ? 1 : 0));
			$try->set_value('nonmandatory', 'replycount',	$data['topic_replies']);
			$try->set_value('nonmandatory', 'postusername',	$idcache->get_id('username', $data['topic_poster']));
			$try->set_value('nonmandatory', 'postuserid',	$idcache->get_id('user', $data['topic_poster']));
			$try->set_value('nonmandatory', 'dateline',		$data['crdate']);
			$try->set_value('nonmandatory', 'deletedcount',	$data['deleted']);
			...
			if($try->is_valid())
			{
				if($try->import_thread($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['thread'] . ' -> ' . $data['topic_title']);
					}
