...
*
*/
class drupal_003 extends drupal_000
{
	var $_dependent 	= '001';


	function drupal_003(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$user_array 	= $this->get_drupal_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			# Just roles atm and HTML enabeled
			#$extra_data = array();
			#$extra_data = unserialize($user_details['data']);
			...
			$try->set_value('nonmandatory', 'joindate',		$user_details['created']);
			$try->_password_md5_already = true;
			$try->set_value('nonmandatory', 'password', 	$user_details['pass']);

			if($user_details['signature'] != '')
			{
				$try->add_default_value('signature', 	$this->drupal_html($this->html_2_bb($user_details['signature'])));
			}
