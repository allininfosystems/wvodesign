...
class bbpress_003 extends bbpress_000
{
	var $_dependent = '001';

	function bbpress_003($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('nonmandatory', 'password',			$data['user_pass']);
			$try->set_value('nonmandatory', 'homepage',			$data['user_url']);
			$try->set_value('nonmandatory', 'joindate',			strtotime($data['user_registered']));
