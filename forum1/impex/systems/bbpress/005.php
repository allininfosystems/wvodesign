...
class bbpress_005 extends bbpress_000
{
	var $_dependent = '004';

	function bbpress_005(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('nonmandatory', 'sticky',		$data['topic_sticky']);
			$try->set_value('nonmandatory', 'open',			$data['topic_open']);
			$try->set_value('nonmandatory', 'replycount',	$data['topic_posts']);
			$try->set_value('nonmandatory', 'dateline',		strtotime($data['topic_time']));
...
# Autogenerated on : April 24, 2008, 9:43 am
