...
class bbpress_006 extends bbpress_000
{
	var $_dependent = '005';

	function bbpress_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$try->set_value('nonmandatory', 'pagetext',		$this->bbpress_html($this->html_2_bb($data['post_text'])));
			$try->set_value('nonmandatory', 'dateline',		strtotime($data['post_time']));
