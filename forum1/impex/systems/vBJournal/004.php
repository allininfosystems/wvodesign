...
class vBJournal_004 extends vBJournal_000
{
	var $_dependent = '003';

	function vBJournal_004(&$displayobject)
	{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			if ($this->_restart)
			{
				...
				{;
					$displayobject->display_now("<h4>{$displayobject->phrases['blog_texts_cleared']}</h4>");
					$this->_restart = true;
				}
				else
				{
					$sessionobject->add_error($Db_target, 'fatal', $class_num, 0, $displayobject->phrases['blog_text_restart_failed'], $displayobject->phrases['check_db_permissions']);
				}
			}

			// Start up the table
			$displayobject->update_basic('title',$displayobject->phrases['import_blog_text']);
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_vBJournal_blogs($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);

		// Display count and pass time
		$displayobject->print_per_page_pass($data_array['count'], $displayobject->phrases['blog_texts'], $start_at);

		$ImpExData_object = new ImpExData($Db_target, $sessionobject, 'blog_text');
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$blog_text->set_value('mandatory', 'bloguserid',		$idcache->get_id('user', $data['userid']));

			if(empty($data['entrytitle']))
			{
				$title = 'title';
			}
			else
			...
			$blog_text->set_value('mandatory', 'title',				$title);
			$blog_text->set_value('mandatory', 'pagetext',			$data['entrytext']);
			$blog_text->set_value('mandatory', 'dateline',			$data['entrydate']);
			$blog_text->set_value('mandatory', 'userid',			$idcache->get_id('user', $data['userid']));
			...
			$blog_text->set_value('nonmandatory', 'username',		$idcache->get_id('username', $data['userid']));
			$blog_text->set_value('nonmandatory', 'ipaddress',		$data['ipaddress']);
			$blog_text->set_value('nonmandatory', 'reportthreadid',	"0");
			$blog_text->set_value('nonmandatory', 'state',			($data['entry_active'] == 1 ? 'visible' : 'moderation')); // enum('moderation', 'visible', 'deleted')
			...
			$blog->set_value('mandatory', 'dateline',				$data['entrydate']);
			#$blog->set_value('mandatory', 'options',				$data['options']);
			$blog->set_value('mandatory', 'title',					$title);
			$blog->set_value('mandatory', 'importblogid',			$import_id);

			// Non mandatory
			$blog->set_value('nonmandatory', 'blogcategory',		$idcache->get_id('blogcategory', "69")); # Default
			...
			$blog->set_value('nonmandatory', 'state',				($data['entry_active'] == 1 ? 'visible' : 'moderation'));
				...
				if($blog_text_id = $blog_text->import_blog_text($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					if($blog->is_valid())
					{
						if($blog->import_blog($Db_target, $t_db_type, $t_tb_prefix))
						{
							...
							{
								$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $blog->how_complete() . '%</b></span> ' . $displayobject->phrases['blog'] . ' -> ' . $data['entrytitle']);
							}
