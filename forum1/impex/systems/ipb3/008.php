...
class ipb3_008 extends ipb3_000
{
	...
	function ipb3_008(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$poll_array 	= $this->get_ipb3_poll_details($Db_source, $source_database_type, $source_table_prefix, $poll_start_at, $poll_per_page);
		...
		foreach ($poll_array as $poll_id => $poll)
		{
			...
			$ipb_voters_array		= $this->get_ipb3_vote_voters($Db_source, $source_database_type, $source_table_prefix, $poll['tid']);
			...
			// Now occasional slashing ? ......
			if (use_utf8_decode)
			{
				$polls = unserialize(stripslashes(utf8_decode($poll['choices'])));
			}
			else 
			{
				$polls = unserialize(stripslashes($poll['choices']));
			}

			if(is_array($polls))
			{
				foreach ($polls as $pollid => $single_poll)
				{
					$numberoptions = count($single_poll['choice']);
					$voters = array_sum($single_poll['votes']);
	
					$question = $single_poll['question'];
	
					$options = implode('|||', $single_poll['choice']);
					$votes = implode('|||', $single_poll['votes']);
				}
			}
			else
			{
			...
			foreach ($ipb_voters_array AS $vid => $info)
			{
				$memberid = $user_ids_array["$info[member_id]"];
			...
			if (use_utf8_decode)
			{
				$try->set_value('mandatory', 'question',			(utf8_decode($poll['poll_question']) ? utf8_decode($poll['poll_question']) : $question));
			}
			else 
			{
				$try->set_value('mandatory', 'question',			($poll['poll_question'] ? $poll['poll_question'] : $question));
			}
			...
			if($try->is_valid())
			{
				...
				if($result)
				{
					if($try->import_poll_to_thread($Db_target, $target_database_type, $target_table_prefix, $vb_poll_id, $poll['tid']))
					{
