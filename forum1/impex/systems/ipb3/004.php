...
class ipb3_004 extends ipb3_000
{
	...
	function ipb3_004(&$displayobject)
	{
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$displayobject->update_html($displayobject->make_yesno_code($displayobject->phrases['get_avatars'], 'get_avatars', $sessionobject->get_session_var('get_avatars')));
			...
			$displayobject->update_html($displayobject->make_input_code($displayobject->phrases['custom_avatar_path'], 'get_customavatars_path', $sessionobject->get_session_var('get_avatars_path'), 1, 60));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$avatarpath = $sessionobject->get_session_var('get_avatars_path');
		$customavatarpath = $sessionobject->get_session_var('get_customavatars_path');

		if ($sessionobject->get_session_var('get_avatars') AND !$sessionobject->get_session_var('userstartat'))
		{
			if (!is_dir($avatarpath))
			{
				$displayobject->display_error(sprintf($displayobject->phrases['path_x_not_found'], htmlspecialchars_uni($avatarpath)));
				exit;
			}

			if (!is_dir($customavatarpath))
			{
				$displayobject->display_error(sprintf($displayobject->phrases['path_x_not_found'], htmlspecialchars_uni($customavatarpath)));
				exit;
			}
		}

		// Ensure there is a trailing slash
		if($avatarpath{strlen($avatarpath)-1} != '/')  $avatarpath .= '/';
		if($customavatarpath{strlen($customavatarpath)-1} != '/')  $customavatarpath .= '/';
		...
		$user_array 	= $this->get_ipb3_user_details($Db_source, $source_database_type, $source_table_prefix, $user_start_at, $user_per_page);
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'usergroupid',					$user_group_ids_array["$user_details[member_group_id]"]);
			...
			{
				if ($user_details['avatar_type'] == 'upload')
				{
					$try->set_value('nonmandatory', 'avatar', $customavatarpath . $avatar);
					#echo " - Set Avatar (upload): " . htmlspecialchars_uni($customavatarpath . $avatar);
				}
				else if ($user_details['avatar_type'] == 'local')
				{
					$try->set_value('nonmandatory', 'avatar', $avatarpath . $avatar);
					#echo " - Set Avatar (local): " . htmlspecialchars_uni($avatarpath . $avatar);
				}
				else if (substr($avatar, 0, 7) == 'http://')
				{
					// Must be a URL, give it a go.
					$try->set_value('nonmandatory', 'avatar', 		$avatar);
					#echo " - Set Avatar (url): " .  htmlspecialchars_uni($avatar);
				}
				else
				{
			...
			$try->add_default_value('signature', 					$this->ipb3_html($this->html_2_bb(html_entity_decode($user_details['signature']))));
