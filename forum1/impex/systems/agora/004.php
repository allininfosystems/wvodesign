...
class agora_004 extends agora_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '003';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$top = NULL; // For the default category id if it isn't in the forum	
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_agora_category_details($Db_source, $source_database_type, $source_table_prefix);
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'title', 				$cat['bn_title']);
				$try->set_value('mandatory', 'displayorder',		$cat['rank']);
				...
				$try->set_value('nonmandatory', 'description', 		$forum['descr']);
				...
				if($try->is_valid())
				{
					...
					else
					{
						$sessionobject->add_error('warning',
												 $this->_modulestring,
												 get_class($this) . "::import_category failed for " . $cat['bn_title'] . " get_phpbb2_categories_details was ok.",
												 'Check database permissions and user table');
						$sessionobject->set_session_var($class_num . '_objects_failed',$sessionobject->get_session_var($class_num. '_objects_failed') + 1 );
						$displayobject->display_now("<br />Got category " . $cat['bn_title'] . " and <b>DID NOT</b> imported to the " . $target_database_type . " database");
					}
				}
				else
				{
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_agora_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'title', 				$forum['bn_title']);
				$try->set_value('mandatory', 'displayorder',		$forum['rank']);
				if ($cat_ids["$forum[parent]"])
				{
					$try->set_value('mandatory', 'parentid',		$cat_ids["$forum[parent]"]);
				}
				else
				{
					if(!$top)
					{
						$top = array_pop($cat_ids);
					}
					$try->set_value('mandatory', 'parentid',		$top);
				}
				...
				$try->set_value('mandatory', 'threadcount',			$forum['totalthreads']);
				$try->set_value('mandatory', 'replycount',			$forum['totalnotes']);
