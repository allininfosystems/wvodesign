...
class dragonfly_004 extends dragonfly_000
{
	var $_dependent = '003';

	function dragonfly_004($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			if($user_group_ids_array["$data[user_group_cp]"])
			{
				$try->set_value('mandatory', 'usergroupid',			$user_group_ids_array["$data[user_group_cp]"]);	
			}
			else
			{
			...
			$try->set_value('nonmandatory', 'joindate',				strtotime($data['user_regdate']));
			...
			$try->add_default_value('signature', 					$this->html_2_bb($user['user_sig'])); 
