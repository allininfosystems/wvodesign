...
class dragonfly_005 extends dragonfly_000
{
	var $_dependent = '004';

	function dragonfly_005($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_dragonfly_cat_details($Db_source, $s_db_type, $s_tb_prefix);
		...
		{
			// Sort out the forums
			$data_array  	= $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}bbforums", 'forum_id', 0, $start_at, $per_page);
			...
			foreach ($data_array['data'] as $import_id => $data)
			{
				...
				// Mandatory
				if ($cat_ids["$data[cat_id]"])
				{
					$try->set_value('mandatory', 'parentid',			$cat_ids["$data[cat_id]"]);
				}
				else 
				{
					$forum_ids_array = $this->get_forum_ids($Db_target, $t_db_type, $t_tb_prefix);
					$try->set_value('mandatory', 'parentid',			$forum_ids_array["$data[parent_id]"]);
				}
