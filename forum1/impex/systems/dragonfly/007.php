...
class dragonfly_007 extends dragonfly_000
{
	var $_dependent = '006';

	function dragonfly_007($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_dragonfly_posts($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Non mandatory
			$try->set_value('nonmandatory', 'attach',			$data['post_attachment']);
			$try->set_value('nonmandatory', 'visible',			'1');
			#$try->set_value('nonmandatory', 'iconid',			$data['iconid']);
			$try->set_value('nonmandatory', 'showsignature',	$data['enable_sig']);
			$try->set_value('nonmandatory', 'allowsmilie',		$data['enable_smilies']);
			$try->set_value('nonmandatory', 'pagetext',			$data['post_text']);
			...
			if($try->is_valid())
			{
				if($try->import_post($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['post'] . ' -> ' . $data['post_subject']);
					}
