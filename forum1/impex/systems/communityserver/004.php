...
\*======================================================================*/
/**
* communityserver_004 Import Forum module
*
...
*
*/
class communityserver_004 extends communityserver_000
{
	var $_dependent 	= '003';

	function communityserver_004(&$displayobject)
	{
		...
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			if ($this->_restart)
			{
				if ($this->restart($sessionobject, $displayobject, $Db_target, $Db_source,'clear_imported_forums'))
				{
					...
					$sessionobject->add_session_var('added_default_cat', 'nope');
					$this->_restart = true;
				}
				else
				{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$forum_array 	= $this->get_communityserver_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
		...
		{
			// Default Category for all the forums
			$try = (phpversion() < '5' ? $forum_object : clone($forum_object));
			$try->set_value('mandatory', 'title', 				'Default Category');
		...
		foreach ($forum_array as $forum_id => $forum_details)
		{
			...
			$try->set_value('mandatory', 'displayorder',			($forum_details['SortOrder'] == 0 ? 1 : $forum_details['SortOrder']));
			
			if($forum_details['ParentID'] != 0)
			{
				$forum_ids_array = $this->get_forum_ids($Db_target, $target_database_type, $target_table_prefix);
				if ($forum_ids_array["$forum_details[ParentID]"])
				{
					$try->set_value('mandatory', 'parentid',				$forum_ids_array["$forum_details[ParentID]"]);
				}
				else 
				{
			...
			// Non Mandatory
			$try->set_value('nonmandatory', 'threadcount',		$forum_details['TotalThreads']);
			$try->set_value('nonmandatory', 'description',		$forum_details['Description']);
			$try->set_value('nonmandatory', 'replycount',		$forum_details['TotalPosts']);
			
			// Can get the last X details, though threads & forums update will get all that.
