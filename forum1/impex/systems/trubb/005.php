...
*
*/
class trubb_005 extends trubb_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '004';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_trubb_cat_details($Db_source, $source_database_type, $source_table_prefix);
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('nonmandatory', 'description', 		'The ' . $cat['name'] . ' forum');
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_trubb_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'parentid',			$cat_ids["$forum[cid]"]);
				$try->set_value('mandatory', 'importforumid',		$forum['fid']);
				$try->set_value('mandatory', 'importcategoryid',	$forum['cid']);

				$try->set_value('nonmandatory', 'description',		$forum['name']);
				$try->set_value('nonmandatory', 'threadcount',		$forum['topics']);
				$try->set_value('nonmandatory', 'replycount',		$forum['replies']);
