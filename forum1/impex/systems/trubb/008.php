...
\*======================================================================*/
/**
* trubb_008 Import Attachment module
*
* @package			ImpEx.trubb
...
*
*/
class trubb_008 extends trubb_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '007';
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$displayobject->update_html($displayobject->make_input_code('Full Path to TruBB uploads folder.','attachmentsfolder',$sessionobject->get_session_var('attachmentsfolder'),1,60));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$attachment_array 	= $this->get_trubb_attachment_details($Db_source, $source_database_type, $source_table_prefix, $attachment_start_at, $attachment_per_page);
		...
		foreach ($attachment_array as $attachment_id => $attachment_details)
		{
			...
			$full_path = $dir . '/' . $attachment_details['filename'] . '-' . $attachment_details['postdate'] . '.ext';
			if(!is_file($full_path))
			{
				$displayobject->display_now('<br /><b>Source file not found </b> :: attachment -> ' . $attachment_details['filename'] . '.' . $attachment_details['ext']);
			...
			// Mandatory
			$try->set_value('mandatory', 'filename',				addslashes($attachment_details['filename'] . '.' . $attachment_details['ext']));
			...
			$try->set_value('nonmandatory', 'dateline',				$attachment_details['postdate']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: attachment -> ' . $attachment_details['filename'] . '.' . $attachment_details['ext']);
