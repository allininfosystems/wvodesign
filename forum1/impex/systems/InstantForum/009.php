...
\*======================================================================*/
/**
* InstantForum_009 Import Attachment module
*
...
*
*/
class InstantForum_009 extends InstantForum_000
{
	var $_dependent 	= '007';


	function InstantForum_009(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$attachment_array 	= $this->get_InstantForum_attachment_details($Db_source, $source_database_type, $source_table_prefix, $attachment_start_at, $attachment_per_page);
		...
		foreach ($attachment_array as $attachment_id => $attachment_details)
		{
			...
			$try->set_value('mandatory', 'filedata',			$attachment_details['AttachmentBLOB']);


			// Non Mandatory
			$try->set_value('nonmandatory', 'extension',		substr($attachment_details['FileName'], strrpos($attachment_details['FileName'], '.')+1));
			$try->set_value('nonmandatory', 'filehash',			md5($attachment_details['AttachmentBLOB']));
			$try->set_value('nonmandatory', 'postid',			$attachment_details['PostID']);
			$try->set_value('nonmandatory', 'filesize',			$attachment_details['ContentLength']);
