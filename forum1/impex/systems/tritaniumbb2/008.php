...
\*======================================================================*/
/**
* tritaniumbb2_008 Import Post module
*
...
*
*/
class tritaniumbb2_008 extends tritaniumbb2_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '007';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$post_array 	= $this->get_tritaniumbb2_post_details($Db_source, $source_database_type, $source_table_prefix, $post_start_at, $post_per_page);
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			$try->set_value('nonmandatory', 'username',			$user_name_array["$post_details[poster_id]"]);
			...
			$try->set_value('nonmandatory', 'allowsmilie',		$post_details['post_enable_smilies']);
			$try->set_value('nonmandatory', 'showsignature',	$post_details['post_show_sig']);
			$try->set_value('nonmandatory', 'ipaddress',		$post_details['post_ip']);
			$try->set_value('nonmandatory', 'iconid',			$post_details['post_pic']);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: post -> ' . $post_details['post_title']);
