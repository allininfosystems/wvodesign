...
\*======================================================================*/
/**
* tritaniumbb2_011 Import Smilie module
*
...
*
*/
class tritaniumbb2_011 extends tritaniumbb2_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '001';
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			$displayobject->update_html($displayobject->make_yesno_code("Would you like the tritanium smilies to over write the vB ones if there is a duplication ?","over_write_smilies",1));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$smilie_array 	= $this->get_tritaniumbb2_smilie_details($Db_source, $source_database_type, $source_table_prefix, $smilie_start_at, $smilie_per_page);
		...
		foreach ($smilie_array as $smilie_id => $smilie_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'smilietext',			$smilie_details['smiley_synonym']);
			$try->set_value('mandatory', 'importsmilieid',		$smilie_id);


			// Non Mandatory
			$try->set_value('nonmandatory', 'title',			$smilie_details['smiley_gfx']);
			$try->set_value('nonmandatory', 'smiliepath',		$smilie_details['smiley_gfx']);
			$try->set_value('nonmandatory', 'imagecategoryid',	$smilie_group_id);
			$try->set_value('nonmandatory', 'displayorder',		$smilie_details['smiley_id']);
