...
\*======================================================================*/
/**
* tritaniumbb2_006 Import Forum module
*
...
*
*/
class tritaniumbb2_006 extends tritaniumbb2_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_tritaniumbb2_cats_details($Db_source, $source_database_type, $source_table_prefix);
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$cat['cat_l']);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_tritaniumbb2_forum_details($Db_source, $source_database_type, $source_table_prefix, $forum_start_at, $forum_per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$forum['order_id']);
				...
				$try->set_value('mandatory', 'threadcount',			$forum['forum_topics_counter']);
