...
\*======================================================================*/
/**
* tritaniumbb2_007 Import Thread module
*
...
*
*/
class tritaniumbb2_007 extends tritaniumbb2_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '006';
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$thread_array 	= $this->get_tritaniumbb2_thread_details($Db_source, $source_database_type, $source_table_prefix, $thread_start_at, $thread_per_page);
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			$try->set_value('nonmandatory', 'dateline',			$thread_details['topic_post_time']);
			$try->set_value('nonmandatory', 'views',			$thread_details['topic_views_counter']);
			$try->set_value('nonmandatory', 'iconid',			$thread_details['topic_pic']);
			$try->set_value('nonmandatory', 'visible',			'1');
			$try->set_value('nonmandatory', 'sticky',			$thread_details['topic_is_pinned']);
			$try->set_value('nonmandatory', 'postusername',		$user_name_array["$thread_details[poster_id]"]);
			$try->set_value('nonmandatory', 'postuserid',		$user_ids_array["$thread_details[poster_id]"]);
