...
\*======================================================================*/
/**
* tritaniumbb2_010 Import Pmtext module
*
...
*
*/
class tritaniumbb2_010 extends tritaniumbb2_000
{
	var $_version 		= '0.0.1';
	var $_dependent 	= '005';
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			...
			// Ask some questions
			$displayobject->update_html($displayobject->make_input_code('Pmtexts to import per cycle (must be greater than 1)','pmtextperpage',100));
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$pmtext_array 	= $this->get_tritaniumbb2_pmtext_details($Db_source, $source_database_type, $source_table_prefix, $pmtext_start_at, $pmtext_per_page);
		...
		foreach ($pmtext_array as $pm_id => $pm)
		{
			...
			$userid 	= $user_ids_array["$pm[pm_to_id]"];
			$username	= $user_name_array["$pm[pm_to_id]"];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$user_ids_array["$pm[pm_from_id]"]);
			$vB_pm_text->set_value('mandatory', 'title',			$pm['pm_subject']);
			$vB_pm_text->set_value('mandatory', 'message',			$this->html_2_bb($pm['pm_text']));
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$user_name_array["$pm[pm_from_id]"]);
			$vB_pm_text->set_value('nonmandatory', 'iconid',		'');
			$vB_pm_text->set_value('nonmandatory', 'dateline',		$pm['pm_send_time']);
			$vB_pm_text->set_value('nonmandatory', 'showsignature',	$pm['pm_show_sig']);
			$vB_pm_text->set_value('nonmandatory', 'allowsmilie',	$pm['pm_enable_smilies']);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$user_ids_array["$pm[pm_to_id]"]);
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$user_ids_array["$pm[pm_from_id]"]);
					...
					if($vB_pm_text->is_valid())
					{
						...
						{
							$displayobject->display_now('<br /><span class="isucc"><b>' . $vB_pm_text->how_complete() . '%,</b></span> Imported from : <b>' . $user_name_array[$pm['pm_from_id']] . '</b> , to : <b>' . $user_name_array[$pm['pm_to_id']] . '</b>');
