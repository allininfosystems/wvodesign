...
class fusionbb_005 extends fusionbb_000
{
	var $_dependent = '004';

	function fusionbb_005(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			$categories_array = $this->get_fusionbb_categories_details($Db_source, $s_db_type, $s_tb_prefix);
		...
		{
			// Sort out the forums
			$forum_array  	=  $this->get_fusionbb_forum_details($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
			...
			foreach ($forum_array as $forum_id => $forum)
			{
				...
				if($forum['forum_parent_id'] == 0)
				{
					$try->set_value('mandatory', 'parentid',		$cat_ids[99999]);
				}
				else
				{
					if ($cat_ids["$forum[forum_parent_id]"] != 0)
					{
