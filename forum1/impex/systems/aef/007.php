...
class aef_007 extends aef_000
{
	var $_dependent = '006';

	function aef_007($displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}posts", 'pid', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'threadid',			$idcache->get_id('thread', $data['post_tid']));
			$try->set_value('mandatory', 'importthreadid',		$data['post_tid']);
			...
			$try->set_value('nonmandatory', 'pagetext',			html_entity_decode($data['post']));
			$try->set_value('nonmandatory', 'dateline',			$data['ptime']);
			$try->set_value('nonmandatory', 'title',			html_entity_decode($data['post_title']));
			...
			if($try->is_valid())
			{
				if($try->import_post($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['post'] . ' -> ' . $data['post_title']);
					}
