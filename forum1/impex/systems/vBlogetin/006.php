...
class vBlogetin_006 extends vBlogetin_000
{
	var $_dependent = '004';

	function vBlogetin_006(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_source_data($Db_source, $s_db_type, "{$s_tb_prefix}blog_comment", 'commentid', 0, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			// Mandatory blog_text
			$try->set_value('mandatory', 'blogid',				$idcache->get_id('blog', $data['entryid']));
			$try->set_value('mandatory', 'bloguserid',			$idcache->get_id('user', $data['bloguserid']));
			$try->set_value('mandatory', 'title',				" ");
			$try->set_value('mandatory', 'pagetext',			$data['body']);
			...
			if($try->is_valid())
			{
				...
				if($try->import_blog_text($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					{
						$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $try->how_complete() . '%</b></span> ' . $idcache->get_id('username', $data['userid']));
					}
