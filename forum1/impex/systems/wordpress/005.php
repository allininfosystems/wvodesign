...
class wordpress_005 extends wordpress_000
{
	var $_dependent = '004';

	function wordpress_005(&$displayobject)
	{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		// Get an array data
		$data_array = $this->get_wordpress_posts($Db_source, $s_db_type, $s_tb_prefix, $start_at, $per_page);
		...
		foreach ($data_array['data'] as $import_id => $data)
		{
			...
			$blog_text->set_value('mandatory', 'bloguserid',		$idcache->get_id('user', $data['post_author']));
			$blog_text->set_value('mandatory', 'title',				$data['post_title']);
			$blog_text->set_value('mandatory', 'pagetext',			$this->html_2_bb($data['post_content']));
			$blog_text->set_value('mandatory', 'dateline',			strtotime($data['post_date']));
			$blog_text->set_value('mandatory', 'userid',			$idcache->get_id('user', $data['post_author']));
			...
			$blog_text->set_value('nonmandatory', 'username',		$idcache->get_id('username', $data['post_author']));
			#$blog_text->set_value('nonmandatory', 'ipaddress',		$data['ipaddress']);
			$blog_text->set_value('nonmandatory', 'reportthreadid',	"0");
			$blog_text->set_value('nonmandatory', 'state',			($data['post_status'] == 'publish' ? 'visible' : 'moderation')); // enum('moderation', 'visible', 'deleted')
			...
			$blog->set_value('nonmandatory', 'username',			$idcache->get_id('username', $data['post_author']));
			$blog->set_value('nonmandatory', 'views',				$data['hits']);
			$blog->set_value('nonmandatory', 'state',				($data['post_status'] == 'publish' ? 'visible' : 'moderation'));
			$blog->set_value('nonmandatory', 'views',				$data['hits']);
			$blog->set_value('nonmandatory', 'blogcategoryid',		$idcache->get_id('blogcategory', $this->get_wordpress_catid($Db_source, $s_db_type, $s_tb_prefix, $import_id)));
				...
				if($blog_text_id = $blog_text->import_blog_text($Db_target, $t_db_type, $t_tb_prefix))
				{
					...
					if($blog->is_valid())
					{
						if($blog->import_blog($Db_target, $t_db_type, $t_tb_prefix))
						{
							...
							{
								$displayobject->display_now('<br /><span class="isucc">' . $import_id . ' :: <b>' . $blog->how_complete() . '%</b></span> ' . $displayobject->phrases['blog'] . ' -> ' . $data['post_title']);
							}
