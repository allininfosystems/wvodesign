	...
	*/
	function clear_imported_blog_users(&$Db_object, &$databasetype, &$tableprefix)
	{
		switch ($databasetype)
		{
			...
			case 'mysql':
			{
				$users = $Db_object->query("SELECT importbloguserid FROM " . $tableprefix  . "blog_user WHERE importbloguserid <> 0");
				...
				if ($Db_object->num_rows($users))
				{
					...
					{
						$removeid[] = $user['importbloguserid'];
					}
					...
					// user
					$Db_object->query("DELETE FROM " . $tableprefix  . "blog_user WHERE bloguserid IN(" . $ids . ")");
	...
	*/
	function clear_imported_blogs(&$Db_object, &$databasetype, &$tableprefix)
	{
		switch ($databasetype)
		{
			...
			case 'mysql':
			{
				$blogs = $Db_object->query("SELECT blogtextid FROM " . $tableprefix  . "blog_text WHERE importblogtextid  <> 0");
	...
	function clear_imported_blog_attachments(&$Db_object, &$databasetype, &$tableprefix)
	{
		switch ($databasetype)
		{
			// MySQL database
			case 'mysql':
			{
				$blogs = $Db_object->query("DELETE FROM {$tableprefix}blog_attachment WHERE importblogattachmentid  <> 0");

				$Db_object->query("ALTER TABLE " . $tableprefix  . "blog_attachment AUTO_INCREMENT=0");
				$Db_object->query("ALTER TABLE " . $tableprefix  . "blog_attachment auto_increment=0");
