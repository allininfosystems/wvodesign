...
if ($_REQUEST['do'] == 'view' AND can_access_logs($vbulletin->config['SpecialUsers']['canviewadminlog'], 1, '<p>' . $vbphrase['control_panel_log_viewing_restricted'] . '</p>'))
{
	...
	if ($db->num_rows($logs))
	{
		...
		while ($log = $db->fetch_array($logs))
		{
			...
			$cell[] = htmlspecialchars_uni(print_r(vb_unserialize($log['paramget']), true));
...
if ($_REQUEST['do'] == 'choose')
{
	...
	if ($show_admin_log)
	{
		...
		{
			//we could pass a flag to print_select_row to do this, but this is more obvious and gets it closer to the source
			$clientnamelist["$clientname[clientname]"] = htmlspecialchars_uni($clientname['clientname']);
		}
		$users = $db->query_read("
...
if ($_REQUEST['do'] == 'viewclient')
{
	...
	print_label_row($vbphrase['apiclientname'], htmlspecialchars_uni($client['clientname']));
	print_label_row($vbphrase['apiclientversion'], htmlspecialchars_uni($client['clientversion']));
	print_label_row($vbphrase['apiclient_platformname'], htmlspecialchars_uni($client['platformname']));
	print_label_row($vbphrase['apiclient_platformversion'], htmlspecialchars_uni($client['platformversion']));
	print_label_row($vbphrase['apiclient_uniqueid'], htmlspecialchars_uni($client['uniqueid']));
