...
define('CVS_REVISION', '$RCSfile$ - $Revision: 93145 $');
define('NOZIP', true);
...
function construct_stylevar_form($title, $stylevarid, $values, $styleid, $masterstyleid)
{
	...
	$svinstance->set_value(vb_unserialize($stylevar['value']));	// remember, our value in db is ALWAYS serialized!
...
if ($_POST['do'] == 'dfn_dosave')
{
	...
	else
	{
		...
		$svdata = vb_unserialize($stylevar_dfn['value']);
	}
...
if ($_POST['do'] == 'savestylevar')
{
	...
	foreach ($vbulletin->GPC['stylevar'] AS $stylevarid => $value)
	{
		...
		{
			$original_value = vb_unserialize($stylevars[$stylevarid][$styleid]['value']);
		}
		else
		{
			// get inherited value
...
function handle_stylevar_delete(e)
{
	...
	if (selected.length != 0)
	{
		...
		var url = "stylevar.php?" + SESSIONURL + "adminhash=" + ADMINHASH + "&do=confirmrevert&" + request_string + "dostyleid=" + ' . $vbulletin->GPC['dostyleid'] . ';
...
function handle_stylevar_click(e)
{
	...
	if (selected.length != 0)
	{
		...
		var url = "stylevar.php?" + SESSIONURL + "adminhash=" + ADMINHASH + "&do=fetchstylevareditor&" + request_string + "dostyleid=" + ' . $vbulletin->GPC['dostyleid'] . ';
		var editorpane = YAHOO.util.Dom.get("edit_scroller");
		editorpane.src = url;
	}
}
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 10:21, Mon Dec 11th 2017 : $Revision: 93145 $
