...
class vBDBSearch_Indexer extends vB_Search_ItemIndexer
{
	...
	public function index($fields)
	{
		...
		$string_fields = array('username', 'ipaddress');
		$int_fields = array('contenttypeid', 'primaryid', 'groupcontenttypeid', 
		'groupid', 'userid', 'dateline', 'searchgroupid');
		...
		/* Update to set the IP address if its missing, or an old style integer gets passed.
		This could happen if a 3rd party search system is being used, that still sends the ip2long value. */
		if (empty($fields['ipaddress']))
		{
			$fields['ipaddress'] = IPADDRESS;
		}
		else if (get_iptype($fields['ipaddress']) == 0)
		{
			$tempip = long2ip($fields['ipaddress']);

			if (get_iptype($tempip))
			{
				$fields['ipaddress'] = $tempip;
			}
			else
			{
				$fields['ipaddress'] = IPADDRESS;
		...
		//index performance proved to be poor.  
	}
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 10:21, Mon Dec 11th 2017 : $Revision: 92429 $
|| # $Date: 2017-01-16 05:22:53 -0800 (Mon, 16 Jan 2017) $
|| ####################################################################
\*======================================================================*/
