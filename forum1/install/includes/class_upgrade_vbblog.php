...
class vB_Upgrade_vbblog extends vB_Upgrade_Version
{
	...
	*/
	function step_4()
	{
		...
		$featured = vb_unserialize($this->registry->options['vbblog_featured']);
	...
	*/
	...
	/**
	* Step #17 - 4.2.5 Beta 2
	* Update blog_trackbacklog for IPv6
	*/
	function step_17()
	{
		if (!$this->verify_product_version('4.2.5 Beta 2'))
		{
			return;
		}

		if ($this->field_exists('blog_trackbacklog', 'ip'))
		{
			$this->run_query(
				sprintf($this->phrase['core']['altering_x_table'], 'blog_trackbacklog', 1, 2),
				"TRUNCATE TABLE " . TABLE_PREFIX . "blog_trackbacklog"
			);

			$this->run_query(
				sprintf($this->phrase['core']['altering_x_table'], 'blog_trackbacklog', 2, 2),
				"ALTER TABLE " . TABLE_PREFIX . "blog_trackbacklog CHANGE ipaddress ipaddress VARCHAR(45) NOT NULL DEFAULT ''"
			);
		}
		else
		{
	...
	/**
	* Step #18 - 4.2.5 Beta 2
	* Update blog_text IP Data
	 */
	public function step_18()
	{
		...
		while ($record = $this->db->fetch_array($records))
		{
			...
			$blogtextid = $record['blogtextid'];
			if ($blogtextid == $record['firstblogtextid'])
			{
				$contenttypeid = vB_Types::instance()->getContentTypeID('vBBlog_BlogEntry');
			}
			else // Comment ..
			{
				$contenttypeid = vB_Types::instance()->getContentTypeID('vBBlog_BlogComment');
			}

			$ipman->set('rectype', 'content');
			$ipman->set('contentid', $blogtextid);
			$ipman->set('contenttypeid', $contenttypeid);

			$ipaddr = long2ip($record['ipaddress']);
			$ipman->set('userid', $record['userid']);
			...
			$ipid = $ipman->save();
			$ipman->update_content('blog_text', 'blogtextid', $ipid, $blogtextid);
			unset($ipman);
		}

		$this->show_message(sprintf($this->phrase['vbphrase']['update_table'], TABLE_PREFIX . 'blog_text'));
	}
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 10:21, Mon Dec 11th 2017 : $Revision: 94259 $
|| # $Date: 2017-06-14 11:29:25 -0700 (Wed, 14 Jun 2017) $
|| ####################################################################
\*======================================================================*/
