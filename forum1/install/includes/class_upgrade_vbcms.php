...
class vB_Upgrade_vbcms extends vB_Upgrade_Version
{
	...
	*/
	public function __construct(&$registry, $phrase, $maxversion)
	{
		...
		$this->db->hide_errors();
			$this->db->query_write(
				"ALTER TABLE " . TABLE_PREFIX . "cms_widgettype ADD UNIQUE INDEX class (class, packageid)"
			);
			$this->db->query_write(
				"ALTER TABLE " . TABLE_PREFIX . "cms_permissions ADD UNIQUE INDEX usergroupid (usergroupid, nodeid)"
			);
		$this->db->show_errors();
	}
