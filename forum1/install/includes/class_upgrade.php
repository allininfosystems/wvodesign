...
*
* @package 		vBulletin
* @version		$Revision: 93876 $
* @date 		$Date: 2017-04-29 04:50:15 -0700 (Sat, 29 Apr 2017) $
*
*/
class vB_Upgrade
{
...
abstract class vB_Upgrade_Abstract
{
	...
	*/
	protected function verify_environment()
	{
		...
		if (!defined('SKIPDB') AND version_compare(MYSQL_VERSION, '5.5.8', '<'))
		{
	...
	protected function verify_files()
	{
		...
		if (file_exists(DIR . '/includes/md5_sums_vbulletin.php'))
		{
			...
			$md5_sum_versions = array('vbulletin' => '4.2.5');
...
abstract class vB_Upgrade_Version
{
	...
	public function execute($check_table = true, $result = null)
	{
		...
		foreach ($this->modifications AS $modification)
		{
			...
			switch ($modification['modification_type'])
			{
				...
				case 'fatal_error':
						$this->add_error('Aborted', 999, true);
					break;
	...
	*/
	...
	/**
	* Aborts Upgrade with an error message.
	*
	* @param	string	Message to display
	*/
	public function abort($message)
	{
		$this->show_message("Upgrade Aborted : $message");

		$this->modifications[] = array(
			'modification_type' => 'fatal_error',
			'data'              => array()
		);
	}
		...
		if ($complete)
		{
			...
			if ($this->SHORT_VERSION == 'final' OR $only)
			{
				$this->db->query_write("
					...
					WHERE script IN ('final', 'vbblog', 'vbcms', 'skimlinks', 'forumrunner')
				");
